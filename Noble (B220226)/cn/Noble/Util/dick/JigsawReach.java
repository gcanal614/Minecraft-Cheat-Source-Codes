...
//	static ArrayList<Vec3> positionsBack = new ArrayList<Vec3>();
	...
	public static boolean infiniteReach(double range, double maxXZTP, double maxYTP, 
		...
		if ((rayTraceWide(new Vec3(mc.player.posX, mc.player.posY, mc.player.posZ),
				new Vec3(en.posX, en.posY, en.posZ), false, false, true))
				|| (rayTrace1 = rayTracePos(
						new Vec3(mc.player.posX, mc.player.posY + mc.player.getEyeHeight(), mc.player.posZ),
						new Vec3(en.posX, en.posY + mc.player.getEyeHeight(), en.posZ), false, false,
						true)) != null) {
			if ((rayTrace = rayTracePos(new Vec3(mc.player.posX, mc.player.posY, mc.player.posZ),
					new Vec3(en.posX, mc.player.posY, en.posZ), false, false, true)) != null
					|| (rayTrace1 = rayTracePos(
							new Vec3(mc.player.posX, mc.player.posY + mc.player.getEyeHeight(),
									mc.player.posZ),
							new Vec3(en.posX, mc.player.posY + mc.player.getEyeHeight(), en.posZ), false, false,
							true)) != null) {
					...
					if (trace.getBlockPos() != null) {
						...
						for (int i = 0; i < maxYTP; i++) {
							MovingObjectPosition tr = rayTracePos(
									new Vec3(mc.player.posX, target.getY() + i, mc.player.posZ),
		...
		if (sneaking) {
			mc.getNetHandler().getNetworkManager().sendPacket(new C0BPacketEntityAction(mc.player, C0BPacketEntityAction.Action.STOP_SNEAKING));
		}
		...
		if (!attack) {
			if (sneaking) {
				mc.getNetHandler().getNetworkManager().sendPacket(new C0BPacketEntityAction(mc.player, C0BPacketEntityAction.Action.START_SNEAKING));
			}
			positions.clear();
			positionsBack.clear();
			return false;
	...
	public static boolean infiniteReach(double range, double maxXZTP, double maxYTP, 
		...
		if ((rayTraceWide(new Vec3(mc.player.posX, mc.player.posY, mc.player.posZ),
				...
				|| (rayTrace1 = rayTracePos(
						new Vec3(mc.player.posX, mc.player.posY + mc.player.getEyeHeight(), mc.player.posZ),
						new Vec3(posX, posY + mc.player.getEyeHeight(), posZ), false, false,
						true)) != null) {
			if ((rayTrace = rayTracePos(new Vec3(mc.player.posX, mc.player.posY, mc.player.posZ),
					new Vec3(posX, mc.player.posY, posZ), false, false, true)) != null
					|| (rayTrace1 = rayTracePos(
							new Vec3(mc.player.posX, mc.player.posY + mc.player.getEyeHeight(),
									mc.player.posZ),
							new Vec3(posX, mc.player.posY + mc.player.getEyeHeight(), posZ), false, false,
							true)) != null) {
				MovingObjectPosition trace = null;
				if (rayTrace == null) {
					trace = rayTrace1;
	...
	public static boolean infiniteReach(Vec3 src, Vec3 dest, double range, double maxXZTP, double maxYTP, 
		...
		if ((rayTraceWide(new Vec3(mc.player.posX, mc.player.posY, mc.player.posZ),
				...
				|| (rayTrace1 = rayTracePos(
						new Vec3(mc.player.posX, mc.player.posY + mc.player.getEyeHeight(), mc.player.posZ),
						new Vec3(dest.xCoord, dest.yCoord + mc.player.getEyeHeight(), dest.zCoord), false, false,
						true)) != null) {
			if ((rayTrace = rayTracePos(new Vec3(mc.player.posX, mc.player.posY, mc.player.posZ),
					new Vec3(dest.xCoord, mc.player.posY, dest.zCoord), false, false, true)) != null
					|| (rayTrace1 = rayTracePos(
							new Vec3(mc.player.posX, mc.player.posY + mc.player.getEyeHeight(),
									mc.player.posZ),
							new Vec3(dest.xCoord, mc.player.posY + mc.player.getEyeHeight(), dest.zCoord), false, false,
							true)) != null) {
				MovingObjectPosition trace = null;
				if (rayTrace == null) {
					trace = rayTrace1;
	...
	public static void attackInf(EntityLivingBase en) {
		...
		float sharpLevel = EnchantmentHelper.getModifierForCreature(mc.player.getHeldItem(), en.getCreatureAttribute());
		boolean vanillaCrit = (mc.player.fallDistance > 0.0F) && (!mc.player.onGround)
				&& (!mc.player.isOnLadder()) && (!mc.player.isInWater())
				&& (!mc.player.isPotionActive(Potion.blindness)) && (mc.player.ridingEntity == null);
		if (sharpLevel > 0.0F) {
	...
	public static MovingObjectPosition rayTracePos(Vec3 vec31, Vec3 vec32, boolean stopOnLiquid,
		...
		if (false) {
			MovingObjectPosition trace2 = mc.world.rayTraceBlocks(vec31, vec32, stopOnLiquid,
					ignoreBlockWithoutBoundingBox, returnLastUncollidableBlock);
			return trace2;
		}
		// MovingObjectPosition trace4 = mc.world.rayTraceBlocks(leftA,
		// left2A, stopOnLiquid, ignoreBlockWithoutBoundingBox,
		// returnLastUncollidableBlock);
		MovingObjectPosition trace1 = mc.world.rayTraceBlocks(left, left2, stopOnLiquid,
				ignoreBlockWithoutBoundingBox, returnLastUncollidableBlock);
		MovingObjectPosition trace2 = mc.world.rayTraceBlocks(vec31, vec32, stopOnLiquid,
				ignoreBlockWithoutBoundingBox, returnLastUncollidableBlock);
		MovingObjectPosition trace3 = mc.world.rayTraceBlocks(right, right2, stopOnLiquid,
				ignoreBlockWithoutBoundingBox, returnLastUncollidableBlock);
		// MovingObjectPosition trace5 = mc.world.rayTraceBlocks(rightA,
		...
		if (trace2 != null || trace1 != null || trace3 != null || trace4 != null || trace5 != null) {
			if (returnLastUncollidableBlock) {
				if (trace5 != null && (JigsawReach.getBlock(trace5.getBlockPos()).getMaterial() != Material.air
						|| trace5.entityHit != null)) {
					// positions.add(BlockTools.getVec3(trace3.getBlockPos()));
					return trace5;
				}
				if (trace4 != null && (JigsawReach.getBlock(trace4.getBlockPos()).getMaterial() != Material.air
						|| trace4.entityHit != null)) {
					// positions.add(BlockTools.getVec3(trace3.getBlockPos()));
					return trace4;
				}
				if (trace3 != null && (JigsawReach.getBlock(trace3.getBlockPos()).getMaterial() != Material.air
						|| trace3.entityHit != null)) {
					// positions.add(BlockTools.getVec3(trace3.getBlockPos()));
					return trace3;
				}
				if (trace1 != null && (JigsawReach.getBlock(trace1.getBlockPos()).getMaterial() != Material.air
						|| trace1.entityHit != null)) {
					// positions.add(BlockTools.getVec3(trace1.getBlockPos()));
					return trace1;
				}
				if (trace2 != null && (JigsawReach.getBlock(trace2.getBlockPos()).getMaterial() != Material.air
	...
	public static boolean rayTraceWide(Vec3 vec31, Vec3 vec32, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox,
		...
		MovingObjectPosition trace5 = null;
		if (returnLastUncollidableBlock) {
			return (trace1 != null && JigsawReach.getBlock(trace1.getBlockPos()).getMaterial() != Material.air)
					|| (trace2 != null && JigsawReach.getBlock(trace2.getBlockPos()).getMaterial() != Material.air)
					|| (trace3 != null && JigsawReach.getBlock(trace3.getBlockPos()).getMaterial() != Material.air)
					|| (trace4 != null && JigsawReach.getBlock(trace4.getBlockPos()).getMaterial() != Material.air)
					|| (trace5 != null && JigsawReach.getBlock(trace5.getBlockPos()).getMaterial() != Material.air);
		} else {
	...
	public static void blinkToPosFromPos(Vec3 src, Vec3 dest, double maxTP) {
		...
		for (int i = 0; i < steps; i++) {
			...
			mc.player.sendQueue.addToSendQueue(new C04PacketPlayerPosition(x, y, z, true));
		}
		mc.player.sendQueue.addToSendQueue(new C04PacketPlayerPosition(x2, y2, z2, true));
	}
	...
	public static ArrayList<EntityLivingBase> getClosestEntitiesToEntity(float range, Entity ent) {
		...
		for (Object o : Minecraft.getMinecraft().world.loadedEntityList) {
			if (JigsawReach.isNotItem(o) && !ent.isEntityEqual((EntityLivingBase) o)) {
	...
	public static ArrayList<EntityLivingBase> getClosestEntities(float range) {
		ArrayList<EntityLivingBase> entities = new ArrayList<EntityLivingBase>();
		for (Object o : Minecraft.getMinecraft().world.loadedEntityList) {
			if (isNotItem(o) && !(o instanceof ClientPlayerEntity)) {
				EntityLivingBase en = (EntityLivingBase) o;
				if (!validEntity(en)) {
					continue;
				}
    ...
    private static boolean validEntity(Entity entity) {
        ...
        if (entity instanceof EntityPlayer && (Teams.isOnSameTeam(entity) || AntiBot.isEntityBot(entity))) {
            return false;
        }
	...
	public static boolean isBlockPosAir(BlockPos blockPos) {
		return mc.world.getBlockState(blockPos).getBlock().getMaterial() == Material.air;
	}
	...
	public static void updateLastGroundLocation() {
		if(mc.player.onGround) {
			lastLoc = new Vec3(mc.player.posX, mc.player.posY, mc.player.posZ);
		}
	}
