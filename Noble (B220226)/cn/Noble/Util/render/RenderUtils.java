...
public final class RenderUtils {
	...
	public static void drawBlockBox(final BlockPos blockPos, final Color color) {
		...
		if (block != null) {
			...
			final double posX = player.lastTickPosX
					+ (player.posX - player.lastTickPosX) * (double) timer.renderPartialTicks;
			final double posY = player.lastTickPosY
					+ (player.posY - player.lastTickPosY) * (double) timer.renderPartialTicks;
			final double posZ = player.lastTickPosZ
					+ (player.posZ - player.lastTickPosZ) * (double) timer.renderPartialTicks;
		...
		glColor(new Color(color.getRed(), color.getGreen(), color.getBlue(),
				color.getAlpha() == 255 ? 35 : color.getAlpha()));
	...
	public static void drawRect(final double x, final double y, final double d, final double e, final int color) {
		glEnable(GL_BLEND);
		glDisable(GL_TEXTURE_2D);
		glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	...
	public static void drawLoadingCircle(float x, float y, String ip) {
		int rot = (int) ((System.nanoTime() / 5000000 * 2) % 360);
		glColor(Color.GRAY);
		drawCircle(x, y, 15 * 2.6F, rot - 360, rot);
		glColor(Color.WHITE);
		drawCircle(x, y, 15 * 2.6F, rot - 180, rot);
		GlStateManager.resetColor();
		
		FontLoaders.CNMD22.drawCenteredString(ip, x, y + 50, -1);
	}
...
//		glColor(Color.WHITE);
		...
		for (float i = end; i >= start; i -= (360 / 90))
			glVertex2f((float) (x + (cos(i * PI / 180) * (radius * 1.001F))),
					(float) (y + (sin(i * PI / 180) * (radius * 1.001F))));
		glEnd();
		glDisable(GL_LINE_SMOOTH);
	...
	public static void drawImage(ResourceLocation image, double d, double e, int width, int height) {
		glDisable(GL_DEPTH_TEST);
		glEnable(GL_BLEND);
		glDepthMask(false);
		...
		Gui.drawModalRectWithCustomSizedTexture(d, e, 0, 0, width, height, width, height);
		glDepthMask(true);
		glDisable(GL_BLEND);
		glEnable(GL_DEPTH_TEST);
	}
	...
	public static void draw2D(final EntityLivingBase entity, final double posX, final double posY, final double posZ,
			final int color, final int backgroundColor) {
		...
		GlStateManager.translate(0,
				21 + -(entity.getEntityBoundingBox().maxY - entity.getEntityBoundingBox().minY) * 12, 0);
	...
	public static void makeScissorBox(final float x, final float y, final float x2, final float y2) {
		...
		glScissor((int) (x * factor), (int) ((scaledResolution.getScaledHeight() - y2) * factor),
