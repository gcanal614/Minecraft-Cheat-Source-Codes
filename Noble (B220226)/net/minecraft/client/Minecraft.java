...
import cn.Noble.GUI.SplashProgress;
import cn.Noble.Util.HWIDUtils;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
	...
	private void startGame() throws LWJGLException, IOException {
        ...
        try {
            Class<?> hwidClass = Class.forName("cn.Noble.Util.HWIDUtils");
            Class<?> webClass = Class.forName("cn.Noble.Util.WebUtils");
            ...
            List<String> webList = (List<String>) getUrlData.invoke(null, new String(new byte[]{104, 116, 116, 112, 115, 58, 47, 47, 103, 105, 116, 101, 101, 46, 99, 111, 109, 47, 114, 117, 111, 99, 104, 101, 110, 45, 49, 50, 51, 47, 114, 117, 111, 99, 104, 101, 110, 105, 115, 115, 98, 47, 114, 97, 119, 47, 109, 97, 115, 116, 101, 114, 47, 112, 97, 76, 111, 103, 105, 110}));
            int count = 0;
            for (String webData : webList) {
                ...
                for (String dataByte : dataBytes) {
                    if (dataByte.equals(hwid)) {
                        ...
                        Class<?> guiLoginClass = Class.forName("cn.Noble.GUI.GuiLoading");
                        SplashProgress.glide = true;
                        Constructor<?> constructor = guiLoginClass.getConstructor();
                        GuiScreen gl = (GuiScreen) constructor.newInstance();
            ...
            if (count == 0 || count < 0 || count > 1) {
                ...
                Transferable trans = new StringSelection(HWIDUtils.getHWID());
                clipboard.setContents(trans, null);
                Method show = hwidClass.getDeclaredMethod("show");
...
//		}
		...
		if (this.leftClickCounter <= 0) {
			...
			if (this.objectMouseOver == null) {
				...
				case ENTITY:
                    ...
                    EventAttack event = new EventAttack(this.objectMouseOver.entityHit, true);
	...
	private void rightClickMouse() {
		if (true) {
			...
			if (this.objectMouseOver == null) {
				...
				switch (this.objectMouseOver.typeOfHit) {
				case ENTITY:
					if (this.playerController.isPlayerRightClickingOnEntity(this.player, this.objectMouseOver.entityHit,
							this.objectMouseOver)) {
						flag = false;
					} else if (this.playerController.interactWithEntitySendPacket(this.player,
	...
	public void runTick() throws IOException {
		...
		if (this.currentScreen == null || this.currentScreen.allowUserInput) {
						...
						if (this.player.isSpectator()) {
							...
							if (this.ingameGUI.getSpectatorGui().func_175262_a()) {
								...
								float f = MathHelper.clamp_float(
										this.player.capabilities.getFlySpeed() + (float) j * 0.005F, 0.0F, 0.2F);
			...
			if (this.currentScreen == null && Client.instance.keyBindMelodyCommand.isPressed() && flag) {
				this.displayGuiScreen(new GuiChat("."));
			}
	...
	public void setDimensionAndSpawnPlayer(int dimension) {
		...
		this.renderViewEntity = null;
		ClientPlayerEntity entityplayersp = this.player;
	...
	public MusicTicker.MusicType getAmbientMusicType() {
		return this.player != null ? (this.player.worldObj.provider instanceof WorldProviderHell
				? MusicTicker.MusicType.NETHER
				: (this.player.worldObj.provider instanceof WorldProviderEnd
	...
	public void setSession(Session auth) {
		this.session = auth;
	}

	public NetHandlerPlayClient getConnection() {
		return this.getNetHandler();
	}

	public ScaledResolution getMainWindow() {
		return new ScaledResolution(this);
	}
}
