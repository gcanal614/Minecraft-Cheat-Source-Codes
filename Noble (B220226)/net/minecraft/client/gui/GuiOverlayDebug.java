	...
	protected void renderDebugInfoLeft() {
		...
		for (int i = 0; i < list.size(); ++i) {
			...
			if (!Strings.isNullOrEmpty(s)) {
				...
				int l = 2;
				int i1 = 2 + j * i;
				this.fontRenderer.drawString(s, 2, i1 - 2, 14737632);
			}
		}
	}
	...
	protected void renderDebugInfoRight(ScaledResolution scaledRes) {
		...
		for (int i = 0; i < list.size(); ++i) {
			...
			if (!Strings.isNullOrEmpty(s)) {
				...
				int i1 = 2 + j * i;
				this.fontRenderer.drawString(s, l, i1 - 2, 14737632);
			}
		}
	}
	...
	protected List<String> call() {
		...
		if (this.isReducedDebug()) {
			return Lists.newArrayList(new String[] {
					"Minecraft 1.18-pre1 (" + this.mc.getVersion() + "/" + ClientBrandRetriever.getClientModName()
							+ ")",
			...
			List<String> list = Lists
					.newArrayList(new String[] {
							...
							String.format("Chunk: %d %d %d in %d %d %d", new Object[] {
