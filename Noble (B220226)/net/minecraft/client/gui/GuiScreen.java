...
import cn.Noble.GUI.ShaderBG.Shaders.BackgroundShader;
...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		int col = new Color(255, 255, 255, 110).getRGB();
	}
	...
	protected void drawHoveringText(List<String> textLines, int x, int y) {
		if (!textLines.isEmpty()) {
			...
			for (int k1 = 0; k1 < textLines.size(); ++k1) {
				...
				this.fontRendererObj.drawStringWithShadow(s1, (float) l1, (float) i2 - 2.2F, -1);
	...
	//	}
        char jb = Keyboard.getEventCharacter();
        int key = Keyboard.getEventKey();
        if (Keyboard.getEventKeyState() || (key == 0 && Character.isDefined(jb)))
        {
            this.keyTyped(jb, key);
        }
...
//	        	RenderUtil.drawGradientSidewaysV(0, -120, this.width, this.height, new Color(HUD.r.getValue().intValue(), HUD.g.getValue().intValue(), HUD.b.getValue().intValue(),alpha).getRGB(), new Color(HUD.r.getValue().intValue(), HUD.g.getValue().intValue(), HUD.b.getValue().intValue(),0).getRGB());
	            this.drawGradientRect(0, 0, this.width, this.height, new Color(HUD.wr.getValue().intValue(), HUD.wg.getValue().intValue(), HUD.wb.getValue().intValue(),0).getRGB(), new Color(HUD.wr.getValue().intValue(), HUD.wg.getValue().intValue(), HUD.wb.getValue().intValue(),alpha).getRGB());
