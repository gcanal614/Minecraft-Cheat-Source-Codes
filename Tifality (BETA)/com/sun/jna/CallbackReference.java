...
extends WeakReference {
    ...
    private static Callback getCallback(Class type2, Pointer p, boolean direct) {
        ...
        synchronized (map3) {
            ...
            Callback cb = (Callback)Proxy.newProxyInstance(type2.getClassLoader(), new Class[]{type2}, h);
            map2.put(cb, null);
            return cb;
        }
    }
    ...
    private static Method getCallbackMethod(Class cls) {
        ...
        Method[] methods2 = pmethods.toArray(new Method[pmethods.size()]);
        if (methods2.length == 1) {
            return CallbackReference.checkMethod(methods2[0]);
        }
    ...
    implements CallbackProxy {
        ...
        private Object invokeCallback(Object[] args2) {
            ...
            if (cb != null) {
                try {
                    result2 = this.convertResult(this.callbackMethod.invoke(cb, callbackArgs));
                }
                catch (IllegalArgumentException e) {
