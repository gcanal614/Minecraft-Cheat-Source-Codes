...
public abstract class Secur32Util {
    public static String getUserNameEx(int format) {
        ...
        boolean result2 = Secur32.INSTANCE.GetUserNameEx(format, buffer, len);
        if (!result2) {
            ...
            result2 = Secur32.INSTANCE.GetUserNameEx(format, buffer, len);
        }
        if (!result2) {
            throw new Win32Exception(Native.getLastError());
        }
    ...
    public static SecurityPackage[] getSecurityPackages() {
        ...
        ArrayList<SecurityPackage> packages2 = new ArrayList<SecurityPackage>(pcPackages.getValue());
        for (Sspi.SecPkgInfo.ByReference packageInfo : packagesInfo) {
            ...
            packages2.add(securityPackage);
        }
        rc = Secur32.INSTANCE.FreeContextBuffer(pPackageInfo.pPkgInfo.getPointer());
        if (0 != rc) {
            throw new Win32Exception(rc);
        }
        return packages2.toArray(new SecurityPackage[0]);
    }
