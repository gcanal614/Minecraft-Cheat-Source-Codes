...
final class Types {
    ...
    static Type newArrayType(Type componentType) {
        if (componentType instanceof WildcardType) {
            ...
            Type[] upperBounds2 = wildcard.getUpperBounds();
            Preconditions.checkArgument(upperBounds2.length == 1, "Wildcard should have only one upper bound.");
            return Types.subtypeOf(Types.newArrayType(upperBounds2[0]));
        }
        return JavaVersion.CURRENT.newArrayType(componentType);
    }

    static ParameterizedType newParameterizedTypeWithOwner(@Nullable Type ownerType, Class<?> rawType, Type ... arguments2) {
        if (ownerType == null) {
            return Types.newParameterizedType(rawType, arguments2);
        }
        Preconditions.checkNotNull(arguments2);
        Preconditions.checkArgument(rawType.getEnclosingClass() != null, "Owner type for unenclosed %s", rawType);
        return new ParameterizedTypeImpl(ownerType, rawType, arguments2);
    }

    static ParameterizedType newParameterizedType(Class<?> rawType, Type ... arguments2) {
        return new ParameterizedTypeImpl(ClassOwnership.JVM_BEHAVIOR.getOwnerType(rawType), rawType, arguments2);
    }
    ...
    Serializable {
        ...
        WildcardTypeImpl(Type[] lowerBounds, Type[] upperBounds2) {
