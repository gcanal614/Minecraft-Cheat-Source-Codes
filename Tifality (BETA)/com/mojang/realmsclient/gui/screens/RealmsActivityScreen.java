    ...
    extends RealmsScrolledSelectionList {
        ...
        protected void renderItem(int i, int x, int y, int h, Tezzelator t, int mouseX, int mouseY) {
            if (RealmsActivityScreen.this.activityMap != null && RealmsActivityScreen.this.activityMap.size() > i) {
                ...
                GL11.glDisable(3553);
                t.begin(7, RealmsDefaultVertexFormat.POSITION_COLOR);
                t.vertex(RealmsActivityScreen.this.activityPoint - 8, (double)y + 6.5, 0.0).color(r, g, b2, 255).endVertex();
                t.vertex(RealmsActivityScreen.this.activityPoint - 3, (double)y + 6.5, 0.0).color(r, g, b2, 255).endVertex();
                t.vertex(RealmsActivityScreen.this.activityPoint - 3, (double)y + 1.5, 0.0).color(r, g, b2, 255).endVertex();
                t.vertex(RealmsActivityScreen.this.activityPoint - 8, (double)y + 1.5, 0.0).color(r, g, b2, 255).endVertex();
                t.end();
                GL11.glEnable(3553);
                ...
                for (Activity activity : row.activities) {
                    ...
                    for (ActivityRender render2 : toRender) {
                        if (!(render2.start + render2.width >= pos - 0.5)) continue;
                        double overlap = render2.start + render2.width - pos;
                        double padding = Math.max(0.0, pos - (render2.start + render2.width));
                        render2.width = render2.width - Math.max(0.0, overlap) + itemWidth + padding;
                        render2.tooltip = render2.tooltip + "\n" + tooltip;
                        exists = true;
                        break;
                    }
                    if (exists) continue;
                ...
                for (ActivityRender render3 : toRender) {
                    GL11.glDisable(3553);
                    t.begin(7, RealmsDefaultVertexFormat.POSITION_COLOR);
                    t.vertex(render3.start, (double)y + 6.5, 0.0).color(r, g, b2, 255).endVertex();
                    t.vertex(render3.start + render3.width, (double)y + 6.5, 0.0).color(r, g, b2, 255).endVertex();
                    t.vertex(render3.start + render3.width, (double)y + 1.5, 0.0).color(r, g, b2, 255).endVertex();
                    t.vertex(render3.start, (double)y + 1.5, 0.0).color(r, g, b2, 255).endVertex();
                    t.end();
                    GL11.glEnable(3553);
                    if (!((double)this.xm() >= render3.start) || !((double)this.xm() <= render3.start + render3.width) || !((double)this.ym() >= (double)y + 1.5) || !((double)this.ym() <= (double)y + 6.5)) continue;
                    RealmsActivityScreen.this.toolTip = render3.tooltip.trim();
                }
            }
        }
