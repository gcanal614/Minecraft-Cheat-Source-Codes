...
extends NumberFormat {
    ...
    public StringBuffer format(double number, StringBuffer result2, FieldPosition fieldPosition) {
        return this.format(number, result2, fieldPosition, false);
    }
     ...
     */
    private StringBuffer format(double number, StringBuffer result2, FieldPosition fieldPosition, boolean parseAttr) {
        fieldPosition.setBeginIndex(0);
        fieldPosition.setEndIndex(0);
        if (Double.isNaN(number)) {
            if (fieldPosition.getField() == 0) {
                fieldPosition.setBeginIndex(result2.length());
            } else if (fieldPosition.getFieldAttribute() == NumberFormat.Field.INTEGER) {
                fieldPosition.setBeginIndex(result2.length());
            }
            result2.append(this.symbols.getNaN());
            if (parseAttr) {
                this.addAttribute(NumberFormat.Field.INTEGER, result2.length() - this.symbols.getNaN().length(), result2.length());
            }
            if (fieldPosition.getField() == 0) {
                fieldPosition.setEndIndex(result2.length());
            } else if (fieldPosition.getFieldAttribute() == NumberFormat.Field.INTEGER) {
                fieldPosition.setEndIndex(result2.length());
            }
            this.addPadding(result2, fieldPosition, 0, 0);
            return result2;
        }
        number = this.multiply(number);
        boolean isNegative = this.isNegative(number);
        if (Double.isInfinite(number = this.round(number))) {
            int prefixLen = this.appendAffix(result2, isNegative, true, parseAttr);
            if (fieldPosition.getField() == 0) {
                fieldPosition.setBeginIndex(result2.length());
            } else if (fieldPosition.getFieldAttribute() == NumberFormat.Field.INTEGER) {
                fieldPosition.setBeginIndex(result2.length());
            }
            result2.append(this.symbols.getInfinity());
            if (parseAttr) {
                this.addAttribute(NumberFormat.Field.INTEGER, result2.length() - this.symbols.getInfinity().length(), result2.length());
            }
            if (fieldPosition.getField() == 0) {
                fieldPosition.setEndIndex(result2.length());
            } else if (fieldPosition.getFieldAttribute() == NumberFormat.Field.INTEGER) {
            ...
            int suffixLen = this.appendAffix(result2, isNegative, false, parseAttr);
            this.addPadding(result2, fieldPosition, prefixLen, suffixLen);
            return result2;
        }
        ...
        synchronized (digitList) {
            ...
            return this.subformat(number, result2, fieldPosition, isNegative, false, parseAttr);
        }
    }
    ...
    public StringBuffer format(long number, StringBuffer result2, FieldPosition fieldPosition) {
        return this.format(number, result2, fieldPosition, false);
    }
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    private StringBuffer format(long number, StringBuffer result2, FieldPosition fieldPosition, boolean parseAttr) {
        boolean isNegative;
        fieldPosition.setBeginIndex(0);
        fieldPosition.setEndIndex(0);
        if (this.roundingIncrementICU != null) {
            return this.format(com.ibm.icu.math.BigDecimal.valueOf(number), result2, fieldPosition);
        }
        ...
        if (this.multiplier != 1) {
            ...
            if (tooBig) {
                return this.format(BigInteger.valueOf(isNegative ? -number : number), result2, fieldPosition, parseAttr);
            }
        }
        ...
        synchronized (digitList) {
            ...
            return this.subformat(number, result2, fieldPosition, isNegative, true, parseAttr);
        }
    }

    public StringBuffer format(BigInteger number, StringBuffer result2, FieldPosition fieldPosition) {
        return this.format(number, result2, fieldPosition, false);
    }
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    private StringBuffer format(BigInteger number, StringBuffer result2, FieldPosition fieldPosition, boolean parseAttr) {
        if (this.roundingIncrementICU != null) {
            return this.format(new com.ibm.icu.math.BigDecimal(number), result2, fieldPosition);
        }
        if (this.multiplier != 1) {
        ...
        synchronized (digitList) {
            this.digitList.set(number, this.precision(true));
            return this.subformat(number.intValue(), result2, fieldPosition, number.signum() < 0, true, parseAttr);
        }
    }

    public StringBuffer format(BigDecimal number, StringBuffer result2, FieldPosition fieldPosition) {
        return this.format(number, result2, fieldPosition, false);
    }
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    private StringBuffer format(BigDecimal number, StringBuffer result2, FieldPosition fieldPosition, boolean parseAttr) {
        if (this.multiplier != 1) {
        ...
        synchronized (digitList) {
            this.digitList.set(number, this.precision(false), !this.useExponentialNotation && !this.areSignificantDigitsUsed());
            return this.subformat(number.doubleValue(), result2, fieldPosition, number.signum() < 0, false, parseAttr);
        }
    }
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    public StringBuffer format(com.ibm.icu.math.BigDecimal number, StringBuffer result2, FieldPosition fieldPosition) {
        if (this.multiplier != 1) {
        ...
        synchronized (digitList) {
            this.digitList.set(number, this.precision(false), !this.useExponentialNotation && !this.areSignificantDigitsUsed());
            return this.subformat(number.doubleValue(), result2, fieldPosition, number.signum() < 0, false, false);
        }
    }
    ...
    private boolean isGroupingPosition(int pos) {
        ...
        if (this.isGroupingUsed() && pos > 0 && this.groupingSize > 0) {
            result2 = this.groupingSize2 > 0 && pos > this.groupingSize ? (pos - this.groupingSize) % this.groupingSize2 == 0 : pos % this.groupingSize == 0;
        }
        return result2;
    }
    ...
    private StringBuffer subformat(int number, StringBuffer result2, FieldPosition fieldPosition, boolean isNegative, boolean isInteger, boolean parseAttr) {
        if (this.currencySignCount == 3) {
            return this.subformat(this.currencyPluralInfo.select(number), result2, fieldPosition, isNegative, isInteger, parseAttr);
        }
        return this.subformat(result2, fieldPosition, isNegative, isInteger, parseAttr);
    }

    private StringBuffer subformat(double number, StringBuffer result2, FieldPosition fieldPosition, boolean isNegative, boolean isInteger, boolean parseAttr) {
        if (this.currencySignCount == 3) {
            return this.subformat(this.currencyPluralInfo.select(number), result2, fieldPosition, isNegative, isInteger, parseAttr);
        }
        return this.subformat(result2, fieldPosition, isNegative, isInteger, parseAttr);
    ...
    private StringBuffer subformat(String pluralCount, StringBuffer result2, FieldPosition fieldPosition, boolean isNegative, boolean isInteger, boolean parseAttr) {
    ...
    private StringBuffer subformat(StringBuffer result2, FieldPosition fieldPosition, boolean isNegative, boolean isInteger, boolean parseAttr) {
        if (this.digitList.isZero()) {
            this.digitList.decimalAt = 0;
        }
        int prefixLen = this.appendAffix(result2, isNegative, true, parseAttr);
        if (this.useExponentialNotation) {
            this.subformatExponential(result2, fieldPosition, parseAttr);
        } else {
            this.subformatFixed(result2, fieldPosition, isInteger, parseAttr);
        }
        int suffixLen = this.appendAffix(result2, isNegative, false, parseAttr);
        this.addPadding(result2, fieldPosition, prefixLen, suffixLen);
        return result2;
    ...
    private void subformatFixed(StringBuffer result2, FieldPosition fieldPosition, boolean isInteger, boolean parseAttr) {
        ...
        int sizeBeforeIntegerPart = result2.length();
        for (i = count - 1; i >= 0; --i) {
            if (i < this.digitList.decimalAt && digitIndex < this.digitList.count && sigCount < maxSigDig) {
                result2.append(digits[this.digitList.getDigitValue(digitIndex++)]);
                ++sigCount;
            } else {
                result2.append(digits[0]);
                if (sigCount > 0) {
                    ++sigCount;
                }
            }
            if (!this.isGroupingPosition(i)) continue;
            result2.append(grouping);
            if (!parseAttr) continue;
            this.addAttribute(NumberFormat.Field.GROUPING_SEPARATOR, result2.length() - 1, result2.length());
        }
        if (fieldPosition.getField() == 0) {
            fieldPosition.setEndIndex(result2.length());
        } else if (fieldPosition.getFieldAttribute() == NumberFormat.Field.INTEGER) {
        ...
        if (!fractionPresent && result2.length() == sizeBeforeIntegerPart) {
            result2.append(digits[0]);
        }
        if (parseAttr) {
            this.addAttribute(NumberFormat.Field.INTEGER, intBegin, result2.length());
        }
        if (this.decimalSeparatorAlwaysShown || fractionPresent) {
            ...
            result2.append(decimal);
            if (fieldPosition.getFieldAttribute() == NumberFormat.Field.DECIMAL_SEPARATOR) {
                fieldPosition.setEndIndex(result2.length());
            }
            if (parseAttr) {
                this.addAttribute(NumberFormat.Field.DECIMAL_SEPARATOR, result2.length() - 1, result2.length());
            }
        }
        ...
        if (parseAttr && (this.decimalSeparatorAlwaysShown || fractionPresent)) {
            this.addAttribute(NumberFormat.Field.FRACTION, fracBegin, result2.length());
        }
    }

    private void subformatExponential(StringBuffer result2, FieldPosition fieldPosition, boolean parseAttr) {
        ...
        for (i = 0; i < totalDigits; ++i) {
            if (i == integerDigits) {
                ...
                if (parseAttr) {
                    intEnd = result2.length();
                    this.addAttribute(NumberFormat.Field.INTEGER, intBegin, result2.length());
                }
                result2.append(decimal);
                if (parseAttr) {
                    int decimalSeparatorBegin = result2.length() - 1;
                    this.addAttribute(NumberFormat.Field.DECIMAL_SEPARATOR, decimalSeparatorBegin, result2.length());
                    fracBegin = result2.length();
                }
                if (fieldPosition.getField() == 1) {
                    fieldPosition.setBeginIndex(result2.length());
                } else if (fieldPosition.getFieldAttribute() == NumberFormat.Field.FRACTION) {
            ...
            result2.append(i < this.digitList.count ? digits[this.digitList.getDigitValue(i)] : digits[0]);
        }
        ...
        result2.append(this.symbols.getExponentSeparator());
        if (parseAttr) {
            this.addAttribute(NumberFormat.Field.EXPONENT_SYMBOL, result2.length() - this.symbols.getExponentSeparator().length(), result2.length());
        }
        if (this.digitList.isZero()) {
            exponent = 0;
        }
        ...
        if (negativeExponent) {
            exponent = -exponent;
            result2.append(this.symbols.getMinusSign());
            if (parseAttr) {
                this.addAttribute(NumberFormat.Field.EXPONENT_SIGN, result2.length() - 1, result2.length());
            }
        } else if (this.exponentSignAlwaysShown) {
            result2.append(this.symbols.getPlusSign());
            if (parseAttr) {
                int expSignBegin = result2.length() - 1;
                this.addAttribute(NumberFormat.Field.EXPONENT_SIGN, expSignBegin, result2.length());
            }
        }
        int expBegin = result2.length();
        ...
        if (parseAttr) {
            this.addAttribute(NumberFormat.Field.EXPONENT, expBegin, result2.length());
        }
    }

    private final void addPadding(StringBuffer result2, FieldPosition fieldPosition, int prefixLen, int suffixLen) {
        int len;
        if (this.formatWidth > 0 && (len = this.formatWidth - result2.length()) > 0) {
            ...
            switch (this.padPosition) {
                case 1: {
                    result2.insert(prefixLen, padding);
                    break;
                }
                case 0: {
                    result2.insert(0, padding);
                    break;
                }
                case 2: {
                    result2.insert(result2.length() - suffixLen, padding);
                    break;
                }
                case 3: {
                    result2.append(padding);
                }
            }
    ...
    private boolean parseForCurrency(String text, ParsePosition parsePosition, Currency[] currency, boolean[] status) {
        ...
        while (i$.hasNext()) {
            ...
            boolean result2 = this.subparse(text, tmpPos, tmpDigitList, tmpStatus, currency, affix.getNegPrefix(), affix.getNegSuffix(), affix.getPosPrefix(), affix.getPosSuffix(), affix.getPatternType());
            if (result2) {
                found = true;
        ...
        boolean result3 = this.subparse(text, tmpPos, tmpDigitList, tmpStatus, currency, this.negativePrefix, this.negativeSuffix, this.positivePrefix, this.positiveSuffix, 0);
...
lbl39:
    ...
    private String toPattern(boolean localized) {
        ...
        for (int part = 0; part < 2; ++part) {
            ...
            int g;
            if (padPos == 0) {
                result2.append(padSpec);
            }
            this.appendAffixPattern(result2, part != 0, true, localized);
            if (padPos == 1) {
                result2.append(padSpec);
            }
            int sub0Start = result2.length();
            ...
            for (i = maxDig; i > 0; --i) {
                ...
                if (useSigDig) {
                    result2.append(maxSigDig >= i && i > maxSigDig - minDig ? sigDigit : digit);
                    continue;
                }
                if (roundingDigits != null && (pos = roundingDecimalPos - i) >= 0 && pos < roundingDigits.length()) {
                    result2.append((char)(roundingDigits.charAt(pos) - 48 + zero));
                    continue;
                }
                result2.append(i <= minDig ? zero : digit);
            }
            if (!useSigDig) {
                if (this.getMaximumFractionDigits() > 0 || this.decimalSeparatorAlwaysShown) {
                    result2.append(localized ? this.symbols.getDecimalSeparator() : (char)'.');
                }
                pos = roundingDecimalPos;
                for (i = 0; i < this.getMaximumFractionDigits(); ++i) {
                    if (roundingDigits != null && pos < roundingDigits.length()) {
                        result2.append(pos < 0 ? zero : (char)(roundingDigits.charAt(pos) - 48 + zero));
                        ++pos;
                        continue;
                    }
                    result2.append(i < this.getMinimumFractionDigits() ? zero : digit);
                }
            }
            if (this.useExponentialNotation) {
                if (localized) {
                ...
                if (this.exponentSignAlwaysShown) {
                    result2.append(localized ? this.symbols.getPlusSign() : (char)'+');
                }
            ...
            if (padSpec != null && !this.useExponentialNotation) {
                int add = this.formatWidth - result2.length() + sub0Start - (part == 0 ? this.positivePrefix.length() + this.positiveSuffix.length() : this.negativePrefix.length() + this.negativeSuffix.length());
                while (add > 0) {
                    result2.insert(sub0Start, digit);
                    if (--add <= 1 || !this.isGroupingPosition(++maxDig)) continue;
                    result2.insert(sub0Start, group);
                    --add;
                }
            }
            if (padPos == 2) {
            ...
            this.appendAffixPattern(result2, part != 0, false, localized);
            if (padPos == 3) {
                result2.append(padSpec);
            }
            if (part != 0) continue;
            if (this.negativeSuffix.equals(this.positiveSuffix) && this.negativePrefix.equals('-' + this.positivePrefix)) break;
            result2.append(localized ? this.symbols.getPatternSeparator() : (char)';');
        }
