...
public class SpoofChecker {
    ...
    void wholeScriptCheck(CharSequence text, ScriptSet result2) {
        int inputIdx = 0;
        ...
        while (inputIdx < text.length()) {
            ...
            if (index == 0) {
                ...
                assert (cpScript > 1);
                result2.intersect(cpScript);
                continue;
            }
            if (index == 1) continue;
            result2.intersect(this.fSpoofData.fScriptSets[index]);
        }
    }
    ...
    public static class Builder {
        ...
        public SpoofChecker build() {
            ...
            SpoofChecker result2 = new SpoofChecker();
            result2.fMagic = this.fMagic;
            result2.fChecks = this.fChecks;
            result2.fSpoofData = this.fSpoofData;
            result2.fAllowedCharsSet = (UnicodeSet)this.fAllowedCharsSet.clone();
            result2.fAllowedCharsSet.freeze();
            result2.fAllowedLocales = this.fAllowedLocales;
            result2.fRestrictionLevel = this.fRestrictionLevel;
            return result2;
        }
        ...
        public Builder setChecks(int checks2) {
            if (0 != (checks2 & 0)) {
                throw new IllegalArgumentException("Bad Spoof Checks value.");
            }
            this.fChecks = checks2 & 0xFFFFFFFF;
            return this;
        }
