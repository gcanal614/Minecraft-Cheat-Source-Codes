...
extends DateFormat {
    ...
    public StringBuffer format(Calendar cal, StringBuffer toAppendTo, FieldPosition pos) {
        ...
        StringBuffer result2 = this.format(cal, this.capitalizationSetting, toAppendTo, pos, null);
        if (backupTZ != null) {
    ...
    protected void subFormat(StringBuffer buf, char ch, int count, int beginOffset, int fieldNum, DisplayContext capitalizationContext, FieldPosition pos, Calendar cal) {
        ...
        switch (patternCharIndex) {
            ...
            case 17: {
                if (count < 4) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.SPECIFIC_SHORT, tz, date);
                    capContextUsageType = DateFormatSymbols.CapitalizationContextUsage.METAZONE_SHORT;
                } else {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.SPECIFIC_LONG, tz, date);
            ...
            case 23: {
                result2 = count < 4 ? this.tzFormat().format(TimeZoneFormat.Style.ISO_BASIC_LOCAL_FULL, tz, date) : (count == 5 ? this.tzFormat().format(TimeZoneFormat.Style.ISO_EXTENDED_FULL, tz, date) : this.tzFormat().format(TimeZoneFormat.Style.LOCALIZED_GMT, tz, date));
                buf.append(result2);
                break;
            }
            case 24: {
                if (count == 1) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.GENERIC_SHORT, tz, date);
                    capContextUsageType = DateFormatSymbols.CapitalizationContextUsage.METAZONE_SHORT;
                } else if (count == 4) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.GENERIC_LONG, tz, date);
                    capContextUsageType = DateFormatSymbols.CapitalizationContextUsage.METAZONE_LONG;
                }
                buf.append(result2);
                break;
            ...
            case 29: {
                if (count == 1) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.ZONE_ID_SHORT, tz, date);
                } else if (count == 2) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.ZONE_ID, tz, date);
                } else if (count == 3) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.EXEMPLAR_LOCATION, tz, date);
                } else if (count == 4) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.GENERIC_LOCATION, tz, date);
            ...
            case 31: {
                if (count == 1) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.LOCALIZED_GMT_SHORT, tz, date);
                } else if (count == 4) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.LOCALIZED_GMT, tz, date);
                }
                buf.append(result2);
                break;
            }
            case 32: {
                if (count == 1) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.ISO_BASIC_SHORT, tz, date);
                } else if (count == 2) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.ISO_BASIC_FIXED, tz, date);
                } else if (count == 3) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.ISO_EXTENDED_FIXED, tz, date);
                } else if (count == 4) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.ISO_BASIC_FULL, tz, date);
                } else if (count == 5) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.ISO_EXTENDED_FULL, tz, date);
                }
                buf.append(result2);
                break;
            }
            case 33: {
                if (count == 1) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.ISO_BASIC_LOCAL_SHORT, tz, date);
                } else if (count == 2) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.ISO_BASIC_LOCAL_FIXED, tz, date);
                } else if (count == 3) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.ISO_EXTENDED_LOCAL_FIXED, tz, date);
                } else if (count == 4) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.ISO_BASIC_LOCAL_FULL, tz, date);
                } else if (count == 5) {
                    result2 = this.tzFormat().format(TimeZoneFormat.Style.ISO_EXTENDED_LOCAL_FULL, tz, date);
                }
                buf.append(result2);
                break;
            }
    ...
    public void parse(String text, Calendar cal, ParsePosition parsePos) {
        ...
        block40: {
            ...
            try {
                ...
                if (ambiguousYear[0] && (parsedDate = (copy2 = (Calendar)cal.clone()).getTime()).before(this.getDefaultCenturyStart())) {
    ...
    protected int matchString(String text, int start, int field, String[] data2, Calendar cal) {
        return this.matchString(text, start, field, data2, null, cal);
    }

    protected int matchString(String text, int start, int field, String[] data2, String monthPattern, Calendar cal) {
        int i = 0;
        ...
        while (i < count) {
            ...
            if (length > bestMatchLength && (matchLength = this.regionMatchesWithOptionalDot(text, start, data2[i], length)) >= 0) {
                bestMatch = i;
                bestMatchLength = matchLength;
                isLeapMonth = 0;
            }
            if (monthPattern != null && (length = (leapMonthName = MessageFormat.format(monthPattern, data2[i])).length()) > bestMatchLength && (matchLength = this.regionMatchesWithOptionalDot(text, start, leapMonthName, length)) >= 0) {
                bestMatch = i;
                bestMatchLength = matchLength;
                isLeapMonth = 1;
            }
    ...
    private int regionMatchesWithOptionalDot(String text, int start, String data2, int length) {
        boolean matches = text.regionMatches(true, start, data2, 0, length);
        if (matches) {
            return length;
        }
        if (data2.length() > 0 && data2.charAt(data2.length() - 1) == '.' && text.regionMatches(true, start, data2, 0, length - 1)) {
            return length - 1;
        }
        return -1;
    }

    protected int matchQuarterString(String text, int start, int field, String[] data2, Calendar cal) {
        int count = data2.length;
        int bestMatchLength = 0;
        int bestMatch = -1;
        int matchLength = 0;
        for (int i = 0; i < count; ++i) {
            int length = data2[i].length();
            if (length <= bestMatchLength || (matchLength = this.regionMatchesWithOptionalDot(text, start, data2[i], length)) < 0) continue;
            bestMatch = i;
            bestMatchLength = matchLength;
        }
        if (bestMatch >= 0) {
    ...
    protected int subParse(String text, int start, char ch, int count, boolean obeyCount, boolean allowNegative, boolean[] ambiguousYear, Calendar cal, MessageFormat numericLeapMonthFormatter) {
        ...
        if (patternCharIndex == 4 || patternCharIndex == 15 || patternCharIndex == 2 && count <= 2 || patternCharIndex == 26 && count <= 2 || patternCharIndex == 1 || patternCharIndex == 18 || patternCharIndex == 30 || patternCharIndex == 0 && cal.getType().equals("chinese") || patternCharIndex == 8) {
            ...
            if (numericLeapMonthFormatter != null && (patternCharIndex == 2 || patternCharIndex == 26)) {
                Object[] args2 = numericLeapMonthFormatter.parse(text, pos);
                if (args2 != null && pos.getIndex() > start && args2[0] instanceof Number) {
                    parsedNumericLeapMonth = true;
                    number = (Number)args2[0];
                    cal.set(22, 1);
                } else {
                    pos.setIndex(start);
                    cal.set(22, 0);
