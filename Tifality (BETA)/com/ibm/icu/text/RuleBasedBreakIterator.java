...
extends BreakIterator {
    ...
    public int previous() {
        ...
        int result2 = this.rulesPrevious();
        if (result2 == -1) {
            return result2;
        }
        ...
        while (result2 < offset && (nextResult = this.handleNext()) < offset) {
            result2 = nextResult;
        }
        if (this.fCachedBreakPositions != null) {
            this.fPositionInCache = 0;
    ...
    private int rulesPrevious() {
        ...
        int result2 = lastResult;
        int lastTag = 0;
        boolean breakTagValid = false;
        while ((result2 = this.handleNext()) != -1 && result2 < start) {
            lastResult = result2;
    ...
    private int handleNext() {
        if (this.fCachedBreakPositions == null || this.fPositionInCache == this.fCachedBreakPositions.length - 1) {
            ...
            if (this.fDictionaryCharCount > 1 && result2 - startPos > 1) {
                ...
                e.findBreaks(this.fText, startPos, result2, false, this.getBreakType(), breaks);
                ...
                this.fCachedBreakPositions[breaksSize + 1] = result2;
                this.fPositionInCache = 0;
            } else {
                this.fCachedBreakPositions = null;
                return result2;
    ...
    private int handleNext(short[] stateTable) {
        ...
        int result2 = initialPosition = text.getIndex();
        int state = 1;
        ...
        while (state != 0) {
            if (c == Integer.MAX_VALUE) {
                if (mode == 2) {
                    if (lookaheadResult <= result2) break;
