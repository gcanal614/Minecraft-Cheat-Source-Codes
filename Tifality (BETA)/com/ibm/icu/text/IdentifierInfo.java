...
public class IdentifierInfo {
    ...
    public Set<BitSet> getAlternates() {
        HashSet<BitSet> result2 = new HashSet<BitSet>();
        for (BitSet item : this.scriptSetSet) {
            result2.add((BitSet)item.clone());
        }
        return result2;
    }
    ...
    public static String displayAlternates(Set<BitSet> alternates) {
        ...
        TreeSet<BitSet> sorted2 = new TreeSet<BitSet>(BITSET_COMPARATOR);
        sorted2.addAll(alternates);
        for (BitSet item : sorted2) {
            if (result2.length() != 0) {
                result2.append("; ");
            }
            result2.append(IdentifierInfo.displayScripts(item));
        }
    ...
    public static String displayScripts(BitSet scripts) {
        ...
        while (i >= 0) {
            ...
            result2.append(UScript.getShortName(i));
    ...
    public static BitSet parseScripts(String scriptsString) {
        ...
        for (String item : scriptsString.trim().split(",?\\s+")) {
            ...
            result2.set(UScript.getCodeFromName(item));
        }
        return result2;
    }
    ...
    public static Set<BitSet> parseAlternates(String scriptsSetString) {
        ...
        for (String item : scriptsSetString.trim().split("\\s*;\\s*")) {
            if (item.length() == 0) continue;
            result2.add(IdentifierInfo.parseScripts(item));
        }
        return result2;
    }
    ...
    public static final BitSet set(BitSet bitset, int ... values2) {
