...
extends UFormat {
    ...
    public Set<String> getArgumentNames() {
        ...
        int partIndex = 0;
        while ((partIndex = this.nextTopLevelArgStart(partIndex)) >= 0) {
            result2.add(this.getArgName(partIndex + 1));
        }
        return result2;
    }
    ...
    public final StringBuffer format(Object[] arguments2, StringBuffer result2, FieldPosition pos) {
        this.format(arguments2, null, new AppendableWrapper(result2), pos);
        return result2;
    }

    public final StringBuffer format(Map<String, Object> arguments2, StringBuffer result2, FieldPosition pos) {
        this.format(null, arguments2, new AppendableWrapper(result2), pos);
        return result2;
    }

    public static String format(String pattern, Object ... arguments2) {
        MessageFormat temp = new MessageFormat(pattern);
        return temp.format(arguments2);
    }

    public static String format(String pattern, Map<String, Object> arguments2) {
        MessageFormat temp = new MessageFormat(pattern);
        return temp.format(arguments2);
    }
    ...
    @Override
    public final StringBuffer format(Object arguments2, StringBuffer result2, FieldPosition pos) {
        this.format(arguments2, new AppendableWrapper(result2), pos);
        return result2;
    }

    @Override
    public AttributedCharacterIterator formatToCharacterIterator(Object arguments2) {
        if (arguments2 == null) {
            throw new NullPointerException("formatToCharacterIterator must be passed non-null object");
        }
        StringBuilder result2 = new StringBuilder();
        AppendableWrapper wrapper = new AppendableWrapper(result2);
        wrapper.useAttributes();
        this.format(arguments2, wrapper, null);
        AttributedString as = new AttributedString(result2.toString());
    ...
    public Map<String, Object> parseToMap(String source, ParsePosition pos) {
        ...
        int backupStartPos = pos.getIndex();
        this.parse(0, source, pos, null, result2);
        if (pos.getIndex() == backupStartPos) {
            return null;
        }
        return result2;
    ...
    public Object[] parse(String source) throws ParseException {
        ...
        Object[] result2 = this.parse(source, pos);
        if (pos.getIndex() == 0) {
    ...
    private void parse(int msgStart, String source, ParsePosition pos, Object[] args2, Map<String, Object> argsMap) {
        if (source == null) {
            return;
        }
    ...
    private void format(int msgStart, double pluralNumber, Object[] args2, Map<String, Object> argsMap, AppendableWrapper dest, FieldPosition fp) {
        String msgString = this.msgPattern.getPatternString();
        int prevIndex = this.msgPattern.getPart(msgStart).getLimit();
        int i = msgStart + 1;
        while (true) {
            ...
            if (type2 == MessagePattern.Part.Type.REPLACE_NUMBER) {
                ...
                if (args2 != null) {
                    ...
                    if (0 <= argNumber && argNumber < args2.length) {
                        arg = args2[argNumber];
                    } else {
                        arg = null;
                        noArg = "{" + argNumber + "}";
                ...
                if (noArg != null) {
                    ...
                    if (formatter instanceof ChoiceFormat || formatter instanceof PluralFormat || formatter instanceof SelectFormat) {
                        ...
                        if (subMsgString.indexOf(123) >= 0 || subMsgString.indexOf(39) >= 0 && !this.msgPattern.jdkAposMode()) {
                            ...
                            subMsgFormat.format(0, 0.0, args2, argsMap, dest, null);
                    ...
                    this.formatComplexSubMessage(subMsgStart, 0.0, args2, argsMap, dest);
                    ...
                    this.formatComplexSubMessage(subMsgStart, number - offset, args2, argsMap, dest);
    ...
    private void formatComplexSubMessage(int msgStart, double pluralNumber, Object[] args2, Map<String, Object> argsMap, AppendableWrapper dest) {
        String subMsgString;
        if (!this.msgPattern.jdkAposMode()) {
            this.format(msgStart, pluralNumber, args2, argsMap, dest, null);
            return;
        }
        String msgString = this.msgPattern.getPatternString();
    ...
    private void format(Object arguments2, AppendableWrapper result2, FieldPosition fp) {
        if (arguments2 == null || arguments2 instanceof Map) {
            this.format(null, (Map)arguments2, result2, fp);
        } else {
            this.format((Object[])arguments2, null, result2, fp);
        }
    }

    private void format(Object[] arguments2, Map<String, Object> argsMap, AppendableWrapper dest, FieldPosition fp) {
        if (arguments2 != null && this.msgPattern.hasNamedArguments()) {
            throw new IllegalArgumentException("This method is not available in MessageFormat objects that use alphanumeric argument names.");
        }
        this.format(0, 0.0, arguments2, argsMap, dest, fp);
    }
    ...
    private static final class AppendableWrapper {
        ...
        public void append(CharacterIterator iterator2) {
            this.length += AppendableWrapper.append(this.app, iterator2);
        }

        public static int append(Appendable result2, CharacterIterator iterator2) {
            try {
                int start = iterator2.getBeginIndex();
                int limit = iterator2.getEndIndex();
                int length = limit - start;
                if (start < limit) {
                    result2.append(iterator2.first());
                    while (++start < limit) {
        ...
        public void formatAndAppend(Format formatter, Object arg) {
            if (this.attributes == null) {
                ...
                while (start < limit) {
                    Map<AttributedCharacterIterator.Attribute, Object> map2 = formattedArg.getAttributes();
                    int runLimit = formattedArg.getRunLimit();
                    if (map2.size() != 0) {
                        for (Map.Entry<AttributedCharacterIterator.Attribute, Object> entry : map2.entrySet()) {
