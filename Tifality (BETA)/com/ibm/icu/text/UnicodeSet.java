...
Freezable<UnicodeSet> {
    ...
    public Object clone() {
        UnicodeSet result2 = new UnicodeSet(this);
        result2.bmpSet = this.bmpSet;
        result2.stringSpan = this.stringSpan;
        return result2;
    }
    ...
    public String toPattern(boolean escapeUnprintable) {
        ...
        return this._toPattern(result2, escapeUnprintable).toString();
    }

    private StringBuffer _toPattern(StringBuffer result2, boolean escapeUnprintable) {
        if (this.pat != null) {
            ...
            while (i < this.pat.length()) {
                ...
                if (escapeUnprintable && Utility.isUnprintable(c)) {
                    ...
                    Utility.escapeUnprintable(result2, c);
                    backslashCount = 0;
                    continue;
                }
                UTF16.append(result2, c);
                if (c == 92) {
                    ++backslashCount;
                    continue;
                }
        ...
        return this._generatePattern(result2, escapeUnprintable, true);
    }

    public StringBuffer _generatePattern(StringBuffer result2, boolean escapeUnprintable) {
        return this._generatePattern(result2, escapeUnprintable, true);
    }

    public StringBuffer _generatePattern(StringBuffer result2, boolean escapeUnprintable, boolean includeStrings) {
        int end;
        int start;
        int i;
        result2.append('[');
        ...
        if (count > 1 && this.getRangeStart(0) == 0 && this.getRangeEnd(count - 1) == 0x10FFFF) {
            ...
            for (i = 1; i < count; ++i) {
                ...
                UnicodeSet._appendToPat(result2, start, escapeUnprintable);
                if (start == end) continue;
                if (start + 1 != end) {
                    result2.append('-');
                }
                UnicodeSet._appendToPat(result2, end, escapeUnprintable);
            }
        } else {
            for (i = 0; i < count; ++i) {
        ...
        if (includeStrings && this.strings.size() > 0) {
            for (String s : this.strings) {
                result2.append('{');
                UnicodeSet._appendToPat(result2, s, escapeUnprintable);
                result2.append('}');
            }
        }
    ...
    public String getRegexEquivalent() {
        ...
        StringBuffer result2 = new StringBuffer("(?:");
        this._generatePattern(result2, true, false);
        for (String s : this.strings) {
            result2.append('|');
            UnicodeSet._appendToPat(result2, s, true);
        }
...
lbl78:
    ...
    private static boolean resemblesPropertyPattern(RuleCharacterIterator chars, int iterOpts) {
        ...
        if (c == 91 || c == 92) {
            ...
            result2 = c == 91 ? d == 58 : d == 78 || d == 112 || d == 80;
        }
        chars.setPos(pos);
        return result2;
    }
    ...
    private static final void addCaseMapping(UnicodeSet set, int result2, StringBuilder full) {
        if (result2 >= 0) {
            if (result2 > 31) {
                set.add(result2);
            } else {
    ...
    public UnicodeSet closeOver(int attribute) {
        ...
        if ((attribute & 6) != 0) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                for (cp = start; cp <= end; ++cp) {
                    int result2 = csp.toFullLower(cp, null, full, root, locCache);
                    UnicodeSet.addCaseMapping(foldSet, result2, full);
                    result2 = csp.toFullTitle(cp, null, full, root, locCache);
                    UnicodeSet.addCaseMapping(foldSet, result2, full);
                    result2 = csp.toFullUpper(cp, null, full, root, locCache);
                    UnicodeSet.addCaseMapping(foldSet, result2, full);
                    result2 = csp.toFullFolding(cp, full, 0);
                    UnicodeSet.addCaseMapping(foldSet, result2, full);
                }
            }
    ...
    public UnicodeSet cloneAsThawed() {
        UnicodeSet result2 = (UnicodeSet)this.clone();
        result2.bmpSet = null;
        result2.stringSpan = null;
        return result2;
    }
    ...
    public int compareTo(UnicodeSet o, ComparisonStyle style) {
        ...
        while (true) {
            int result2;
            if (0 != (result2 = this.list[i] - o.list[i])) {
    ...
    public static <T extends Comparable<T>> int compare(Iterator<T> first, Iterator<T> other) {
        ...
        do {
            ...
            return 1;
        } while ((result2 = (item1 = (Comparable)first.next()).compareTo(item2 = (Comparable)other.next())) == 0);
        return result2;
    }
    ...
    public String stripFrom(CharSequence source, boolean matches) {
        ...
        while (pos < source.length()) {
            ...
            result2.append(source.subSequence(pos, inside));
    ...
    implements SymbolTable {
        ...
        public boolean applyPropertyAlias(String propertyName, String propertyValue, UnicodeSet result2) {
            return false;
        }
