...
public final class StringPrep {
    ...
    private static final void getValues(char trieWord, Values values2) {
        values2.reset();
        if (trieWord == '\u0000') {
            values2.type = 4;
        } else if (trieWord >= '\ufff0') {
            values2.type = trieWord - 65520;
        } else {
            values2.type = 1;
            if ((trieWord & 2) > 0) {
                values2.isIndex = true;
                values2.value = trieWord >> 2;
            } else {
                values2.isIndex = false;
                values2.value = trieWord << 16 >> 16;
                values2.value >>= 2;
            }
            if (trieWord >> 2 == 16319) {
                values2.type = 3;
    ...
    private StringBuffer map(UCharacterIterator iter, int options) throws StringPrepParseException {
        ...
        while ((ch = iter.nextCodePoint()) != -1) {
            result2 = this.getCodePointValue(ch);
            StringPrep.getValues(result2, val);
    ...
    public StringBuffer prepare(UCharacterIterator src, int options) throws StringPrepParseException {
        ...
        while ((ch = iter.nextCodePoint()) != -1) {
            char result2 = this.getCodePointValue(ch);
            StringPrep.getValues(result2, val);
            if (val.type == 2) {
    ...
    public String prepare(String src, int options) throws StringPrepParseException {
        StringBuffer result2 = this.prepare(UCharacterIterator.getInstance(src), options);
