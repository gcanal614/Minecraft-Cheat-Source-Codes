...
public class Bidi {
    ...
    public void setCustomClassifier(BidiClassifier classifier2) {
        this.customClassifier = classifier2;
    }
    ...
    void getLogicalToVisualRunsMap() {
        ...
        long[] keys2 = new long[count];
        for (i = 0; i < count; ++i) {
            keys2[i] = ((long)this.runs[i].start << 32) + (long)i;
        }
        Arrays.sort(keys2);
        for (i = 0; i < count; ++i) {
            this.logicalToVisualRunsMap[i] = (int)(keys2[i] & 0xFFFFFFFFFFFFFFFFL);
        }
