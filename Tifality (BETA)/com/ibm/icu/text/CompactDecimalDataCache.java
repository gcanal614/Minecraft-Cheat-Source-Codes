...
class CompactDecimalDataCache {
    ...
    DataBundle get(ULocale locale) {
        DataBundle result2 = this.cache.get(locale);
        if (result2 == null) {
            result2 = CompactDecimalDataCache.load(locale);
    ...
    private static Data loadStyle(ICUResourceBundle r, ULocale locale, String style) {
        ...
        for (int i = 0; i < size; ++i) {
            CompactDecimalDataCache.populateData(r.get(i), locale, style, result2);
        }
        CompactDecimalDataCache.fillInMissing(result2);
        return result2;
    }

    private static void populateData(UResourceBundle divisorData, ULocale locale, String style, Data result2) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if ((nz = CompactDecimalDataCache.populatePrefixSuffix(pluralVariant, thisIndex, template, locale, style, result2)) == numZeros) continue;
            if (numZeros != 0) {
    ...
    private static int populatePrefixSuffix(String pluralVariant, int idx, String template, ULocale locale, String style, Data result2) {
        int i;
