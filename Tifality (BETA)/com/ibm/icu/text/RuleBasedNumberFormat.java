...
extends NumberFormat {
    ...
    public RuleBasedNumberFormat(String description2) {
        this.locale = ULocale.getDefault(ULocale.Category.FORMAT);
        this.init(description2, null);
    }

    public RuleBasedNumberFormat(String description2, String[][] localizations) {
        this.locale = ULocale.getDefault(ULocale.Category.FORMAT);
        this.init(description2, localizations);
    }

    public RuleBasedNumberFormat(String description2, Locale locale) {
        this(description2, ULocale.forLocale(locale));
    }

    public RuleBasedNumberFormat(String description2, ULocale locale) {
        this.locale = locale;
        this.init(description2, null);
    }

    public RuleBasedNumberFormat(String description2, String[][] localizations, ULocale locale) {
        this.locale = locale;
        this.init(description2, localizations);
    }
    ...
    public RuleBasedNumberFormat(ULocale locale, int format) {
        ...
        try {
            description2 = bundle.getString(rulenames[format - 1]);
        }
            ...
            try {
                ...
                while (it.hasNext()) {
                    description2 = description2.concat(it.nextString());
                }
            }
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.ruleSets.length; ++i) {
            result2.append(this.ruleSets[i].toString());
        }
    ...
    private void readObject(ObjectInputStream in) throws IOException {
        ULocale loc;
        String description2 = in.readUTF();
        try {
        ...
        RuleBasedNumberFormat temp = new RuleBasedNumberFormat(description2, loc);
    ...
    public ULocale[] getRuleSetDisplayNameLocales() {
        if (this.ruleSetDisplayNames != null) {
            ...
            ULocale[] result2 = new ULocale[locales.length];
            for (int i = 0; i < locales.length; ++i) {
                result2[i] = new ULocale(locales[i]);
            }
            return result2;
        }
        return null;
    ...
    private String extractSpecial(StringBuilder description2, String specialName) {
        String result2 = null;
        int lp = description2.indexOf(specialName);
        if (lp != -1 && (lp == 0 || description2.charAt(lp - 1) == ';')) {
            int lpStart;
            int lpEnd = description2.indexOf(";%", lp);
            if (lpEnd == -1) {
                lpEnd = description2.length() - 1;
            }
            for (lpStart = lp + specialName.length(); lpStart < lpEnd && PatternProps.isWhiteSpace(description2.charAt(lpStart)); ++lpStart) {
            }
            result2 = description2.substring(lpStart, lpEnd);
            description2.delete(lp, lpEnd + 1);
        }
        return result2;
    }

    private void init(String description2, String[][] localizations) {
        int i;
        this.initLocalizations(localizations);
        StringBuilder descBuf = this.stripWhitespace(description2);
    ...
    private void initLocalizations(String[][] localizations) {
        if (localizations != null) {
            ...
            for (int i = 1; i < localizations.length; ++i) {
                String[] data2 = localizations[i];
                String loc = data2[0];
                String[] names = new String[data2.length - 1];
                ...
                System.arraycopy(data2, 1, names, 0, names.length);
                m.put(loc, names);
            }
            if (!m.isEmpty()) {
    ...
    private StringBuilder stripWhitespace(String description2) {
        StringBuilder result2 = new StringBuilder();
        int start = 0;
        while (start != -1 && start < description2.length()) {
            while (start < description2.length() && PatternProps.isWhiteSpace(description2.charAt(start))) {
                ++start;
            }
            if (start < description2.length() && description2.charAt(start) == ';') {
                ++start;
                continue;
            }
            int p = description2.indexOf(59, start);
            if (p == -1) {
                result2.append(description2.substring(start));
                start = -1;
                continue;
            }
            if (p < description2.length()) {
                result2.append(description2.substring(start, p + 1));
                start = p + 1;
                continue;
            }
            start = -1;
    ...
    private String format(double number, NFRuleSet ruleSet) {
        ...
        ruleSet.format(number, result2, 0);
        this.postProcess(result2, ruleSet);
        return result2.toString();
    }
    ...
    private void postProcess(StringBuffer result2, NFRuleSet ruleSet) {
        if (this.postProcessRules != null) {
            ...
            this.postProcessor.process(result2, ruleSet);
        }
    }
