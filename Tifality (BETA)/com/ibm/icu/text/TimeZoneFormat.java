...
Serializable {
    ...
    public String format(Style style, TimeZone tz, long date, Output<TimeType> timeType) {
        ...
        switch (style) {
            case GENERIC_LOCATION: {
                result2 = this.getTimeZoneGenericNames().getGenericLocationName(ZoneMeta.getCanonicalCLDRID(tz));
                break;
            }
            case GENERIC_LONG: {
                result2 = this.getTimeZoneGenericNames().getDisplayName(tz, TimeZoneGenericNames.GenericNameType.LONG, date);
                break;
            }
            case GENERIC_SHORT: {
                result2 = this.getTimeZoneGenericNames().getDisplayName(tz, TimeZoneGenericNames.GenericNameType.SHORT, date);
                break;
            }
            case SPECIFIC_LONG: {
                result2 = this.formatSpecific(tz, TimeZoneNames.NameType.LONG_STANDARD, TimeZoneNames.NameType.LONG_DAYLIGHT, date, timeType);
                break;
            }
            case SPECIFIC_SHORT: {
                result2 = this.formatSpecific(tz, TimeZoneNames.NameType.SHORT_STANDARD, TimeZoneNames.NameType.SHORT_DAYLIGHT, date, timeType);
                break;
            }
        }
        if (result2 == null) {
            ...
            switch (style) {
                ...
                case SPECIFIC_LONG: 
                case LOCALIZED_GMT: {
                    result2 = this.formatOffsetLocalizedGMT(offset);
                    break;
                }
                case GENERIC_SHORT: 
                case SPECIFIC_SHORT: 
                case LOCALIZED_GMT_SHORT: {
                    result2 = this.formatOffsetShortLocalizedGMT(offset);
                    break;
                }
                case ISO_BASIC_SHORT: {
                    result2 = this.formatOffsetISO8601Basic(offset, true, true, true);
                    break;
                }
                case ISO_BASIC_LOCAL_SHORT: {
                    result2 = this.formatOffsetISO8601Basic(offset, false, true, true);
                    break;
                }
                case ISO_BASIC_FIXED: {
                    result2 = this.formatOffsetISO8601Basic(offset, true, false, true);
                    break;
                }
                case ISO_BASIC_LOCAL_FIXED: {
                    result2 = this.formatOffsetISO8601Basic(offset, false, false, true);
                    break;
                }
                case ISO_BASIC_FULL: {
                    result2 = this.formatOffsetISO8601Basic(offset, true, false, false);
                    break;
                }
                case ISO_BASIC_LOCAL_FULL: {
                    result2 = this.formatOffsetISO8601Basic(offset, false, false, false);
                    break;
                }
                case ISO_EXTENDED_FIXED: {
                    result2 = this.formatOffsetISO8601Extended(offset, true, false, true);
                    break;
                }
                case ISO_EXTENDED_LOCAL_FIXED: {
                    result2 = this.formatOffsetISO8601Extended(offset, false, false, true);
                    break;
                }
                case ISO_EXTENDED_FULL: {
                    result2 = this.formatOffsetISO8601Extended(offset, true, false, false);
                    break;
                }
                case ISO_EXTENDED_LOCAL_FULL: {
                    result2 = this.formatOffsetISO8601Extended(offset, false, false, false);
                    break;
                }
                case ZONE_ID: {
                    result2 = tz.getID();
                ...
                case EXEMPLAR_LOCATION: {
                    result2 = this.formatExemplarLocation(tz);
                }
            }
            if (timeType != null) {
    ...
    public StringBuffer format(Object obj, StringBuffer toAppendTo, FieldPosition pos) {
        ...
        assert (tz != null);
        String result2 = this.formatOffsetLocalizedGMT(tz.getOffset(date));
        toAppendTo.append(result2);
        if (pos.getFieldAttribute() == DateFormat.Field.TIME_ZONE || pos.getField() == 17) {
            pos.setBeginIndex(0);
            pos.setEndIndex(result2.length());
        }
        return toAppendTo;
    }
    ...
    private String formatOffsetISO8601(int offset, boolean isBasic, boolean useUtcIndicator, boolean isShort, boolean ignoreSeconds) {
        ...
        int[] fields2 = new int[]{absOffset / 3600000, (absOffset %= 3600000) / 60000, (absOffset %= 60000) / 1000};
        assert (fields2[0] >= 0 && fields2[0] <= 23);
        assert (fields2[1] >= 0 && fields2[1] <= 59);
        assert (fields2[2] >= 0 && fields2[2] <= 59);
        for (lastIdx = maxFields.ordinal(); lastIdx > minFields.ordinal() && fields2[lastIdx] == 0; --lastIdx) {
        }
        StringBuilder buf = new StringBuilder();
        int sign = 43;
        if (offset < 0) {
            for (idx = 0; idx <= lastIdx; ++idx) {
                if (fields2[idx] == 0) continue;
                sign = 45;
                break;
            }
        }
    ...
    private int parseOffsetFields(String text, int start, boolean isShort, int[] parsedLen) {
        ...
        if (outLen > 0 && this._abuttingOffsetHoursAndMinutes) {
            ...
            for (GMTOffsetPatternType gmtPatType : PARSE_GMT_OFFSET_TYPES) {
                Object[] items = this._gmtOffsetPatternItems[gmtPatType.ordinal()];
                assert (items != null);
                tmpLen = this.parseOffsetFieldsWithPattern(text, start, items, true, fields2);
    ...
    private int parseOffsetFieldsWithPattern(String text, int start, Object[] patternItems, boolean forceSingleHourDigit, int[] fields2) {
        assert (fields2 != null && fields2.length >= 3);
        fields2[2] = 0;
        fields2[1] = 0;
        fields2[0] = 0;
    ...
    private void writeObject(ObjectOutputStream oos) throws IOException {
        ObjectOutputStream.PutField fields2 = oos.putFields();
        fields2.put("_locale", this._locale);
        fields2.put("_tznames", this._tznames);
        fields2.put("_gmtPattern", this._gmtPattern);
        fields2.put("_gmtOffsetPatterns", this._gmtOffsetPatterns);
        fields2.put("_gmtOffsetDigits", this._gmtOffsetDigits);
        fields2.put("_gmtZeroFormat", this._gmtZeroFormat);
        fields2.put("_parseAllStyles", this._parseAllStyles);
        oos.writeFields();
    }

    private void readObject(ObjectInputStream ois) throws ClassNotFoundException, IOException {
        ObjectInputStream.GetField fields2 = ois.readFields();
        this._locale = (ULocale)fields2.get("_locale", null);
        if (this._locale == null) {
            throw new InvalidObjectException("Missing field: locale");
        }
        this._tznames = (TimeZoneNames)fields2.get("_tznames", null);
        if (this._tznames == null) {
            throw new InvalidObjectException("Missing field: tznames");
        }
        this._gmtPattern = (String)fields2.get("_gmtPattern", null);
        if (this._gmtPattern == null) {
            throw new InvalidObjectException("Missing field: gmtPattern");
        }
        String[] tmpGmtOffsetPatterns = (String[])fields2.get("_gmtOffsetPatterns", null);
        ...
        this._gmtOffsetDigits = (String[])fields2.get("_gmtOffsetDigits", null);
        ...
        this._gmtZeroFormat = (String)fields2.get("_gmtZeroFormat", null);
        if (this._gmtZeroFormat == null) {
            throw new InvalidObjectException("Missing field: gmtZeroFormat");
        }
        this._parseAllStyles = fields2.get("_parseAllStyles", false);
        if (fields2.defaulted("_parseAllStyles")) {
    ...
    public TimeZoneFormat cloneAsThawed() {
        TimeZoneFormat copy2 = (TimeZoneFormat)super.clone();
        copy2._frozen = false;
        return copy2;
    }
    ...
    extends SoftCache<ULocale, TimeZoneFormat, ULocale> {
        ...
        @Override
        protected TimeZoneFormat createInstance(ULocale key, ULocale data2) {
            TimeZoneFormat fmt = new TimeZoneFormat(data2);
            fmt.freeze();
            return fmt;
        }
    }
