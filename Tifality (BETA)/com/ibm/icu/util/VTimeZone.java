...
extends BasicTimeZone {
        ...
        if (finalRuleCount == 1) {
            if (rules.size() == 1) {
                ...
                if (start == finalStart) {
                    ...
                    int[] fields2 = Grego.timeToFields(start.getTime(), null);
                    newRule = new AnnualTimeZoneRule(finalRule.getName(), finalRule.getRawOffset(), finalRule.getDSTSavings(), finalRule.getRule(), finalRule.getStartYear(), fields2[0]);
                }
    ...
    private static TimeZoneRule createRuleByRRULE(String tzname, int rawOffset, int dstSavings, long start, List<String> dates, int fromOffset) {
        ...
        if (dates.size() == 1) {
            ...
            for (int i = 1; i < dates.size(); ++i) {
                rrule = dates.get(i);
                long[] unt = new long[1];
                int[] fields2 = VTimeZone.parseRRULE(rrule, unt);
                if (unt[0] > until[0]) {
                    until = unt;
                }
                if (fields2[0] == -1 || fields2[1] == 0 || fields2[3] == 0) {
                    return null;
                }
                int count = fields2.length - 3;
                if (daysCount + count > 7) {
                    return null;
                }
                if (fields2[1] != dayOfWeek) {
                    return null;
                }
                if (fields2[0] != month) {
                    if (anotherMonth == -1) {
                        int diff = fields2[0] - month;
                        if (diff == -11 || diff == -1) {
                            earliestMonth = anotherMonth = fields2[0];
                            earliestDay = 31;
                        } else {
                            if (diff != 11 && diff != 1) return null;
                            anotherMonth = fields2[0];
                        }
                    } else if (fields2[0] != month && fields2[0] != anotherMonth) {
                        return null;
                    }
                }
                if (fields2[0] == earliestMonth) {
                    for (int j = 0; j < count; ++j) {
                        int dom = fields2[3 + j];
                        dom = dom > 0 ? dom : MONTHLENGTH[fields2[0]] + dom + 1;
                        earliestDay = dom < earliestDay ? dom : earliestDay;
                    }
                }
                daysCount += count;
    ...
    private static String getDateTimeString(long time) {
        int[] fields2 = Grego.timeToFields(time, null);
        StringBuilder sb = new StringBuilder(15);
        sb.append(VTimeZone.numToString(fields2[0], 4));
        sb.append(VTimeZone.numToString(fields2[1] + 1, 2));
        sb.append(VTimeZone.numToString(fields2[2], 2));
        sb.append('T');
        int t = fields2[5];
        int hour = t / 3600000;
