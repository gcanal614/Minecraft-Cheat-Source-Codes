...
Iterable<Entry> {
    ...
    public BytesTrie.Result next(CharSequence s, int sIndex, int sLimit) {
        ...
        block0: while (true) {
            ...
            while (true) {
                if (node < 48) {
                    BytesTrie.Result result2 = this.branchNext(pos, node, inUnit);
        ...
        do {
            if (inUnit == this.chars_.charAt(pos++)) {
                ...
                if ((node & 0x8000) != 0) {
                    ...
                    result2 = node >= 64 ? valueResults_[node >> 15] : BytesTrie.Result.NO_VALUE;
                }
                this.pos_ = pos;
                return result2;
            }
        ...
        while (true) {
            ...
            if (node < 64) {
                ...
                if (length < 0) {
                    ...
                    node = c;
                    if (c >= '@') {
                        result2 = valueResults_[node >> 15];
                        return result2;
                    }
                }
                result2 = BytesTrie.Result.NO_VALUE;
                return result2;
            }
