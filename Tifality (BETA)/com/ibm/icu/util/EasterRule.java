...
implements DateRule {
    ...
    private Date doFirstBetween(Date start, Date end) {
        GregorianCalendar gregorianCalendar = this.calendar;
        synchronized (gregorianCalendar) {
            Date result2 = this.computeInYear(start, this.calendar);
            if (result2.before(start)) {
                this.calendar.setTime(start);
                this.calendar.get(1);
                this.calendar.add(1, 1);
                result2 = this.computeInYear(this.calendar.getTime(), this.calendar);
            }
            if (end != null && result2.after(end)) {
                return null;
            }
            return result2;
        }
