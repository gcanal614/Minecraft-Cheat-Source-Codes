...
implements Enumeration<Object> {
    ...
    public String nextToken() {
        if (this.m_tokenOffset_ < 0) {
            ...
            if (this.m_returnDelimiters_) {
                ...
                String result3 = tokenlimit < 0 ? this.m_source_.substring(this.m_nextOffset_) : this.m_source_.substring(this.m_nextOffset_, tokenlimit);
        ...
        String result4 = this.m_tokenLimit_[this.m_tokenOffset_] >= 0 ? this.m_source_.substring(this.m_tokenStart_[this.m_tokenOffset_], this.m_tokenLimit_[this.m_tokenOffset_]) : this.m_source_.substring(this.m_tokenStart_[this.m_tokenOffset_]);
        ++this.m_tokenOffset_;
        this.m_nextOffset_ = -1;
        if (this.m_tokenOffset_ < this.m_tokenSize_) {
    ...
    public int countTokens() {
        ...
        if (this.hasMoreTokens()) {
            ...
            do {
                if (this.m_tokenStart_.length == result2) {
                ...
                this.m_tokenStart_[result2] = this.m_nextOffset_;
                if (this.m_returnDelimiters_) {
                    boolean contains;
                    int c = UTF16.charAt(this.m_source_, this.m_nextOffset_);
                    boolean bl = this.delims == null ? this.m_delimiters_.contains(c) : (contains = c < this.delims.length && this.delims[c]);
                    if (contains) {
                        if (this.m_coalesceDelimiters_) {
                            this.m_tokenLimit_[result2] = this.getNextNonDelimiter(this.m_nextOffset_);
                        } else {
                            int p = this.m_nextOffset_ + 1;
                            if (p == this.m_length_) {
                                p = -1;
                            }
                            this.m_tokenLimit_[result2] = p;
                        }
                    } else {
                        this.m_tokenLimit_[result2] = this.getNextDelimiter(this.m_nextOffset_);
                    }
                    this.m_nextOffset_ = this.m_tokenLimit_[result2];
                } else {
                    this.m_tokenLimit_[result2] = this.getNextDelimiter(this.m_nextOffset_);
                    this.m_nextOffset_ = this.getNextNonDelimiter(this.m_tokenLimit_[result2]);
                }
                ++result2;
            } while (this.m_nextOffset_ >= 0);
            this.m_tokenOffset_ = 0;
            this.m_tokenSize_ = result2;
    ...
    private int getNextDelimiter(int offset) {
        if (offset >= 0) {
            ...
            int c = 0;
            if (this.delims == null) {
                while (!this.m_delimiters_.contains(c = UTF16.charAt(this.m_source_, result2)) && ++result2 < this.m_length_) {
                }
            } else {
                while (!((c = UTF16.charAt(this.m_source_, result2)) < this.delims.length && this.delims[c] || ++result2 >= this.m_length_)) {
                }
            }
            if (result2 < this.m_length_) {
                return result2;
            }
        }
