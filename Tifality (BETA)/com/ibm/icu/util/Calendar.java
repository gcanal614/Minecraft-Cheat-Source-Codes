...
Comparable<Calendar> {
    ...
    public static final String[] getKeywordValuesForLocale(String key, ULocale locale, boolean commonlyUsed) {
        ...
        for (i = 0; i < caltypes.length; ++i) {
            values2.add(caltypes[i]);
        }
        for (i = 0; i < calTypes.length; ++i) {
            if (values2.contains(calTypes[i])) continue;
            values2.add(calTypes[i]);
        }
    ...
    public int getActualMaximum(int field) {
        ...
        switch (field) {
            case 5: {
                ...
                result2 = this.handleGetMonthLength(cal.get(19), cal.get(2));
                break;
            }
            case 6: {
                Calendar cal = (Calendar)this.clone();
                cal.setLenient(true);
                cal.prepareGetActual(field, false);
                result2 = this.handleGetYearLength(cal.get(19));
                break;
            }
            case 0: 
            case 7: 
            ...
            case 20: 
            case 21: {
                result2 = this.getMaximum(field);
                break;
            }
            default: {
                result2 = this.getActualHelper(field, this.getLeastMaximum(field), this.getMaximum(field));
            }
        }
        return result2;
    }
    ...
    public int getActualMinimum(int field) {
        ...
        switch (field) {
            ...
            case 20: 
            case 21: {
                result2 = this.getMinimum(field);
                break;
            }
            default: {
                result2 = this.getActualHelper(field, this.getGreatestMinimum(field), this.getMinimum(field));
            }
        }
        return result2;
    }
    ...
    private static DateFormat formatHelper(Calendar cal, ULocale loc, int dateStyle, int timeStyle) {
        ...
        DateFormat result2 = cal.handleGetDateFormat(pattern, override, loc);
        result2.setCalendar(cal);
        return result2;
    }
    ...
    private void setWeekData(ULocale locale) {
        WeekData data2 = cachedLocaleData.get(locale);
        if (data2 == null) {
            ULocale useLocale;
            ...
            data2 = new WeekData(wdi[0], wdi[1], wdi[2], wdi[3], wdi[4], wdi[5], calData.getULocale());
            cachedLocaleData.put(locale, data2);
        }
        this.setFirstDayOfWeek(data2.firstDayOfWeek);
        this.setMinimalDaysInFirstWeek(data2.minimalDaysInFirstWeek);
        this.weekendOnset = data2.weekendOnset;
        this.weekendOnsetMillis = data2.weekendOnsetMillis;
        this.weekendCease = data2.weekendCease;
        this.weekendCeaseMillis = data2.weekendCeaseMillis;
        ULocale uloc = data2.actualLocale;
