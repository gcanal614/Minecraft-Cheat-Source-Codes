...
Freezable<TimeZone> {
    ...
    public int getOffset(long date) {
        ...
        this.getOffset(date, false, result2);
    ...
    public void getOffset(long date, boolean local, int[] offsets) {
        ...
        int[] fields2 = new int[6];
        int pass = 0;
        while (true) {
            Grego.timeToFields(date, fields2);
            offsets[1] = this.getOffset(1, fields2[0], fields2[1], fields2[2], fields2[3], fields2[5]) - offsets[0];
    ...
    private String _getDisplayName(int style, boolean daylight, ULocale locale) {
        ...
        if (style == 7 || style == 3 || style == 2) {
            ...
            switch (style) {
                case 7: {
                    result2 = tzfmt.format(TimeZoneFormat.Style.GENERIC_LOCATION, this, date, timeType);
                    break;
                }
                case 3: {
                    result2 = tzfmt.format(TimeZoneFormat.Style.GENERIC_LONG, this, date, timeType);
                    break;
                }
                case 2: {
                    result2 = tzfmt.format(TimeZoneFormat.Style.GENERIC_SHORT, this, date, timeType);
                }
            }
            if (daylight && timeType.value == TimeZoneFormat.TimeType.STANDARD || !daylight && timeType.value == TimeZoneFormat.TimeType.DAYLIGHT) {
                ...
                result2 = style == 2 ? tzfmt.formatOffsetShortLocalizedGMT(offset) : tzfmt.formatOffsetLocalizedGMT(offset);
            }
            ...
            switch (style) {
                case 5: {
                    result2 = tzfmt.formatOffsetLocalizedGMT(offset);
                    break;
                }
                case 4: {
                    result2 = tzfmt.formatOffsetISO8601Basic(offset, false, false, false);
                }
            }
        } else {
            ...
            result2 = tznames.getDisplayName(ZoneMeta.getCanonicalCLDRID(this), nameType, date);
            if (result2 == null) {
                TimeZoneFormat tzfmt = TimeZoneFormat.getInstance(locale);
                int offset = daylight && this.useDaylightTime() ? this.getRawOffset() + this.getDSTSavings() : this.getRawOffset();
                String string = result2 = style == 1 ? tzfmt.formatOffsetLocalizedGMT(offset) : tzfmt.formatOffsetShortLocalizedGMT(offset);
            }
        }
        assert (result2 != null);
        return result2;
    ...
    public static TimeZone getTimeZone(String ID, int type2) {
        return TimeZone.getTimeZone(ID, type2, false);
    }

    private static synchronized TimeZone getTimeZone(String ID, int type2, boolean frozen) {
        TimeZone result2;
        if (type2 == 1) {
            result2 = JavaTimeZone.createTimeZone(ID);
            if (result2 != null) {
            ...
            result2 = ZoneMeta.getSystemTimeZone(ID);
        }
        if (result2 == null) {
            result2 = ZoneMeta.getCustomTimeZone(ID);
        }
        if (result2 == null) {
            LOGGER.fine("\"" + ID + "\" is a bogus id so timezone is falling back to Etc/Unknown(GMT).");
            result2 = UNKNOWN_ZONE;
        }
        return frozen ? result2 : result2.cloneAsThawed();
    }
