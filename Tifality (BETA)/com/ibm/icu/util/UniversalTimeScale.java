...
public final class UniversalTimeScale {
    ...
    public static long from(long otherTime, int timeScale) {
        TimeScaleData data2 = UniversalTimeScale.fromRangeCheck(otherTime, timeScale);
        return (otherTime + data2.epochOffset) * data2.units;
    }

    public static BigDecimal bigDecimalFrom(double otherTime, int timeScale) {
        TimeScaleData data2 = UniversalTimeScale.getTimeScaleData(timeScale);
        BigDecimal other = new BigDecimal(String.valueOf(otherTime));
        BigDecimal units = new BigDecimal(data2.units);
        BigDecimal epochOffset = new BigDecimal(data2.epochOffset);
    ...
    public static long toLong(long universalTime, int timeScale) {
        TimeScaleData data2 = UniversalTimeScale.toRangeCheck(universalTime, timeScale);
        if (universalTime < 0L) {
            if (universalTime < data2.minRound) {
                return (universalTime + data2.unitsRound) / data2.units - data2.epochOffsetP1;
            }
            return (universalTime - data2.unitsRound) / data2.units - data2.epochOffset;
        }
        if (universalTime > data2.maxRound) {
            return (universalTime - data2.unitsRound) / data2.units - data2.epochOffsetM1;
        }
        return (universalTime + data2.unitsRound) / data2.units - data2.epochOffset;
    }
    ...
    public static long getTimeScaleValue(int scale, int value) {
        TimeScaleData data2 = UniversalTimeScale.getTimeScaleData(scale);
        switch (value) {
            case 0: {
                return data2.units;
            }
            case 1: {
                return data2.epochOffset;
            }
            case 2: {
                return data2.fromMin;
            }
            case 3: {
                return data2.fromMax;
            }
            case 4: {
                return data2.toMin;
            }
            ...
            case 6: {
                return data2.epochOffsetP1;
            }
            case 7: {
                return data2.epochOffsetM1;
            }
            case 8: {
                return data2.unitsRound;
            }
            case 9: {
                return data2.minRound;
            }
            case 10: {
                return data2.maxRound;
            }
        }
    ...
    private static TimeScaleData toRangeCheck(long universalTime, int scale) {
        TimeScaleData data2 = UniversalTimeScale.getTimeScaleData(scale);
        if (universalTime >= data2.toMin && universalTime <= data2.toMax) {
            return data2;
        }
    ...
    private static TimeScaleData fromRangeCheck(long otherTime, int scale) {
        TimeScaleData data2 = UniversalTimeScale.getTimeScaleData(scale);
        if (otherTime >= data2.fromMin && otherTime <= data2.fromMax) {
            return data2;
        }
