...
public class LocaleMatcher {
    ...
    private ULocale addLikelySubtags(ULocale languageCode) {
        ULocale result2 = ULocale.addLikelySubtags(languageCode);
        if (result2 == null || result2.equals(languageCode)) {
    ...
    implements Freezable<LanguageMatcherData> {
        ...
        private LanguageMatcherData addDistance(String desired, String supported, int percent, boolean oneway, String comment) {
            ...
            Row.R3<LocalePatternMatcher, LocalePatternMatcher, Double> data2 = Row.of(desiredMatcher, supportedMatcher, score);
            Row.R3<LocalePatternMatcher, LocalePatternMatcher, Double> data22 = oneway ? null : Row.of(supportedMatcher, desiredMatcher, score);
            switch (desiredLen) {
                case language: {
                    String dlanguage = desiredMatcher.getLanguage();
                    String slanguage = supportedMatcher.getLanguage();
                    this.languageScores.addDataToScores(dlanguage, slanguage, data2);
                    if (oneway) break;
                    this.languageScores.addDataToScores(slanguage, dlanguage, data22);
                    break;
                }
                case script: {
                    String dscript = desiredMatcher.getScript();
                    String sscript = supportedMatcher.getScript();
                    this.scriptScores.addDataToScores(dscript, sscript, data2);
                    if (oneway) break;
                    this.scriptScores.addDataToScores(sscript, dscript, data22);
                    break;
                }
                case region: {
                    String dregion = desiredMatcher.getRegion();
                    String sregion = supportedMatcher.getRegion();
                    this.regionScores.addDataToScores(dregion, sregion, data2);
                    if (oneway) break;
                    this.regionScores.addDataToScores(sregion, dregion, data22);
                }
            }
            return this;
        }
        ...
        public LanguageMatcherData cloneAsThawed() {
            try {
                LanguageMatcherData result2 = (LanguageMatcherData)this.clone();
                result2.languageScores = this.languageScores.cloneAsThawed();
                result2.scriptScores = this.scriptScores.cloneAsThawed();
                result2.regionScores = this.regionScores.cloneAsThawed();
    ...
    implements Freezable<ScoreData> {
        ...
        void addDataToScores(String desired, String supported, Row.R3<LocalePatternMatcher, LocalePatternMatcher, Double> data2) {
            this.scores.add(data2);
        }
        ...
        public ScoreData cloneAsThawed() {
            try {
                ScoreData result2 = (ScoreData)this.clone();
                result2.scores = (LinkedHashSet)result2.scores.clone();
                result2.frozen = false;
                return result2;
            }
            catch (CloneNotSupportedException e) {
    ...
    private static class LocalePatternMatcher {
        ...
        public String toString() {
            String result2 = this.getLanguage();
            if (this.level != Level.language) {
                result2 = result2 + "-" + this.getScript();
                if (this.level != Level.script) {
                    result2 = result2 + "-" + this.getRegion();
                }
            }
            return result2;
        }
