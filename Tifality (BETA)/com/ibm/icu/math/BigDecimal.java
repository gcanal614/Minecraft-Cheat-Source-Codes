...
Comparable<BigDecimal> {
    ...
    public int intValueExact() {
        ...
        if (lodigit + useexp == 9 && (topdig = result2 / 1000000000) != this.mant[0]) {
            if (result2 == Integer.MIN_VALUE && this.ind == -1 && this.mant[0] == 2) {
                return result2;
            }
            throw new ArithmeticException("Conversion overflow: " + this.toString());
        }
    ...
    public long longValueExact() {
        ...
        for (i = 0; i <= $17; ++i) {
            result2 *= 10L;
            if (i > lodigit) continue;
            result2 += (long)this.mant[i];
        }
        if (lodigit + useexp == 18 && (topdig = result2 / 1000000000000000000L) != (long)this.mant[0]) {
            if (result2 == Long.MIN_VALUE && this.ind == -1 && this.mant[0] == 9) {
                return result2;
            }
            throw new ArithmeticException("Conversion overflow: " + this.toString());
        }
    ...
    private static final BigDecimal clone(BigDecimal dec) {
        BigDecimal copy2 = new BigDecimal();
        copy2.ind = dec.ind;
        copy2.exp = dec.exp;
        copy2.form = dec.form;
        copy2.mant = dec.mant;
