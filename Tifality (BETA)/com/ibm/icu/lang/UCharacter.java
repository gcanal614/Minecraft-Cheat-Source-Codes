...
UCharacterEnums.ECharacterDirection {
    ...
    public static String toUpperCase(ULocale locale, String str) {
        ...
        while ((c = iter.nextCaseMapCP()) >= 0) {
            if ((c = UCaseProps.INSTANCE.toFullUpper(c, iter, result2, locale, locCache)) < 0) {
                c ^= 0xFFFFFFFF;
    ...
    public static String toLowerCase(ULocale locale, String str) {
        ...
        locCache[0] = 0;
        while ((c = iter.nextCaseMapCP()) >= 0) {
            if ((c = UCaseProps.INSTANCE.toFullLower(c, iter, result2, locale, locCache)) < 0) {
                c ^= 0xFFFFFFFF;
            } else if (c <= 31) continue;
            result2.appendCodePoint(c);
        }
        ...
        while (prev < srcLength) {
            ...
            if (prev < index) {
                ...
                if ((options & 0x200) == 0 && 0 == UCaseProps.INSTANCE.getType(c)) {
                    ...
                    if (prev < titleStart) {
                        result2.append(str, prev, titleStart);
                    }
                } else {
                    titleStart = prev;
                }
                if (titleStart < index) {
                    FirstIJ = true;
                    c = UCaseProps.INSTANCE.toFullTitle(c, iter, result2, locale, locCache);
                    while (true) {
                        if (c < 0) {
                            result2.appendCodePoint(c ^= 0xFFFFFFFF);
                        } else if (c > 31) {
                            result2.appendCodePoint(c);
                        }
                        if ((options & 0x100) != 0) {
                        ...
                        c = UCaseProps.INSTANCE.toFullLower(nc, iter, result2, locale, locCache);
                    }
                }
            }
            prev = index;
    ...
    public static final String foldCase(String str, int options) {
        ...
        while (i < length) {
            ...
            if ((c = UCaseProps.INSTANCE.toFullFolding(c, result2, options)) < 0) {
                c ^= 0xFFFFFFFF;
            } else if (c <= 31) continue;
            result2.appendCodePoint(c);
        }
    ...
    extends Character.Subset {
        ...
        private static String trimBlockName(String name) {
            ...
            StringBuilder result2 = new StringBuilder(upper.length());
