...
implements ValueIterator {
     ...
     */
    private boolean iterateSingleGroup(ValueIterator.Element result2, int limit) {
            ...
            synchronized (GROUP_LENGTHS_) {
                ...
                while (this.m_current_ < limit) {
                    ...
                    if (name != null && name.length() > 0) {
                        result2.integer = this.m_current_;
                        result2.value = name;
    ...
    private boolean iterateGroup(ValueIterator.Element result2, int limit) {
        ...
        while (this.m_groupIndex_ < this.m_name_.m_groupcount_ && this.m_current_ < limit) {
            ...
            if (startMSB == (gMSB = this.m_name_.getGroupMSB(this.m_groupIndex_))) {
                if (startMSB == UCharacterName.getCodepointMSB(limit - 1)) {
                    return this.iterateSingleGroup(result2, limit);
                }
                if (!this.iterateSingleGroup(result2, UCharacterName.getGroupLimit(gMSB))) {
                    return false;
                }
                ++this.m_groupIndex_;
                continue;
            ...
            if (this.m_choice_ == 2 && !this.iterateExtended(result2, gMIN)) {
                return false;
            }
    ...
    private boolean iterateExtended(ValueIterator.Element result2, int limit) {
        while (this.m_current_ < limit) {
