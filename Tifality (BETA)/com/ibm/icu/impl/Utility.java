...
public final class Utility {
    ...
    public static int unescapeAt(String s, int[] offset16) {
        ...
        if (minDig != 0) {
            while (offset < length && n < maxDig && (dig = UCharacter.digit(c = UTF16.charAt(s, offset), bitsPerDigit == 3 ? 8 : 16)) >= 0) {
                result2 = result2 << bitsPerDigit | dig;
                offset += UTF16.getCharCount(c);
                ++n;
            }
            if (n < minDig) {
            ...
            if (result2 < 0 || result2 >= 0x110000) {
                return -1;
            }
            if (offset < length && UTF16.isLeadSurrogate((char)result2)) {
                int ahead = offset + 1;
                c = s.charAt(offset);
                ...
                if (UTF16.isTrailSurrogate((char)c)) {
                    offset = ahead;
                    result2 = UCharacterProperty.getRawSupplementary((char)result2, (char)c);
                }
            }
            offset16[0] = offset;
            return result2;
    ...
    public static String hex(long i, int places) {
        ...
        if ((result2 = Long.toString(i, 16).toUpperCase(Locale.ENGLISH)).length() < places) {
            result2 = "0000000000000000".substring(result2.length(), places) + result2;
        }
        if (negative) {
            return '-' + result2;
        }
    ...
    public static <S extends CharSequence, U extends CharSequence, T extends Appendable> T hex(S s, int width, U separator, boolean useCodePoints, T result2) {
        try {
            if (useCodePoints) {
                int cp;
                for (int i = 0; i < s.length(); i += UTF16.getCharCount(cp)) {
                    ...
                    result2.append(Utility.hex(cp, width));
                }
            } else {
                for (int i = 0; i < s.length(); ++i) {
                    ...
                    result2.append(Utility.hex(s.charAt(i), width));
                }
            }
            return result2;
        }
    ...
    public static String hex(byte[] o, int start, int end, String separator) {
        ...
        for (int i = start; i < end; ++i) {
            ...
            result2.append(Utility.hex(o[i]));
        }
    ...
    private static <T extends Appendable> void recursiveAppendNumber(T result2, int n, int radix, int minDigits) {
        try {
            int digit = n % radix;
            if (n >= radix || minDigits > 1) {
                Utility.recursiveAppendNumber(result2, n / radix, radix, minDigits - 1);
            }
            result2.append(DIGITS[digit]);
        }
        catch (IOException e) {
            throw new IllegalIcuArgumentException(e);
        }
    ...
    public static <T extends Appendable> T appendNumber(T result2, int n, int radix, int minDigits) {
        try {
            if (radix < 2 || radix > 36) {
            ...
            Utility.recursiveAppendNumber(result2, abs, radix, minDigits);
            return result2;
        }
        catch (IOException e) {
            throw new IllegalIcuArgumentException(e);
    ...
    public static <T extends Appendable> boolean escapeUnprintable(T result2, int c) {
        try {
            if (Utility.isUnprintable(c)) {
                result2.append('\\');
                if ((c & 0xFFFF0000) != 0) {
                    result2.append('U');
                    result2.append(DIGITS[0xF & c >> 28]);
                    result2.append(DIGITS[0xF & c >> 24]);
                    result2.append(DIGITS[0xF & c >> 20]);
                    result2.append(DIGITS[0xF & c >> 16]);
                } else {
                    result2.append('u');
                }
                result2.append(DIGITS[0xF & c >> 12]);
                result2.append(DIGITS[0xF & c >> 8]);
                result2.append(DIGITS[0xF & c >> 4]);
                result2.append(DIGITS[0xF & c]);
                return true;
            }
            return false;
        }
    ...
    public static String valueOf(int[] source) {
        StringBuilder result2 = new StringBuilder(source.length);
        for (int i = 0; i < source.length; ++i) {
            result2.appendCodePoint(source[i]);
        }
        return result2.toString();
    }
