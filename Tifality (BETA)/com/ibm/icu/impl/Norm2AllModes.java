...
public final class Norm2AllModes {
    ...
    private static CacheBase<String, Norm2AllModes, InputStream> cache = new SoftCache<String, Norm2AllModes, InputStream>(){
        ...
        @Override
        protected Norm2AllModes createInstance(String key, InputStream data2) {
            Normalizer2Impl impl = data2 == null ? new Normalizer2Impl().load("data/icudt51b/" + key + ".nrm") : new Normalizer2Impl().load(data2);
    ...
    public static Norm2AllModes getInstance(InputStream data2, String name) {
        Norm2AllModesSingleton singleton;
        if (data2 == null && (singleton = name.equals("nfc") ? NFCSingleton.INSTANCE : (name.equals("nfkc") ? NFKCSingleton.INSTANCE : (name.equals("nfkc_cf") ? NFKC_CFSingleton.INSTANCE : null))) != null) {
            if (singleton.exception != null) {
                throw singleton.exception;
            }
            return singleton.allModes;
        }
        return cache.getInstance(name, data2);
    }
