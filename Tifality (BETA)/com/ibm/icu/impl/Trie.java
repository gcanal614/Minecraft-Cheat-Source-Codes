...
public abstract class Trie {
    ...
    public int getSerializedDataSize() {
        int result2 = 16;
        result2 += this.m_dataOffset_ << 1;
        if (this.isCharTrie()) {
            result2 += this.m_dataLength_ << 1;
        } else if (this.isIntTrie()) {
            result2 += this.m_dataLength_ << 2;
        }
        return result2;
    }
    ...
    protected Trie(InputStream inputStream, DataManipulate dataManipulate) throws IOException {
        ...
        int signature2 = input.readInt();
        this.m_options_ = input.readInt();
        if (!this.checkHeader(signature2)) {
    ...
    private final boolean checkHeader(int signature2) {
        if (signature2 != 1416784229) {
            return false;
        }
