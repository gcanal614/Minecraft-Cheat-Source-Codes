    ...
    public static DataRecord read(String ln, RecordReader in) {
        if (in.open("DataRecord")) {
            ...
            if (in.open("ScopeDataList")) {
                ...
                while (null != (data2 = ScopeData.read(in))) {
                    list.add(data2);
                }
                if (in.close()) {
    ...
    public static class ScopeData {
        ...
        public static ScopeData read(RecordReader in) {
            if (in.open("ScopeData")) {
                ScopeData scope2 = new ScopeData();
                scope2.prefix = in.string("prefix");
                scope2.requiresDigitPrefix = in.bool("requiresDigitPrefix");
                scope2.suffix = in.string("suffix");
                if (in.close()) {
                    return scope2;
                }
            }
