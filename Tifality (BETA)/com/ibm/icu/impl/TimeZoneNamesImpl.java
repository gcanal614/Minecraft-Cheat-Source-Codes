...
extends TimeZoneNames {
    ...
    public synchronized Set<String> getAvailableMetaZoneIDs() {
        if (METAZONE_IDS == null) {
            ...
            Set<String> keys2 = mapTimezones.keySet();
            METAZONE_IDS = Collections.unmodifiableSet(keys2);
        }
        return METAZONE_IDS;
    }
    ...
    public String getMetaZoneID(String tzID, long date) {
        ...
        List maps = (List)TZ_TO_MZS_CACHE.getInstance(tzID, tzID);
        for (MZMapEntry map2 : maps) {
            if (date < map2.from() || date >= map2.to()) continue;
            mzID = map2.mzID();
            break;
        }
        return mzID;
    ...
    extends SoftCache<String, Map<String, String>, String> {
        ...
        @Override
        protected Map<String, String> createInstance(String key, String data2) {
            ...
            try {
                ...
                map2 = new HashMap<String, String>(regions.size());
                for (String region : regions) {
                    String tzID = regionMap.getString(region).intern();
                    map2.put(region.intern(), tzID);
                }
            }
    ...
    extends SoftCache<String, List<MZMapEntry>, String> {
        ...
        @Override
        protected List<MZMapEntry> createInstance(String key, String data2) {
            List<MZMapEntry> mzMaps = null;
            UResourceBundle bundle = UResourceBundle.getBundleInstance("com/ibm/icu/impl/data/icudt51b", "metaZones");
            UResourceBundle metazoneInfoBundle = bundle.get("metazoneInfo");
            String tzkey = data2.replace('/', ':');
            try {
    ...
    implements TextTrieMap.ResultHandler<NameInfo> {
        ...
        @Override
        public boolean handlePrefixMatch(int matchLength, Iterator<NameInfo> values2) {
            while (values2.hasNext()) {
                TimeZoneNames.MatchInfo minfo;
                NameInfo ninfo = values2.next();
