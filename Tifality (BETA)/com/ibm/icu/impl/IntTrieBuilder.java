...
extends TrieBuilder {
    ...
    public IntTrie serialize(TrieBuilder.DataManipulate datamanipulate, Trie.DataManipulate triedatamanipulate) {
        ...
        int[] data2 = new int[this.m_dataLength_];
        for (int i = 0; i < this.m_indexLength_; ++i) {
            index[i] = (char)(this.m_index_[i] >>> 2);
        }
        System.arraycopy(this.m_data_, 0, data2, 0, this.m_dataLength_);
        int options = 37;
        options |= 0x100;
        if (this.m_isLatin1Linear_) {
            options |= 0x200;
        }
        return new IntTrie(index, data2, this.m_initialValue_, options, triedatamanipulate);
    }
    ...
    private static final int findSameDataBlock(int[] data2, int dataLength, int otherBlock, int step) {
        dataLength -= 32;
        for (int block = 0; block <= dataLength; block += step) {
            if (!IntTrieBuilder.equal_int(data2, block, otherBlock, 32)) continue;
            return block;
        }
        return -1;
    }
