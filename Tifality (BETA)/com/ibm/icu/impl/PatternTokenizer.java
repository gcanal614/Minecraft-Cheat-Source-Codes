...
public class PatternTokenizer {
    ...
    public String quoteLiteral(String string) {
        ...
        for (int i = 0; i < string.length(); i += UTF16.getCharCount(cp)) {
            ...
            if (this.escapeCharacters.contains(cp)) {
                ...
                this.appendEscaped(result2, cp);
                continue;
            }
            if (this.needingQuoteCharacters.contains(cp)) {
                if (quotedChar == IN_QUOTE) {
                    UTF16.append(result2, cp);
    ...
    private void appendEscaped(StringBuffer result2, int cp) {
        if (cp <= 65535) {
            result2.append("\\u").append(Utility.hex(cp, 4));
        } else {
            result2.append("\\U").append(Utility.hex(cp, 8));
        }
    }
    ...
    public String normalize() {
        ...
        while (true) {
            ...
            if (status != 1) {
                result2.append(this.quoteLiteral(buffer));
                continue;
            }
            result2.append(buffer);
        }
    }
