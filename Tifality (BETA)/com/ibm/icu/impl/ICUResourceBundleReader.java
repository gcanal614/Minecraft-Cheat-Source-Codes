...
implements ICUBinary.Authenticate {
    ...
    extends Container {
        ...
        int findTableItem(CharSequence key) {
            ...
            while (start < limit) {
                ...
                int result2 = this.keyOffsets != null ? this.reader.compareKeys(key, this.keyOffsets[mid]) : this.reader.compareKeys32(key, this.key32Offsets[mid]);
                if (result2 < 0) {
                    limit = mid;
                    continue;
                }
    ...
    extends SoftCache<ReaderInfo, ICUResourceBundleReader, ReaderInfo> {
        ...
        @Override
        protected ICUResourceBundleReader createInstance(ReaderInfo key, ReaderInfo data2) {
            String fullName = ICUResourceBundleReader.getFullName(data2.baseName, data2.localeID);
            InputStream stream = ICUData.getStream(data2.loader, fullName);
            if (stream == null) {
                return NULL_READER;
            }
            return new ICUResourceBundleReader(stream, data2.baseName, data2.localeID, data2.loader);
        }
    }
