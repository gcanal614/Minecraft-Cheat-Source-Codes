...
public class TextTrieMap<V> {
    ...
    private synchronized void find(Node node, CharIterator chitr, ResultHandler<V> handler) {
        Iterator values2 = node.values();
        if (values2 != null && !handler.handlePrefixMatch(chitr.processedLength(), values2)) {
            return;
        }
    ...
    private class Node {
        ...
        private Node(char[] text, List<V> values2, List<Node> children) {
            this._text = text;
    ...
    implements ResultHandler<V> {
        ...
        @Override
        public boolean handlePrefixMatch(int matchLength, Iterator<V> values2) {
            if (matchLength > this.length) {
                this.length = matchLength;
                this.matches = values2;
            }
            return true;
        }
