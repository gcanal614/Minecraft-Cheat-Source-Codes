...
implements ICUBinary.Authenticate {
    ...
    protected void read(UCharacterName data2) throws IOException {
        ...
        data2.setToken(token, tokenstr);
        count = this.m_dataInputStream_.readChar();
        data2.setGroupCountSize(count, 3);
    ...
    private UCharacterName.AlgorithmName readAlg() throws IOException {
        ...
        if (!result2.setInfo(rangestart, rangeend = this.m_dataInputStream_.readInt(), type2 = this.m_dataInputStream_.readByte(), (byte)(variant = this.m_dataInputStream_.readByte()))) {
            return null;
        }
        ...
        if ((size -= 12 + prefix.length() + 1) > 0) {
            ...
            result2.setFactorString(string);
        }
        return result2;
    }
}
