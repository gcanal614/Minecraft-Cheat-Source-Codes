...
public final class UCharacterName {
    ...
    public String getName(int ch, int choice) {
        ...
        String result2 = null;
        result2 = this.getAlgName(ch, choice);
        if (result2 == null || result2.length() == 0) {
            result2 = choice == 2 ? this.getExtendedName(ch) : this.getGroupName(ch, choice);
        }
        return result2;
    }
    ...
    public int getCharFromName(int choice, String name) {
        ...
        int result2 = UCharacterName.getExtendedChar(name.toLowerCase(Locale.ENGLISH), choice);
        if (result2 >= -1) {
            return result2;
        }
        ...
        if (choice == 0 || choice == 2) {
            ...
            --count;
            while (count >= 0) {
                result2 = this.m_algorithm_[count].getChar(upperCaseName);
                if (result2 >= 0) {
                    return result2;
                }
                --count;
        ...
        if (choice == 2) {
            result2 = this.getGroupChar(upperCaseName, 0);
            if (result2 == -1) {
                result2 = this.getGroupChar(upperCaseName, 3);
            }
        } else {
            result2 = this.getGroupChar(upperCaseName, choice);
        }
        return result2;
    }
    ...
    public String getExtendedName(int ch) {
        String result2 = this.getName(ch, 0);
        if (result2 == null && result2 == null) {
            result2 = this.getExtendedOr10Name(ch);
        }
        return result2;
    }
    ...
    public int getGroup(int codepoint) {
        ...
        int result2 = 0;
        while (result2 < endGroup - 1) {
            int gindex = result2 + endGroup >> 1;
    ...
    public String getExtendedOr10Name(int ch) {
        ...
        if (result2 == null) {
            ...
            result2 = type2 >= TYPE_NAMES_.length ? UNKNOWN_TYPE_NAME_ : TYPE_NAMES_[type2];
            StringBuffer stringBuffer = this.m_utilStringBuffer_;
            synchronized (stringBuffer) {
                this.m_utilStringBuffer_.delete(0, this.m_utilStringBuffer_.length());
                this.m_utilStringBuffer_.append('<');
                this.m_utilStringBuffer_.append(result2);
                this.m_utilStringBuffer_.append('-');
    ...
    private synchronized int getGroupChar(String name, int choice) {
        for (int i = 0; i < this.m_groupcount_; ++i) {
            ...
            int result2 = this.getGroupChar(startgpstrindex, this.m_grouplengths_, name, choice);
            if (result2 == -1) continue;
            return this.m_groupinfo_[i * this.m_groupsize_] << 5 | result2;
        }
        return -1;
    }
    ...
    private int getGroupChar(int index, char[] length, String name, int choice) {
        ...
        for (int result2 = 0; result2 <= 32; ++result2) {
            int nindex = 0;
            int len = length[result2];
            if (choice != 0 && choice != 2) {
                int fieldIndex = choice == 4 ? 2 : choice;
                do {
                    int oldindex = index;
    ...
    private static int getType(int ch) {
        ...
        int result2 = UCharacter.getType(ch);
        if (result2 == 18) {
            result2 = ch <= 56319 ? 31 : 32;
        }
        return result2;
    }
    ...
    private static int getExtendedChar(String name, int choice) {
        if (name.charAt(0) == '<') {
            ...
            if (choice == 2 && name.charAt(endIndex = name.length() - 1) == '>' && (startIndex = name.lastIndexOf(45)) >= 0) {
                ...
                int result2 = -1;
                try {
                    result2 = Integer.parseInt(name.substring(startIndex, endIndex), 16);
                }
                ...
                for (int i = 0; i < length; ++i) {
                    ...
                    if (UCharacterName.getType(result2) != i) break;
                    return result2;
                }
            }
            return -1;
    ...
    private int addAlgorithmName(int maxlength) {
        ...
        for (int i = this.m_algorithm_.length - 1; i >= 0; --i) {
            result2 = this.m_algorithm_[i].add(this.m_nameSet_, maxlength);
            if (result2 <= maxlength) continue;
            maxlength = result2;
        }
        return maxlength;
    }
    ...
    static final class AlgorithmName {
        ...
        int getChar(String name) {
            ...
            switch (this.m_type_) {
                case 0: {
                    try {
                        int result2 = Integer.parseInt(name.substring(prefixlen), 16);
                        if (this.m_rangestart_ > result2) return -1;
                        if (result2 > this.m_rangeend_) return -1;
                        return result2;
                    }
                    catch (NumberFormatException e) {
                        return -1;
