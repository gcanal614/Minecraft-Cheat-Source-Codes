...
implements Serializable {
    ...
    public final List<String> split(@NotNull CharSequence input, int limit) {
        ...
        ArrayList<String> result2 = new ArrayList<String>(limit > 0 ? RangesKt.coerceAtMost(limit, 10) : 10);
        int lastStart = 0;
        ...
        do {
            ...
            n = matcher.start();
            bl = false;
            result2.add(((Object)charSequence.subSequence(lastStart, n)).toString());
            lastStart = matcher.end();
        } while ((lastSplit < 0 || result2.size() != lastSplit) && matcher.find());
        charSequence = input;
        n = input.length();
        bl = false;
        result2.add(((Object)charSequence.subSequence(lastStart, n)).toString());
