...
public abstract class PrimitiveSpreadBuilder<T> {
     ...
     */
    @NotNull
    protected final T toArray(@NotNull T values2, @NotNull T result2) {
        ...
        if (n <= n2) {
            while (true) {
                ...
                if ((spreadArgument = this.spreads[i]) != null) {
                    if (copyValuesFrom < i) {
                        System.arraycopy(values2, copyValuesFrom, result2, dstIndex, (int)(i - copyValuesFrom));
                        dstIndex += i - copyValuesFrom;
                    }
                    int spreadSize = this.getSize(spreadArgument);
                    System.arraycopy(spreadArgument, 0, result2, dstIndex, spreadSize);
                    dstIndex += spreadSize;
        ...
        if (copyValuesFrom < this.size) {
            System.arraycopy(values2, copyValuesFrom, result2, dstIndex, this.size - copyValuesFrom);
        }
        return result2;
    }
