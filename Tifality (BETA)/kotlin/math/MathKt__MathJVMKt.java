...
extends MathKt__MathHKt {
    ...
    public static final double acosh(double x) {
        ...
        if (x < 1.0) {
            ...
            double y;
            double result2 = y = Math.sqrt(x - 1.0);
            if (y >= Constants.taylor_2_bound) {
                result2 -= y * y * y / (double)12;
            }
            d = Math.sqrt(2.0) * result2;
        }
        return d;
    }
    ...
    public static final float truncate(float x) {
        ...
        block5: {
            block4: {
                float f3 = x;
                boolean bl = false;
                if (Float.isNaN(f3)) break block4;
                f3 = x;
                bl = false;
                if (!Float.isInfinite(f3)) break block5;
            }
            f2 = x;
            return f2;
        }
        if (x > 0.0f) {
            boolean bl = false;
            f2 = (float)Math.floor(x);
            return f2;
        }
        boolean bl = false;
        f2 = (float)Math.ceil(x);
        return f2;
    }
