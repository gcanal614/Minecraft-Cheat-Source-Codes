...
implements PropertyComponent {
    ...
    public void drawComponent(ScaledResolution scaledResolution, int mouseX, int mouseY) {
        ...
        float valueWidth = Wrapper.getFontRenderer().getWidth(valueString) + 2.0f;
        float overflowWidth = Wrapper.getFontRenderer().getWidth(name) + 3.0f - ((float)width - valueWidth);
        ...
        Gui.drawRect((float)x - (showOverflowBox ? overflowWidth : 0.0f), y, x + width, y + height, this.getSecondaryBackgroundColor(hovered));
        Gui.drawRect((double)x, (double)y, (double)x + (double)width * sliderPercentage, (double)(y + height), SLIDER_COLOR);
        if (needScissorBox) {
            OGLUtils.startScissorBox(scaledResolution, x, y, (int)((float)width - valueWidth - 4.0f), height);
        }
        Wrapper.getFontRenderer().drawStringWithShadow(name, (float)(x + 2) - (showOverflowBox ? overflowWidth : 0.0f), y + middleHeight - 3, 0xFFFFFF);
        if (needScissorBox) {
            OGLUtils.endScissorBox();
        }
        Wrapper.getFontRenderer().drawStringWithShadow(valueString, (float)(x + width) - valueWidth, y + middleHeight - 3, -1);
    }
