    ...
    public static int hashCode(ByteBuf buffer) {
        ...
        if (buffer.order() == ByteOrder.BIG_ENDIAN) {
            for (i = intCount; i > 0; --i) {
                hashCode2 = 31 * hashCode2 + buffer.getInt(arrayIndex);
                arrayIndex += 4;
            }
        } else {
            for (i = intCount; i > 0; --i) {
                hashCode2 = 31 * hashCode2 + ByteBufUtil.swapInt(buffer.getInt(arrayIndex));
                arrayIndex += 4;
            }
        }
        for (i = byteCount; i > 0; --i) {
            hashCode2 = 31 * hashCode2 + buffer.getByte(arrayIndex++);
        }
        if (hashCode2 == 0) {
            hashCode2 = 1;
        }
