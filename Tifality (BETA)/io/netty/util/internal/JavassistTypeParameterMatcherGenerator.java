...
public final class JavassistTypeParameterMatcherGenerator {
    ...
    public static TypeParameterMatcher generate(Class<?> type2) {
        ClassLoader classLoader = PlatformDependent.getContextClassLoader();
        if (classLoader == null) {
            classLoader = PlatformDependent.getSystemClassLoader();
        }
        return JavassistTypeParameterMatcherGenerator.generate(type2, classLoader);
    }
     ...
     */
    public static TypeParameterMatcher generate(Class<?> type2, ClassLoader classLoader) {
        String typeName = JavassistTypeParameterMatcherGenerator.typeName(type2);
    ...
    private static String typeName(Class<?> type2) {
        if (type2.isArray()) {
            return JavassistTypeParameterMatcherGenerator.typeName(type2.getComponentType()) + "[]";
        }
