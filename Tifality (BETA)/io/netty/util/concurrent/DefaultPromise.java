...
implements Promise<V> {
    ...
    public boolean isSuccess() {
        ...
        if (result2 == null || result2 == UNCANCELLABLE) {
            return false;
        }
        return !(result2 instanceof CauseHolder);
    }
    ...
    public boolean cancel(boolean mayInterruptIfRunning) {
        Object result2 = this.result;
        if (DefaultPromise.isDone0(result2) || result2 == UNCANCELLABLE) {
            return false;
        }
        DefaultPromise defaultPromise = this;
        synchronized (defaultPromise) {
    ...
    public boolean setUncancellable() {
        Object result2 = this.result;
        if (DefaultPromise.isDone0(result2)) {
            return !DefaultPromise.isCancelled0(result2);
        }
        DefaultPromise defaultPromise = this;
        synchronized (defaultPromise) {
            result2 = this.result;
    ...
    private boolean setSuccess0(V result2) {
        ...
        synchronized (defaultPromise) {
            ...
            this.result = result2 == null ? SUCCESS : result2;
            if (this.hasWaiters()) {
                this.notifyAll();
            }
        }
    ...
    private static void notifyListeners0(Future<?> future, DefaultFutureListeners listeners) {
        ...
        for (int i = 0; i < size; ++i) {
            DefaultPromise.notifyListener0(future, a2[i]);
        }
    }
