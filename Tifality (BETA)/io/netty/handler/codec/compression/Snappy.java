...
class Snappy {
    ...
    public void encode(ByteBuf in, ByteBuf out, int length) {
        ...
        int i = 0;
        while (true) {
            if (((b2 = length >>> i * 7) & 0xFFFFFF80) == 0) break;
    ...
    public static int calculateChecksum(ByteBuf data2) {
     ...
     */
    public static int calculateChecksum(ByteBuf data2, int offset, int length) {
        Crc32c crc32 = new Crc32c();
        try {
            if (data2.hasArray()) {
                crc32.update(data2.array(), data2.arrayOffset() + offset, length);
            } else {
                byte[] array = new byte[length];
                data2.getBytes(offset, array);
    ...
    static void validateChecksum(int expectedChecksum, ByteBuf data2, int offset, int length) {
        int actualChecksum = Snappy.calculateChecksum(data2, offset, length);
