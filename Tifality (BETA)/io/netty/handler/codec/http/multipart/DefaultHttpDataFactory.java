...
implements HttpDataFactory {
    ...
    @Override
    public void removeHttpDataFromClean(HttpRequest request, InterfaceHttpData data2) {
        if (data2 instanceof HttpData) {
            List<HttpData> fileToDelete = this.getList(request);
            fileToDelete.remove(data2);
        }
    }
    ...
    public void cleanRequestHttpDatas(HttpRequest request) {
        ...
        if (fileToDelete != null) {
            for (HttpData data2 : fileToDelete) {
                data2.delete();
            }
            fileToDelete.clear();
        }
