...
public class HttpPostRequestDecoder {
    ...
    protected void addHttpData(InterfaceHttpData data2) {
        if (data2 == null) {
            return;
        }
        ...
        while (!this.skipOneLine()) {
            ...
            if (contents[0].equalsIgnoreCase("Content-Disposition")) {
                ...
                for (int i = 2; i < contents.length; ++i) {
                    Attribute attribute;
                    String[] values2 = StringUtil.split(contents[i], '=');
                    try {
                        String name = HttpPostRequestDecoder.cleanString(values2[0]);
            ...
            if (contents[0].equalsIgnoreCase("Content-Type")) {
                if (contents[1].equalsIgnoreCase("multipart/mixed")) {
                    if (this.currentStatus == MultiPartStatus.DISPOSITION) {
                        String[] values3 = StringUtil.split(contents[2], '=');
                        this.multipartMixedBoundary = "--" + values3[1];
                ...
                for (int i = 1; i < contents.length; ++i) {
                    ...
                    if (contents[i].toLowerCase().startsWith("charset")) {
                        ...
                        String[] values4 = StringUtil.split(contents[i], '=');
                        try {
                            attribute2 = this.factory.createAttribute(this.request, "charset", HttpPostRequestDecoder.cleanString(values4[1]));
                        }
                        catch (NullPointerException e) {
                            throw new ErrorDataDecoderException(e);
                        }
    ...
    private static String[] splitMultipartHeader(String sb) {
        ...
        String[] values2 = svalue.indexOf(59) >= 0 ? StringUtil.split(svalue, ';') : StringUtil.split(svalue, ',');
