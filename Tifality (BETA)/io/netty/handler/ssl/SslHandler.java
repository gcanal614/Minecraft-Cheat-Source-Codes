...
implements ChannelOutboundHandler {
        ...
        block19: {
            ...
            try {
                block14: while (true) {
                    ...
                    SSLEngineResult result2 = this.wrap(this.engine, buf, out);
                    promise = !buf.isReadable() ? this.pendingUnencryptedWrites.remove() : null;
                    if (result2.getStatus() == SSLEngineResult.Status.CLOSED) {
    ...
    private void wrapNonAppData(ChannelHandlerContext ctx, boolean inUnwrap) throws SSLException {
        ...
        try {
            ...
            block12: do {
                ...
                if ((result2 = this.wrap(this.engine, Unpooled.EMPTY_BUFFER, (ByteBuf)out)).bytesProduced() > 0) {
                    ctx.write(out);
                    if (inUnwrap) {
                        this.needsFlush = true;
                    }
    ...
    private void unwrap(ChannelHandlerContext ctx, ByteBuffer packet, int initialOutAppBufCapacity) throws SSLException {
        ...
        try {
            block12: while (true) {
                SSLEngineResult result2 = SslHandler.unwrap(this.engine, packet, decodeOut);
