...
implements SctpChannel {
    ...
    protected int doReadMessages(List<Object> buf) throws Exception {
        ...
        boolean free = true;
        try {
            ByteBuffer data2 = buffer.internalNioBuffer(buffer.writerIndex(), buffer.writableBytes());
            int pos = data2.position();
            MessageInfo messageInfo = ch.receive(data2, null, this.notificationHandler);
            if (messageInfo == null) {
                int n = 0;
                return n;
            }
            buf.add(new SctpMessage(messageInfo, buffer.writerIndex(buffer.writerIndex() + data2.position() - pos)));
            free = false;
            int n = 1;
            return n;
        }
    ...
    protected boolean doWriteMessage(Object msg, ChannelOutboundBuffer in) throws Exception {
        ...
        ByteBuf data2 = packet.content();
        int dataLen = data2.readableBytes();
        if (dataLen == 0) {
            return true;
        }
        ByteBufAllocator alloc = this.alloc();
        boolean bl = needsCopy = data2.nioBufferCount() != 1;
        if (!needsCopy && !data2.isDirect() && alloc.isDirectBufferPooled()) {
            needsCopy = true;
        }
        if (!needsCopy) {
            nioData = data2.nioBuffer();
        } else {
            data2 = alloc.directBuffer(dataLen).writeBytes(data2);
            nioData = data2.nioBuffer();
        }
