    ...
    protected int doReadMessages(List<Object> msgs) throws Exception {
        ...
        try {
            for (SelectionKey ignored : reableKeys) {
                ...
                boolean free = true;
                try {
                    ByteBuffer data2 = buffer.nioBuffer(buffer.writerIndex(), buffer.writableBytes());
                    MessageInfo messageInfo = this.ch.receive(data2, null, this.notificationHandler);
                    if (messageInfo == null) {
                        int n = readMessages;
                        return n;
                    }
                    data2.flip();
                    msgs.add(new SctpMessage(messageInfo, buffer.writerIndex(buffer.writerIndex() + data2.remaining())));
                    free = false;
                    ++readMessages;
                }
    ...
    protected void doWrite(ChannelOutboundBuffer in) throws Exception {
        ...
        if (selectedKeys > 0) {
            ...
            do {
                ...
                if (data2.nioBufferCount() != -1) {
                    nioData = data2.nioBuffer();
                } else {
                    nioData = ByteBuffer.allocate(dataLen);
                    data2.getBytes(data2.readerIndex(), nioData);
                    nioData.flip();
                }
