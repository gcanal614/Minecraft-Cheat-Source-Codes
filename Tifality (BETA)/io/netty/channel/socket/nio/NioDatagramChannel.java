...
implements DatagramChannel {
    ...
    protected int doReadMessages(List<Object> buf) throws Exception {
        ...
        ByteBuf data2 = allocHandle.allocate(config.getAllocator());
        boolean free = true;
        try {
            ByteBuffer nioData = data2.internalNioBuffer(data2.writerIndex(), data2.writableBytes());
            ...
            data2.writerIndex(data2.writerIndex() + readBytes);
            allocHandle.record(readBytes);
            buf.add(new DatagramPacket(data2, this.localAddress(), remoteAddress));
            free = false;
            int n = 1;
            return n;
        }
    ...
    protected boolean doWriteMessage(Object msg, ChannelOutboundBuffer in) throws Exception {
        ...
        ByteBuffer nioData = data2.internalNioBuffer(data2.readerIndex(), dataLen);
    ...
    public ChannelFuture joinGroup(InetAddress multicastAddress, NetworkInterface networkInterface, InetAddress source, ChannelPromise promise) {
        ...
        try {
            ...
            synchronized (nioDatagramChannel) {
                List<MembershipKey> keys2 = null;
                if (this.memberships == null) {
                    this.memberships = new HashMap<InetAddress, List<MembershipKey>>();
                } else {
                    keys2 = this.memberships.get(multicastAddress);
                }
                if (keys2 == null) {
