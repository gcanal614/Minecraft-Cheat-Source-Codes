...
extends MethodVisitor {
    ...
    public void visitFrame(int type2, int nLocal, Object[] local, int nStack, Object[] stack) {
        ...
        switch (type2) {
            ...
            default: {
                throw new IllegalArgumentException("Invalid frame type " + type2);
            }
        }
        if (nLocal > mLocal) {
            throw new IllegalArgumentException("Invalid nLocal=" + nLocal + " for frame type " + type2);
        }
        if (nStack > mStack) {
            throw new IllegalArgumentException("Invalid nStack=" + nStack + " for frame type " + type2);
        }
        if (type2 != 2) {
    ...
    public void visitTypeInsn(int opcode, String type2) {
        ...
        CheckMethodAdapter.checkInternalName(type2, "type");
        if (opcode == 187 && type2.charAt(0) == '[') {
            throw new IllegalArgumentException("NEW cannot be used to create arrays: " + type2);
        }
    ...
    public void visitLocalVariable(String name, String desc, String signature2, Label start, Label end, int index) {
        ...
        super.visitLocalVariable(name, desc, signature2, start, end, index);
    }
    ...
    static void checkOpcode(int opcode, int type2) {
        if (opcode < 0 || opcode > 199 || TYPE[opcode] != type2) {
