...
public abstract class AbstractInsnNode {
    ...
    static LabelNode clone(LabelNode label, Map<LabelNode, LabelNode> map2) {
        return map2.get(label);
    }

    static LabelNode[] clone(List<LabelNode> labels, Map<LabelNode, LabelNode> map2) {
        LabelNode[] clones = new LabelNode[labels.size()];
        for (int i = 0; i < clones.length; ++i) {
            clones[i] = map2.get(labels.get(i));
        }
        return clones;
    }
