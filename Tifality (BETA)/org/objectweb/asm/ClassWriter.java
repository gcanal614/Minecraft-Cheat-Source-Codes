...
extends ClassVisitor {
    ...
    public final void visit(int version, int access, String name, String signature2, String superName, String[] interfaces) {
        this.version = version;
        this.access = access;
        this.name = this.newClass(name);
        this.thisName = name;
        if (signature2 != null) {
            this.signature = this.newUTF8(signature2);
        }
    ...
    public final FieldVisitor visitField(int access, String name, String desc, String signature2, Object value) {
        return new FieldWriter(this, access, name, desc, signature2, value);
    }

    public final MethodVisitor visitMethod(int access, String name, String desc, String signature2, String[] exceptions) {
        return new MethodWriter(this, access, name, desc, signature2, exceptions, this.computeMaxs, this.computeFrames);
    }
    ...
    Item newMethodItem(String owner, String name, String desc, boolean itf) {
        int type2 = itf ? 11 : 10;
        this.key3.set(type2, owner, name, desc);
        Item result2 = this.get(this.key3);
        if (result2 == null) {
            this.put122(type2, this.newClass(owner), this.newNameType(name, desc));
            result2 = new Item(this.index++, this.key3);
            this.put(result2);
        }
        return result2;
    ...
    int addUninitializedType(String type2, int offset) {
        ...
        this.key.hashCode = Integer.MAX_VALUE & 31 + type2.hashCode() + offset;
        Item result2 = this.get(this.key);
        if (result2 == null) {
            result2 = this.addType(this.key);
        }
