...
final class Frame {
    ...
    private int get(int local) {
        ...
        int type2 = this.outputLocals[local];
        if (type2 == 0) {
            type2 = this.outputLocals[local] = 0x2000000 | local;
        }
        return type2;
    }

    private void set(int local, int type2) {
        int n;
        ...
        this.outputLocals[local] = type2;
    }

    private void push(int type2) {
        int n;
        ...
        this.outputStack[this.outputStackTop++] = type2;
    ...
    private static int type(ClassWriter cw, String desc) {
        ...
        switch (desc.charAt(dims)) {
            ...
            default: {
                ...
                data2 = 0x1700000 | cw.addType(t);
            }
        }
        return dims - index << 28 | data2;
    }
    ...
    private int init(ClassWriter cw, int t) {
        ...
        if (t == 0x1000006) {
            ...
            String type2 = cw.typeTable[t & 0xFFFFF].strVal1;
            s = 0x1700000 | cw.addType(type2);
        } else {
            return t;
        }
    ...
    void initInputFrame(ClassWriter cw, int access, Type[] args2, int maxLocals) {
        ...
        for (int j = 0; j < args2.length; ++j) {
            int t = Frame.type(cw, args2[j].getDescriptor());
