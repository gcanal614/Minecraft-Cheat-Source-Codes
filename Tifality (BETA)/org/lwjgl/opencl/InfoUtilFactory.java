...
final class InfoUtilFactory {
    ...
    implements CLContext.CLContextUtil {
        ...
        public CLContext create(CLPlatform platform, List<CLDevice> devices, CLContextCallback pfn_notify, Drawable share_drawable, IntBuffer errcode_ret) throws LWJGLException {
            ...
            PointerBuffer properties2 = APIUtil.getBufferPointer(propertyCount + devices.size());
            properties2.put(4228L).put(platform);
            if (share_drawable != null) {
                share_drawable.setCLSharingProperties(properties2);
            }
            properties2.put(0L);
            properties2.position(propertyCount);
            for (CLDevice device : devices) {
                properties2.put(device);
            }
            ...
            try {
                __result = new CLContext(CL10.nclCreateContext(MemoryUtil.getAddress0(properties2.getBuffer()), devices.size(), MemoryUtil.getAddress(properties2, propertyCount), pfn_notify == null ? 0L : pfn_notify.getPointer(), user_data, MemoryUtil.getAddressSafe(errcode_ret), function_pointer), platform);
                if (LWJGLUtil.DEBUG) {
                    Util.checkCLError(errcode_ret.get(0));
                }
        ...
        public CLContext createFromType(CLPlatform platform, long device_type, CLContextCallback pfn_notify, Drawable share_drawable, IntBuffer errcode_ret) throws LWJGLException {
            int propertyCount = 2 + (share_drawable == null ? 0 : 4) + 1;
            PointerBuffer properties2 = APIUtil.getBufferPointer(propertyCount);
            properties2.put(4228L).put(platform);
            if (share_drawable != null) {
                share_drawable.setCLSharingProperties(properties2);
            }
            properties2.put(0L);
            properties2.flip();
            return CL10.clCreateContextFromType(properties2, device_type, pfn_notify, errcode_ret);
        }
