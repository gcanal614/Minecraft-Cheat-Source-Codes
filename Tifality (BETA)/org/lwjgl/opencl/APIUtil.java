...
final class APIUtil {
    ...
    static CLPlatform getCLPlatform(PointerBuffer properties2) {
        long key;
        long platformID = 0L;
        int keys2 = properties2.remaining() / 2;
        for (int k = 0; k < keys2 && (key = properties2.get(k << 1)) != 0L; ++k) {
            if (key != 4228L) continue;
            platformID = properties2.get((k << 1) + 1);
            break;
        }
        if (platformID == 0L) {
    ...
    static ByteBuffer getNativeKernelArgs(long user_func_ref, CLMem[] clMems, long[] sizes) {
        ByteBuffer args2 = APIUtil.getBufferByte(12 + (clMems == null ? 0 : clMems.length * (4 + PointerBuffer.getPointerSize())));
        args2.putLong(0, user_func_ref);
        if (clMems == null) {
            args2.putInt(8, 0);
        } else {
            args2.putInt(8, clMems.length);
            int byteIndex = 12;
            for (int i = 0; i < clMems.length; ++i) {
                ...
                args2.putInt(byteIndex, (int)sizes[i]);
