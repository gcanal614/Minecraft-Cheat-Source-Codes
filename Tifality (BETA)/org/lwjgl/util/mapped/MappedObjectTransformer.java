...
public class MappedObjectTransformer {
    ...
    public static void register(Class<? extends MappedObject> type2) {
        ...
        for (int i$ = 0; i$ < len$; ++i$) {
            ...
            fields2.put(field.getName(), fieldInfo);
        ...
        if (className_to_subtype.put(className, mappedType = new MappedSubtypeInfo(className, fields2, (int)(sizeof += (long)padding), align, padding, cacheLinePadded)) != null) {
    ...
    static byte[] transformMappedObject(byte[] bytecode) {
        ...
        ClassAdapter cv = new ClassAdapter(cw){
            ...
            public MethodVisitor visitMethod(int access, String name, String desc, String signature2, String[] exceptions) {
                ...
                return super.visitMethod(access, name, desc, signature2, exceptions);
            }
        };
    ...
    static byte[] transformMappedAPI(String className, byte[] bytecode) {
        ...
        TransformationAdapter cv = ta = new TransformationAdapter(cw, className);
        if (className_to_subtype.containsKey(className)) {
            cv = MappedObjectTransformer.getMethodGenAdapter(className, (ClassVisitor)((Object)cv));
        }
        new ClassReader(bytecode).accept((ClassVisitor)((Object)cv), 4);
        if (!ta.transformed) {
            return bytecode;
        }
        ...
        switch (methodInsn.getOpcode()) {
            case 182: {
                ...
                if ("dup".equals(methodInsn.name) && methodInsn.desc.equals("()L" + MAPPED_OBJECT_JVM + ";")) {
                    i = MappedObjectTransformer.replace(instructions, i, methodInsn, MappedObjectTransformer.generateDupInstructions(methodInsn));
                    break;
                }
                if ("slice".equals(methodInsn.name) && methodInsn.desc.equals("()L" + MAPPED_OBJECT_JVM + ";")) {
                    i = MappedObjectTransformer.replace(instructions, i, methodInsn, MappedObjectTransformer.generateSliceInstructions(methodInsn));
                    break;
                }
                if ("runViewConstructor".equals(methodInsn.name) && "()V".equals(methodInsn.desc)) {
                    i = MappedObjectTransformer.replace(instructions, i, methodInsn, MappedObjectTransformer.generateRunViewConstructorInstructions(methodInsn));
                    break;
                }
                if ("copyTo".equals(methodInsn.name) && methodInsn.desc.equals("(L" + MAPPED_OBJECT_JVM + ";)V")) {
                    i = MappedObjectTransformer.replace(instructions, i, methodInsn, MappedObjectTransformer.generateCopyToInstructions(mappedType));
                    break;
                }
                if (!"copyRange".equals(methodInsn.name) || !methodInsn.desc.equals("(L" + MAPPED_OBJECT_JVM + ";I)V")) break;
                i = MappedObjectTransformer.replace(instructions, i, methodInsn, MappedObjectTransformer.generateCopyRangeInstructions(mappedType));
                break;
            }
            case 183: {
            ...
            case 184: {
                ...
                i = MappedObjectTransformer.replace(instructions, i, methodInsn, MappedObjectTransformer.generateMapInstructions(mappedType, methodInsn.owner, isMapDirectMethod, isMallocMethod));
            }
        }
        return i;
    }
    ...
    private static InsnList generateMapInstructions(MappedSubtypeInfo mappedType, String className, boolean mapDirectMethod, boolean mallocMethod) {
        ...
        if (mallocMethod) {
            ...
            trg.add(new MethodInsnNode(184, mappedType.cacheLinePadded ? MappedObjectTransformer.jvmClassName(CacheUtil.class) : MappedObjectTransformer.jvmClassName(BufferUtils.class), "createByteBuffer", "(I)L" + MappedObjectTransformer.jvmClassName(ByteBuffer.class) + ";"));
        } else if (mapDirectMethod) {
            trg.add(new MethodInsnNode(184, MAPPED_HELPER_JVM, "newBuffer", "(JI)L" + MappedObjectTransformer.jvmClassName(ByteBuffer.class) + ";"));
        }
    ...
    private static InsnList generateByteBufferInstructions(FieldInsnNode fieldInsn, MappedSubtypeInfo mappedSubtype, long fieldOffset) {
        ...
        if (fieldInsn.getOpcode() == 180) {
            ...
            list.add(new LdcInsnNode(mappedSubtype.fields.get((Object)fieldInsn.name).length));
            list.add(new InsnNode(136));
            list.add(new MethodInsnNode(184, MAPPED_HELPER_JVM, "newBuffer", "(JI)L" + MappedObjectTransformer.jvmClassName(ByteBuffer.class) + ";"));
            return list;
        }
        throw new InternalError();
    }
    ...
    static int transformArrayAccess(InsnList instructions, int i, Map<AbstractInsnNode, Frame<BasicValue>> frameMap, VarInsnNode loadInsn, MappedSubtypeInfo mappedSubtype, int var) {
        ...
        while (true) {
            ...
            if (stackSize == loadStackSize + 1 && nextInsn.getOpcode() == 50) {
                ...
                while ((nextInsn = nextInsn.getNext()) != null) {
                    ...
                    if (stackSize == loadStackSize + 1 && nextInsn.getOpcode() == 181) {
                        ...
                        instructions.insert(nextInsn, new MethodInsnNode(184, mappedSubtype.className, MappedObjectTransformer.setterName(fieldInsn.name), "(L" + mappedSubtype.className + ";I" + fieldInsn.desc + ")V"));
                        instructions.remove(nextInsn);
                        break;
                    }
                    if (stackSize == loadStackSize && nextInsn.getOpcode() == 180) {
                        fieldInsn = (FieldInsnNode)nextInsn;
                        instructions.insert(nextInsn, new MethodInsnNode(184, mappedSubtype.className, MappedObjectTransformer.getterName(fieldInsn.name), "(L" + mappedSubtype.className + ";I)" + fieldInsn.desc));
                        instructions.remove(nextInsn);
                        break;
                    }
                    ...
                    throw new ClassFormatError("Invalid " + mappedSubtype.className + " view array usage detected: " + MappedObjectTransformer.getOpcodeName(nextInsn));
                }
            ...
            if (stackSize == loadStackSize && nextInsn.getOpcode() == 190) {
                ...
                instructions.remove(nextInsn);
                loadInsn.var = var;
                instructions.insert((AbstractInsnNode)loadInsn, new MethodInsnNode(182, mappedSubtype.className, CAPACITY_METHOD_NAME, "()I"));
                return i + 1;
            }
    ...
    private static void getClassEnums(Class clazz, Map<Integer, String> map2, String ... prefixFilters) {
        try {
            block2: for (Field field : clazz.getFields()) {
                ...
                if (map2.put((Integer)field.get(null), field.getName()) == null) continue;
                throw new IllegalStateException();
            }
        }
        catch (Exception e) {
    ...
    private static void printBytecode(byte[] bytecode) {
        ...
        TraceClassVisitor tracer = new TraceClassVisitor(new ClassWriter(0), new PrintWriter(sw));
    ...
    private static class MappedSubtypeInfo {
        ...
        MappedSubtypeInfo(String className, Map<String, FieldInfo> fields2, int sizeof, int align, int padding, boolean cacheLinePadded) {
    ...
    extends ClassAdapter {
        ...
        public FieldVisitor visitField(int access, String name, String desc, String signature2, Object value) {
            ...
            if ((access & 8) == 0) {
                return new FieldNode(access, name, desc, signature2, value){
        ...
        public MethodVisitor visitMethod(int access, String name, String desc, String signature2, String[] exceptions) {
            ...
            if ("<init>".equals(name) && (mappedSubtype = className_to_subtype.get(this.className)) != null) {
                ...
                MethodVisitor mv = super.visitMethod(access, name, desc, signature2, exceptions);
                mv.visitVarInsn(25, 0);
            ...
            final MethodVisitor mv = super.visitMethod(access, name, desc, signature2, exceptions);
            return new MethodNode(access, name, desc, signature2, exceptions){
