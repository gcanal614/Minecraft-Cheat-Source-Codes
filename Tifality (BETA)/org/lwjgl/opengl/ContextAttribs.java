...
public final class ContextAttribs {
    ...
    IntBuffer getAttribList() {
        ...
        LinkedHashMap<Integer, Integer> map2 = new LinkedHashMap<Integer, Integer>(8);
        if (this.majorVersion != 1 || this.minorVersion != 0) {
            map2.put(8337, this.majorVersion);
            map2.put(8338, this.minorVersion);
        }
        if (this.contextFlags != 0) {
            map2.put(8340, this.contextFlags);
        }
        if (this.profileMask != 0) {
            map2.put(37158, this.profileMask);
        }
        if (this.contextResetNotificationStrategy != 33377) {
            map2.put(33366, this.contextResetNotificationStrategy);
        }
        if (this.contextReleaseBehavior != 8344) {
            map2.put(8343, this.contextReleaseBehavior);
        }
        if (this.layerPlane != 0) {
            map2.put(8339, this.layerPlane);
        }
        if (map2.isEmpty()) {
            return null;
        }
        IntBuffer attribs = BufferUtils.createIntBuffer(map2.size() * 2 + 1);
        for (Map.Entry attrib : map2.entrySet()) {
