...
implements DisplayImplementation {
    ...
    private void grabPointer() {
        int result2;
        if (!this.pointer_grabbed && (result2 = LinuxDisplay.nGrabPointer(LinuxDisplay.getDisplay(), LinuxDisplay.getWindow(), 0L)) == 0) {
    ...
    public boolean isCloseRequested() {
        boolean result2 = this.close_requested;
        this.close_requested = false;
        return result2;
    }
        ...
        for (ByteBuffer icon : icons) {
            ...
            for (int y = 0; y < dimension; ++y) {
                for (int x = 0; x < dimension; ++x) {
                    ...
                    byte b2 = icon.get(x * 4 + y * dimension * 4 + 2);
                    byte a2 = icon.get(x * 4 + y * dimension * 4 + 3);
                    icon_argb.put(a2);
                    icon_argb.put(r);
                    icon_argb.put(g);
                    icon_argb.put(b2);
