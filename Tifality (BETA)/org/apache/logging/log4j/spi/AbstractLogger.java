...
implements Logger {
    ...
    @Override
    public <R> R exit(R result2) {
        return this.exit(FQCN, result2);
    }

    protected <R> R exit(String fqcn, R result2) {
        if (this.isEnabled(Level.TRACE, EXIT_MARKER, (Object)null, null)) {
            this.log(EXIT_MARKER, fqcn, Level.TRACE, this.toExitMsg(result2), null);
        }
        return result2;
    }
    ...
    private Message toExitMsg(Object result2) {
        if (result2 == null) {
            return this.messageFactory.newMessage("exit");
        }
        return this.messageFactory.newMessage("exit with(" + result2 + ")");
    }
