...
implements Message {
    ...
    public FormattedMessage(String messagePattern, Object[] arguments2, Throwable throwable) {
        this.messagePattern = messagePattern;
        this.argArray = arguments2;
        this.throwable = throwable;
    }

    public FormattedMessage(String messagePattern, Object[] arguments2) {
        this.messagePattern = messagePattern;
        this.argArray = arguments2;
        this.throwable = null;
    }
    ...
    protected Message getMessage(String msgPattern, Object[] args2, Throwable throwable) {
        try {
            MessageFormat format = new MessageFormat(msgPattern);
            Format[] formats = format.getFormats();
            if (formats != null && formats.length > 0) {
                return new MessageFormatMessage(msgPattern, args2);
            }
        }
        catch (Exception ex) {
            // empty catch block
        ...
        try {
            if (MSG_PATTERN.matcher(msgPattern).find()) {
                return new StringFormattedMessage(msgPattern, args2);
            }
        }
        catch (Exception ex) {
            // empty catch block
        }
        return new ParameterizedMessage(msgPattern, args2, throwable);
    }
