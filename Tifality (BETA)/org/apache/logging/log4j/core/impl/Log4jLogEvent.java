...
implements LogEvent {
    ...
    public Log4jLogEvent(String loggerName, Marker marker, String fqcn, Level level, Message message, List<Property> properties2, Throwable t) {
        this(loggerName, marker, fqcn, level, message, t, Log4jLogEvent.createMap(properties2), ThreadContext.getDepth() == 0 ? null : ThreadContext.cloneStack(), null, null, System.currentTimeMillis());
    }
    ...
    private static Map<String, String> createMap(List<Property> properties2) {
        Map<String, String> contextMap = ThreadContext.getImmutableContext();
        if (contextMap == null && (properties2 == null || properties2.size() == 0)) {
            return null;
        }
        if (properties2 == null || properties2.size() == 0) {
            return contextMap;
        }
        HashMap<String, String> map2 = new HashMap<String, String>(contextMap);
        for (Property prop : properties2) {
            if (map2.containsKey(prop.getName())) continue;
    ...
    public static Log4jLogEvent deserialize(Serializable event) {
        ...
        if (event instanceof LogEventProxy) {
            ...
            Log4jLogEvent result2 = new Log4jLogEvent(proxy.name, proxy.marker, proxy.fqcnOfLogger, proxy.level, proxy.message, proxy.throwable, (Map<String, String>)proxy.mdc, proxy.ndc, proxy.threadName, proxy.location, proxy.timestamp);
            result2.setEndOfBatch(proxy.isEndOfBatch);
            result2.setIncludeLocation(proxy.isLocationRequired);
            return result2;
        }
    ...
    implements Serializable {
        ...
        protected Object readResolve() {
            Log4jLogEvent result2 = new Log4jLogEvent(this.name, this.marker, this.fqcnOfLogger, this.level, this.message, this.throwable, this.mdc, this.ndc, this.threadName, this.location, this.timestamp);
            result2.setEndOfBatch(this.isEndOfBatch);
            result2.setIncludeLocation(this.isLocationRequired);
            return result2;
        }
    }
}
