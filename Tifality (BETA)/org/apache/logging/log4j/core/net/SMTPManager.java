...
extends AbstractManager {
    ...
    protected SMTPManager(String name, Session session, MimeMessage message, FactoryData data2) {
        super(name);
    ...
    implements ManagerFactory<SMTPManager, FactoryData> {
        ...
        public SMTPManager createManager(String name, FactoryData data2) {
            ...
            properties2.put("mail.transport.protocol", data2.protocol);
            if (properties2.getProperty("mail.host") == null) {
                properties2.put("mail.host", NetUtils.getLocalHostname());
            }
            if (null != data2.host) {
                properties2.put(prefix + ".host", data2.host);
            }
            if (data2.port > 0) {
                properties2.put(prefix + ".port", String.valueOf(data2.port));
            }
            if (null != (authenticator = this.buildAuthenticator(data2.username, data2.password))) {
                properties2.put(prefix + ".auth", "true");
            }
            Session session = Session.getInstance((Properties)properties2, (Authenticator)authenticator);
            session.setProtocolForAddress("rfc822", data2.protocol);
            session.setDebug(data2.isDebug);
            try {
                message = new MimeMessageBuilder(session).setFrom(data2.from).setReplyTo(data2.replyto).setRecipients(Message.RecipientType.TO, data2.to).setRecipients(Message.RecipientType.CC, data2.cc).setRecipients(Message.RecipientType.BCC, data2.bcc).setSubject(data2.subject).getMimeMessage();
            }
            catch (MessagingException e) {
                LOGGER.error("Could not set SMTPAppender message options.", (Throwable)e);
                message = null;
            }
            return new SMTPManager(name, session, message, data2);
        }
