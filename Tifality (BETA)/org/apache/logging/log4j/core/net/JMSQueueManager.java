...
extends AbstractJMSManager {
    ...
    implements ManagerFactory<JMSQueueManager, FactoryData> {
        ...
        @Override
        public JMSQueueManager createManager(String name, FactoryData data2) {
            try {
                Context ctx = AbstractJMSManager.createContext(data2.factoryName, data2.providerURL, data2.urlPkgPrefixes, data2.securityPrincipalName, data2.securityCredentials);
                QueueInfo info = JMSQueueManager.connect(ctx, data2.factoryBindingName, data2.queueBindingName, data2.userName, data2.password, true);
                return new JMSQueueManager(name, ctx, data2.factoryBindingName, data2.queueBindingName, data2.userName, data2.password, info);
            }
            catch (NamingException ex) {
