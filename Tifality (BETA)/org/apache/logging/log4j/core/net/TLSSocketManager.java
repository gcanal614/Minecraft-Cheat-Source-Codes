...
extends TCPSocketManager {
    ...
    implements ManagerFactory<TLSSocketManager, TLSFactoryData> {
        ...
        @Override
        public TLSSocketManager createManager(String name, TLSFactoryData data2) {
            InetAddress address = null;
            OutputStream os = null;
            Socket socket = null;
            try {
                address = this.resolveAddress(data2.host);
                socket = this.createSocket(data2);
                os = socket.getOutputStream();
                this.checkDelay(data2.delay, os);
            }
            catch (IOException e) {
            ...
            return this.createManager(name, os, socket, data2.sslConfig, address, data2.host, data2.port, data2.delay, data2.immediateFail, data2.layout);
        }
        ...
        private Socket createSocket(TLSFactoryData data2) throws IOException {
            SSLSocketFactory socketFactory = TLSSocketManager.createSSLSocketFactory(data2.sslConfig);
            SSLSocket socket = (SSLSocket)socketFactory.createSocket(data2.host, data2.port);
            return socket;
        }
