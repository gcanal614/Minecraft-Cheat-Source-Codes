...
extends Logger {
    ...
    private static ExceptionHandler getExceptionHandler() {
        ...
        try {
            ...
            ExceptionHandler result2 = (ExceptionHandler)klass.newInstance();
            LOGGER.debug("AsyncLogger.ExceptionHandler=" + result2);
            return result2;
        }
    ...
    public void log(Marker marker, String fqcn, Level level, Message data2, Throwable t) {
        ...
        info.translator.setValues(this, this.getName(), marker, fqcn, level, data2, t, ThreadContext.getImmutableContext(), ThreadContext.getImmutableStack(), info.cachedThreadName, includeLocation ? this.location(fqcn) : null, clock.currentTimeMillis());
    ...
    public void actualAsyncLog(RingBufferLogEvent event) {
        Map<Property, Boolean> properties2 = this.config.loggerConfig.getProperties();
        event.mergePropertiesIntoContextMap(properties2, this.config.config.getStrSubstitutor());
