...
implements NoSQLConnection<BasicDBObject, MongoDBObject> {
    ...
    public void insertObject(NoSQLObject<BasicDBObject> object) {
        try {
            WriteResult result2 = this.collection.insert((DBObject)object.unwrap(), this.writeConcern);
            if (result2.getError() != null && result2.getError().length() > 0) {
                throw new AppenderLoggingException("Failed to write log event to MongoDB due to error: " + result2.getError() + ".");
            }
        }
