...
implements NoSQLProvider<CouchDBConnection> {
    ...
    private CouchDBProvider(CouchDbClient client, String description2) {
        this.client = client;
        this.description = "couchDb{ " + description2 + " }";
    }
        ...
        if (factoryClassName != null && factoryClassName.length() > 0 && factoryMethodName != null && factoryMethodName.length() > 0) {
            try {
                ...
                if (object instanceof CouchDbClient) {
                    ...
                    description2 = "uri=" + client.getDBUri();
                }
                if (object instanceof CouchDbProperties) {
                    CouchDbProperties properties2 = (CouchDbProperties)object;
                    client = new CouchDbClient(properties2);
                    description2 = "uri=" + client.getDBUri() + ", username=" + properties2.getUsername() + ", passwordHash=" + NameUtil.md5(password + CouchDBProvider.class.getName()) + ", maxConnections=" + properties2.getMaxConnections() + ", connectionTimeout=" + properties2.getConnectionTimeout() + ", socketTimeout=" + properties2.getSocketTimeout();
                }
                if (object == null) {
            ...
            description2 = "uri=" + client.getDBUri() + ", username=" + username + ", passwordHash=" + NameUtil.md5(password + CouchDBProvider.class.getName());
        } else {
            LOGGER.error("No factory method was provided so the database name is required.");
            return null;
        }
        return new CouchDBProvider(client, description2);
    }
}
