...
extends AbstractAppender {
    ...
    @Override
    public void start() {
        Map<String, Appender> map2 = this.config.getAppenders();
        ArrayList<AppenderControl> appenders = new ArrayList<AppenderControl>();
        for (AppenderRef appenderRef : this.appenderRefs) {
            if (map2.containsKey(appenderRef.getRef())) {
                appenders.add(new AppenderControl(map2.get(appenderRef.getRef()), appenderRef.getLevel(), appenderRef.getFilter()));
                continue;
            }
            LOGGER.error("No appender named {} was configured", appenderRef);
        }
        if (this.errorRef != null) {
            if (map2.containsKey(this.errorRef)) {
                this.errorAppender = new AppenderControl(map2.get(this.errorRef), null, null);
            } else {
    ...
    extends Thread {
        ...
        public void run() {
            ...
            while (!this.queue.isEmpty()) {
                try {
                    s = this.queue.take();
                    if (!(s instanceof Log4jLogEvent)) continue;
                    event = Log4jLogEvent.deserialize(s);
                    event.setEndOfBatch(this.queue.isEmpty());
                    for (AppenderControl control : this.appenders) {
                        control.callAppender(event);
