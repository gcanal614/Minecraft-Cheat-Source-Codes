...
extends MapFilter {
    private StructuredDataFilter(Map<String, List<String>> map2, boolean oper, Filter.Result onMatch, Filter.Result onMismatch) {
        super(map2, oper, onMatch, onMismatch);
    }
    ...
    private String getValue(StructuredDataMessage data2, String key) {
        if (key.equalsIgnoreCase("id")) {
            return data2.getId().toString();
        }
        if (key.equalsIgnoreCase("id.name")) {
            return data2.getId().getName();
        }
        if (key.equalsIgnoreCase("type")) {
            return data2.getType();
        }
        if (key.equalsIgnoreCase("message")) {
            return data2.getFormattedMessage();
        }
        return data2.getData().get(key);
    }
    ...
    public static StructuredDataFilter createFilter(@PluginElement(value="Pairs") KeyValuePair[] pairs, @PluginAttribute(value="operator") String oper, @PluginAttribute(value="onMatch") String match, @PluginAttribute(value="onMismatch") String mismatch) {
        ...
        for (KeyValuePair pair : pairs) {
            ...
            ArrayList<String> list = (ArrayList<String>)map2.get(pair.getKey());
            if (list != null) {
                list.add(value);
                continue;
            }
        ...
        return new StructuredDataFilter(map2, isAnd, onMatch, onMismatch);
    }
}
