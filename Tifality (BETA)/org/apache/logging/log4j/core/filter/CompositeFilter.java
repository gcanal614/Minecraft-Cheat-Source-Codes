...
LifeCycle {
    ...
    public Filter.Result filter(Logger logger, Level level, Marker marker, String msg, Object ... params) {
        ...
        for (Filter filter : this.filters) {
            result2 = filter.filter(logger, level, marker, msg, params);
            if (result2 != Filter.Result.ACCEPT && result2 != Filter.Result.DENY) continue;
            return result2;
        }
        return result2;
    }
    ...
    public Filter.Result filter(Logger logger, Level level, Marker marker, Object msg, Throwable t) {
        Filter.Result result2 = Filter.Result.NEUTRAL;
        for (Filter filter : this.filters) {
            result2 = filter.filter(logger, level, marker, msg, t);
            if (result2 != Filter.Result.ACCEPT && result2 != Filter.Result.DENY) continue;
            return result2;
        }
        return result2;
