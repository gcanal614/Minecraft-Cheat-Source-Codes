...
extends AbstractLogger {
    ...
    public void log(Marker marker, String fqcn, Level level, Message msg, Throwable t) {
        ...
        StatusData data2 = new StatusData(element, level, msg, t);
        this.msgLock.lock();
        try {
            this.messages.add(data2);
        }
        finally {
            this.msgLock.unlock();
        }
        if (this.listeners.size() > 0) {
            for (StatusListener listener : this.listeners) {
                if (!data2.getLevel().isAtLeastAsSpecificAs(listener.getStatusLevel())) continue;
                listener.log(data2);
            }
        } else {
    ...
    @Override
    protected boolean isEnabled(Level level, Marker marker, String data2) {
        return this.isEnabled(level, marker);
    }

    @Override
    protected boolean isEnabled(Level level, Marker marker, String data2, Throwable t) {
        return this.isEnabled(level, marker);
    }

    @Override
    protected boolean isEnabled(Level level, Marker marker, String data2, Object ... p1) {
        return this.isEnabled(level, marker);
    }

    @Override
    protected boolean isEnabled(Level level, Marker marker, Object data2, Throwable t) {
        return this.isEnabled(level, marker);
    }

    @Override
    protected boolean isEnabled(Level level, Marker marker, Message data2, Throwable t) {
        return this.isEnabled(level, marker);
    }
