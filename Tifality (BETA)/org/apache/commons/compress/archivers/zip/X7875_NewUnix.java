...
Serializable {
    ...
    public byte[] getLocalFileDataData() {
        ...
        byte[] data2 = new byte[3 + uidBytes.length + gidBytes.length];
        ZipUtil.reverse(uidBytes);
        ZipUtil.reverse(gidBytes);
        int pos = 0;
        data2[pos++] = ZipUtil.unsignedIntToSignedByte(this.version);
        data2[pos++] = ZipUtil.unsignedIntToSignedByte(uidBytes.length);
        System.arraycopy(uidBytes, 0, data2, pos, uidBytes.length);
        pos += uidBytes.length;
        data2[pos++] = ZipUtil.unsignedIntToSignedByte(gidBytes.length);
        System.arraycopy(gidBytes, 0, data2, pos, gidBytes.length);
        return data2;
    }
    ...
    public void parseFromLocalFileData(byte[] data2, int offset, int length) throws ZipException {
        this.reset();
        this.version = ZipUtil.signedByteToUnsignedInt(data2[offset++]);
        int uidSize = ZipUtil.signedByteToUnsignedInt(data2[offset++]);
        byte[] uidBytes = new byte[uidSize];
        System.arraycopy(data2, offset, uidBytes, 0, uidSize);
        offset += uidSize;
        this.uid = new BigInteger(1, ZipUtil.reverse(uidBytes));
        int gidSize = ZipUtil.signedByteToUnsignedInt(data2[offset++]);
        byte[] gidBytes = new byte[gidSize];
        System.arraycopy(data2, offset, gidBytes, 0, gidSize);
