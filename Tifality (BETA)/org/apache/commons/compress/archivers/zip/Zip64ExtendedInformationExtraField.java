...
implements ZipExtraField {
    ...
    public byte[] getCentralDirectoryData() {
        byte[] data2 = new byte[this.getCentralDirectoryLength().getValue()];
        int off = this.addSizes(data2);
        if (this.relativeHeaderOffset != null) {
            System.arraycopy(this.relativeHeaderOffset.getBytes(), 0, data2, off, 8);
            off += 8;
        }
        if (this.diskStart != null) {
            System.arraycopy(this.diskStart.getBytes(), 0, data2, off, 4);
            off += 4;
        }
        return data2;
    }
    ...
    private int addSizes(byte[] data2) {
        int off = 0;
        if (this.size != null) {
            System.arraycopy(this.size.getBytes(), 0, data2, 0, 8);
            off += 8;
        }
        if (this.compressedSize != null) {
            System.arraycopy(this.compressedSize.getBytes(), 0, data2, off, 8);
            off += 8;
        }
        return off;
    }
