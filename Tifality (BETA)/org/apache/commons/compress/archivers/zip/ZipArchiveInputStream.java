...
extends ArchiveInputStream {
    ...
    public static boolean matches(byte[] signature2, int length) {
        if (length < ZipArchiveOutputStream.LFH_SIG.length) {
            return false;
        }
        return ZipArchiveInputStream.checksig(signature2, ZipArchiveOutputStream.LFH_SIG) || ZipArchiveInputStream.checksig(signature2, ZipArchiveOutputStream.EOCD_SIG) || ZipArchiveInputStream.checksig(signature2, ZipArchiveOutputStream.DD_SIG) || ZipArchiveInputStream.checksig(signature2, ZipLong.SINGLE_SEGMENT_SPLIT_MARKER.getBytes());
    }

    private static boolean checksig(byte[] signature2, byte[] expected) {
        for (int i = 0; i < expected.length; ++i) {
            if (signature2[i] == expected[i]) continue;
            return false;
        }
        return true;
    }
