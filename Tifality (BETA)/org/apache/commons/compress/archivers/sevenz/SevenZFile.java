...
implements Closeable {
    ...
    private Archive readHeaders(byte[] password) throws IOException {
        byte[] signature2 = new byte[6];
        this.file.readFully(signature2);
        if (!Arrays.equals(signature2, sevenZSignature)) {
    ...
    private InputStream buildDecoderStack(Folder folder, long folderOffset, int firstPackStreamIndex, SevenZArchiveEntry entry) throws IOException {
        ...
        LinkedList<SevenZMethodConfiguration> methods2 = new LinkedList<SevenZMethodConfiguration>();
        for (Coder coder : folder.getOrderedCoders()) {
            if (coder.numInStreams != 1L || coder.numOutStreams != 1L) {
                throw new IOException("Multi input/output stream coders are not yet supported");
            }
            ...
            methods2.addFirst(new SevenZMethodConfiguration(method, Coders.findByMethod(method).getOptionsFromCoder(coder, inputStreamStack)));
        }
        entry.setContentMethods(methods2);
        if (folder.hasCrc) {
    ...
    public static boolean matches(byte[] signature2, int length) {
        ...
        for (int i = 0; i < sevenZSignature.length; ++i) {
            if (signature2[i] == sevenZSignature[i]) continue;
            return false;
        }
        return true;
    }
