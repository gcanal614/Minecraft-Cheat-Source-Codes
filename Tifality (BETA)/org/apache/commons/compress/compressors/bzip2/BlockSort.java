...
class BlockSort {
    ...
    BlockSort(BZip2CompressorOutputStream.Data data2) {
        this.quadrant = data2.sfmap;
    }

    void blockSort(BZip2CompressorOutputStream.Data data2, int last) {
        this.workLimit = 30 * last;
        this.workDone = 0;
        this.firstAttempt = true;
        if (last + 1 < 10000) {
            this.fallbackSort(data2, last);
        } else {
            this.mainSort(data2, last);
    ...
    final void fallbackSort(BZip2CompressorOutputStream.Data data2, int last) {
        data2.block[0] = data2.block[last + 1];
        this.fallbackSort(data2.fmap, data2.block, last + 1);
        int i = 0;
        while (i < last + 1) {
            int n = i++;
            data2.fmap[n] = data2.fmap[n] - 1;
        }
        for (i = 0; i < last + 1; ++i) {
            if (data2.fmap[i] != -1) continue;
            data2.fmap[i] = last;
            break;
        }
    }
    ...
    final void mainSort(BZip2CompressorOutputStream.Data dataShadow, int lastShadow) {
        ...
        for (i = 0; i <= 255; ++i) {
            ...
            for (j2 = 0; j2 <= 255; ++j2) {
                copy2[j2] = ftab[(j2 << 8) + ss] & 0xFFDFFFFF;
            }
            ...
            for (j2 = ftab[ss << 8] & 0xFFDFFFFF; j2 < hj; ++j2) {
                ...
                fmap[copy2[c1]] = fmap_j == 0 ? lastShadow : fmap_j - 1;
                int n4 = c1;
                copy2[n4] = copy2[n4] + 1;
            }
            j2 = 256;
