...
implements DataInput {
    ...
    @Override
    public void readFully(byte[] data2) throws IOException, EOFException {
        this.readFully(data2, 0, data2.length);
    }

    @Override
    public void readFully(byte[] data2, int offset, int length) throws IOException, EOFException {
        int count;
        for (int remaining = length; remaining > 0; remaining -= count) {
            int location = offset + length - remaining;
            count = this.read(data2, location, remaining);
            if (-1 != count) continue;
