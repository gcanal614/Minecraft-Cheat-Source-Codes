...
import club.tifality.manager.event.impl.packet.DisconnectEvent;
...
implements INetHandlerPlayClient {
    ...
    private static boolean validateResourcePackUrl(NetHandlerPlayClient client, String url, String hash) {
        try {
            ...
            Wrapper.addChatMessage(EnumChatFormatting.RED.toString() + (Object)((Object)EnumChatFormatting.BOLD) + "Cancelled malicious resource download!");
            DevNotifications.getManager().post(EnumChatFormatting.RED.toString() + (Object)((Object)EnumChatFormatting.BOLD) + "Cancelled malicious resource download!");
            return false;
        }
        catch (UnsupportedEncodingException | URISyntaxException var7) {
            return false;
        }
    }
    ...
    public void handleEntityMetadata(S1CPacketEntityMetadata packetIn) {
        ...
        if (entity != null && watchableObjects != null) {
            ...
            if (entity instanceof EntityLivingBase && !(entity instanceof EntityPlayerSP) && (watchableObject = watchableObjects.get(0)).getDataValueId() == 6 && watchableObject.getObjectType() == 3) {
                float newHealth = ((Float)watchableObject.getObject()).floatValue();
                EntityLivingBase elb = (EntityLivingBase)entity;
                float health = elb.getHealth();
                float dif = health - newHealth;
                if (dif >= 0.0f && newHealth != 10.0f) {
                    Tifality.getInstance().getEventBus().post(new DamageEntityEvent(elb, dif));
                }
            }
    ...
    public void handleDisconnect(S40PacketDisconnect packetIn) {
        ...
        Tifality.getInstance().getEventBus().post(disconnectEvent);
    }
    ...
    public void sendPacket(Packet<?> p) {
        ...
        Tifality.getInstance().getEventBus().post(sendEvent);
    ...
    public void sendPacketButRichRoll(Packet<?> packet) {
    ...
    public void handleAnimation(S0BPacketAnimation packetIn) {
        ...
        if (entity != null) {
            if (packetIn.getAnimationType() == 0) {
                ...
                Tifality.getInstance().getEventBus().post(new EntitySwingEvent(entitylivingbase));
    ...
    private void spawnParticleEntityEvent(Entity entity, int type2) {
        int multiplier;
        EnumParticleTypes particleType;
        EnumParticleTypes enumParticleTypes = particleType = type2 == 0 ? EnumParticleTypes.CRIT : EnumParticleTypes.CRIT_MAGIC;
        if (entity instanceof EntityPlayerSP) {
            multiplier = 1;
        } else {
            SpawnParticleEntityEvent event = new SpawnParticleEntityEvent(type2, 1);
    ...
    @Override
    public void handleStatistics(S37PacketStatistics p_handleStatistics_1_) {
        PacketThreadUtil.checkThreadAndEnqueue(p_handleStatistics_1_, this, this.gameController);
        boolean flag = false;
        for (Map.Entry<StatBase, Integer> entry : p_handleStatistics_1_.func_148974_c().entrySet()) {
    ...
    public void handleTitle(S45PacketTitle packetIn) {
        ...
        DisplayTitleEvent event = new DisplayTitleEvent(StringUtils.stripControlCodes(s2));
        Tifality.getInstance().getEventBus().post(event);
    ...
    public void handleResourcePack(S48PacketResourcePackSend packetIn) {
        ...
        if (!NetHandlerPlayClient.validateResourcePackUrl(this, s, s1 = packetIn.getHash())) {
            return;
        }
    ...
    @Override
    public void handleScoreboardObjective(S3BPacketScoreboardObjective p_handleScoreboardObjective_1_) {
        PacketThreadUtil.checkThreadAndEnqueue(p_handleScoreboardObjective_1_, this, this.gameController);
        Scoreboard scoreboard = this.clientWorldController.getScoreboard();
        if (p_handleScoreboardObjective_1_.func_149338_e() == 0) {
            ScoreObjective scoreobjective1 = scoreboard.addScoreObjective(p_handleScoreboardObjective_1_.func_149339_c(), IScoreObjectiveCriteria.DUMMY);
            scoreobjective1.setDisplayName(p_handleScoreboardObjective_1_.func_149337_d());
            scoreobjective1.setRenderType(p_handleScoreboardObjective_1_.func_179817_d());
        } else {
            ScoreObjective scoreobjective1 = scoreboard.getObjective(p_handleScoreboardObjective_1_.func_149339_c());
            if (p_handleScoreboardObjective_1_.func_149338_e() == 1) {
                scoreboard.removeObjective(scoreobjective1);
            } else if (p_handleScoreboardObjective_1_.func_149338_e() == 2) {
                scoreobjective1.setDisplayName(p_handleScoreboardObjective_1_.func_149337_d());
                scoreobjective1.setRenderType(p_handleScoreboardObjective_1_.func_179817_d());
            }
        }
    ...
    @Override
    public void handleDisplayScoreboard(S3DPacketDisplayScoreboard p_handleDisplayScoreboard_1_) {
        PacketThreadUtil.checkThreadAndEnqueue(p_handleDisplayScoreboard_1_, this, this.gameController);
        Scoreboard scoreboard = this.clientWorldController.getScoreboard();
        if (p_handleDisplayScoreboard_1_.func_149370_d().length() == 0) {
            scoreboard.setObjectiveInDisplaySlot(p_handleDisplayScoreboard_1_.func_149371_c(), null);
        } else {
            ScoreObjective scoreobjective = scoreboard.getObjective(p_handleDisplayScoreboard_1_.func_149370_d());
            scoreboard.setObjectiveInDisplaySlot(p_handleDisplayScoreboard_1_.func_149371_c(), scoreobjective);
        }
    }
