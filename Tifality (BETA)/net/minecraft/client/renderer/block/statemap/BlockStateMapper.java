...
public class BlockStateMapper {
    ...
    public Map<IBlockState, ModelResourceLocation> putAllStateModelLocations() {
        IdentityHashMap<IBlockState, ModelResourceLocation> map2 = Maps.newIdentityHashMap();
        for (Block block : Block.blockRegistry) {
            if (this.setBuiltInBlocks.contains(block)) continue;
            map2.putAll(((IStateMapper)Objects.firstNonNull(this.blockStateMap.get(block), new DefaultStateMapper())).putStateModelLocations(block));
        }
        return map2;
    }
}
