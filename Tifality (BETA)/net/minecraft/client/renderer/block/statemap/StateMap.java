...
extends StateMapperBase {
    ...
    protected ModelResourceLocation getModelResourceLocation(IBlockState state) {
        ...
        String s = this.name == null ? ((ResourceLocation)Block.blockRegistry.getNameForObject(state.getBlock())).toString() : this.name.getName((Comparable)map2.remove(this.name));
        if (this.suffix != null) {
            s = s + this.suffix;
        }
        for (IProperty<?> iproperty : this.ignored) {
            map2.remove(iproperty);
        }
