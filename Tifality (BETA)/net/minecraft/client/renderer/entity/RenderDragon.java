...
extends RenderLiving<EntityDragon> {
    ...
    protected void drawRechargeRay(EntityDragon dragon, double p_180574_2_, double p_180574_4_, double p_180574_6_, float p_180574_8_) {
        ...
        GL11.glRotatef((float)(-Math.atan2(f4, f2)) * 180.0f / (float)Math.PI - 90.0f, 0.0f, 1.0f, 0.0f);
        GL11.glRotatef((float)(-Math.atan2(f5, f3)) * 180.0f / (float)Math.PI - 90.0f, 1.0f, 0.0f, 0.0f);
        ...
        for (int j = 0; j <= 8; ++j) {
            ...
            worldrenderer.pos(f9 * 0.2f, f10 * 0.2f, 0.0).tex(f11, f8).color4i(0, 0, 0, 255).endVertex();
            worldrenderer.pos(f9, f10, f6).tex(f11, f7).color4i(255, 255, 255, 255).endVertex();
        }
        tessellator.draw();
