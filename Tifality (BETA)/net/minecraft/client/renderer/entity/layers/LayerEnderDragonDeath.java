    ...
    public void doRenderLayer(EntityDragon entitylivingbaseIn, float p_177141_2_, float p_177141_3_, float partialTicks, float p_177141_5_, float p_177141_6_, float p_177141_7_, float scale) {
        if (entitylivingbaseIn.deathTicks > 0) {
            ...
            while ((float)i < (f + f * f) / 2.0f * 60.0f) {
                ...
                worldrenderer.pos(0.0, 0.0, 0.0).color4i(255, 255, 255, (int)(255.0f * (1.0f - f1))).endVertex();
                worldrenderer.pos(-0.866 * (double)f3, f2, -0.5f * f3).color4i(255, 0, 255, 0).endVertex();
                worldrenderer.pos(0.866 * (double)f3, f2, -0.5f * f3).color4i(255, 0, 255, 0).endVertex();
                worldrenderer.pos(0.0, f2, 1.0f * f3).color4i(255, 0, 255, 0).endVertex();
                worldrenderer.pos(-0.866 * (double)f3, f2, -0.5f * f3).color4i(255, 0, 255, 0).endVertex();
                tessellator.draw();
                ++i;
            }
