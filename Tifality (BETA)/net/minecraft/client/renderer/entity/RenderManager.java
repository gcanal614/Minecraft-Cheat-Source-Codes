...
public class RenderManager {
    ...
    public <T extends Entity> Render<T> getEntityClassRenderObject(Class<? extends Entity> p_78715_1_) {
        Render<T> render2 = this.entityRenderMap.get(p_78715_1_);
        if (render2 == null && p_78715_1_ != Entity.class) {
            render2 = this.getEntityClassRenderObject(p_78715_1_.getSuperclass());
            this.entityRenderMap.put(p_78715_1_, render2);
        }
        return render2;
    }
    ...
    public boolean shouldRender(Entity entityIn, ICamera camera, double camX, double camY, double camZ) {
        Render<Entity> render2 = this.getEntityRenderObject(entityIn);
        return render2 != null && render2.shouldRender(entityIn, camera, camX, camY, camZ);
    }
    ...
    public void renderWitherSkull(Entity entityIn, float partialTicks) {
        ...
        Render<Entity> render2 = this.getEntityRenderObject(entityIn);
        if (render2 != null && this.renderEngine != null) {
            int i = entityIn.getBrightnessForRender(partialTicks);
            int j = i % 65536;
            int k = i / 65536;
            OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, (float)j / 1.0f, (float)k / 1.0f);
            GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
            render2.renderName(entityIn, d0 - renderPosX, d1 - renderPosY, d2 - renderPosZ);
        }
    }
    ...
    public boolean doRenderEntity(Entity entity, double x, double y, double z, float entityYaw, float partialTicks, boolean p_147939_10_) {
        Render<Entity> render2 = null;
        try {
            render2 = this.getEntityRenderObject(entity);
            if (render2 != null && this.renderEngine != null) {
                try {
                    if (render2 instanceof RendererLivingEntity) {
                        ((RendererLivingEntity)render2).setRenderOutlines(this.renderOutlines);
                    }
                    if (CustomEntityModels.isActive()) {
                        this.renderRender = render2;
                    }
                    render2.doRender(entity, x, y, z, entityYaw, partialTicks);
                }
                ...
                try {
                    if (!this.renderOutlines) {
                        render2.doRenderShadowAndFire(entity, x, y, z, entityYaw, partialTicks);
                    }
                }
            ...
            crashreportcategory1.addCrashSection("Assigned renderer", render2);
    ...
    private void renderDebugBoundingBox(Entity entityIn, double p_85094_2_, double p_85094_4_, double p_85094_6_, float p_85094_8_, float p_85094_9_) {
        ...
        worldrenderer.pos(p_85094_2_ + vec3.xCoord * 2.0, p_85094_4_ + (double)entityIn.getEyeHeight() + vec3.yCoord * 2.0, p_85094_6_ + vec3.zCoord * 2.0).color4i(0, 0, 255, 255).endVertex();
        tessellator.draw();
