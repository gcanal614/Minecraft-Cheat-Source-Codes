...
extends GuiContainer {
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        GuiBrewingStand.drawTexturedModalRect(i, j, 0, 0, this.xSize, this.ySize);
        int k = this.tileBrewingStand.getField(0);
        if (k > 0) {
            int l = (int)(28.0f * (1.0f - (float)k / 400.0f));
            if (l > 0) {
                GuiBrewingStand.drawTexturedModalRect(i + 97, j + 16, 176, 0, 9, l);
            }
            int i1 = k / 2 % 7;
            switch (i1) {
                case 0: {
            ...
            if (l > 0) {
                GuiBrewingStand.drawTexturedModalRect(i + 65, j + 14 + 29 - l, 185, 29 - l, 12, l);
            }
        }
    }
}
