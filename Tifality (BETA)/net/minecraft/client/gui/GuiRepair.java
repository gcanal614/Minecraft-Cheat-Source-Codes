...
implements ICrafting {
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        int i = (this.width - this.xSize) / 2;
        int j = (this.height - this.ySize) / 2;
        GuiRepair.drawTexturedModalRect(i, j, 0, 0, this.xSize, this.ySize);
        GuiRepair.drawTexturedModalRect(i + 59, j + 20, 0, this.ySize + (this.anvil.getSlot(0).getHasStack() ? 0 : 16), 110, 16);
        if ((this.anvil.getSlot(0).getHasStack() || this.anvil.getSlot(1).getHasStack()) && !this.anvil.getSlot(2).getHasStack()) {
            GuiRepair.drawTexturedModalRect(i + 99, j + 45, this.xSize, 0, 28, 21);
        }
    }
