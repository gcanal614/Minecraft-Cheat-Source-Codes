...
extends NBTBase {
    ...
    public NBTTagList getTagList(String key, int type2) {
        try {
            ...
            return nbttaglist.tagCount() > 0 && nbttaglist.getTagType() != type2 ? new NBTTagList() : nbttaglist;
        }
        catch (ClassCastException classcastexception) {
    ...
    private static void writeEntry(String name, NBTBase data2, DataOutput output) throws IOException {
        output.writeByte(data2.getId());
        if (data2.getId() != 0) {
