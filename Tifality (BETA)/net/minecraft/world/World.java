...
implements IBlockAccess {
    ...
    public void initialize(WorldSettings settings2) {
    ...
    public int getLightFromNeighborsFor(EnumSkyBlock type2, BlockPos pos) {
        if (this.provider.getHasNoSky() && type2 == EnumSkyBlock.SKY) {
            return 0;
        }
        if (pos.getY() < 0) {
    ...
    public Vec3 getCloudColour(float partialTicks) {
        ...
        float f2 = (float)(cloudColour >> 16 & 0xFFL) / 255.0f;
        float f3 = (float)(cloudColour >> 8 & 0xFFL) / 255.0f;
        float f4 = (float)(cloudColour & 0xFFL) / 255.0f;
    ...
    public void updateEntities() {
        ...
        for (i1 = 0; i1 < this.weatherEffects.size(); ++i1) {
            entity2 = this.weatherEffects.get(i1);
            try {
                ++entity2.ticksExisted;
                entity2.onUpdate();
            }
            catch (Throwable var9) {
                CrashReport crashreport1 = CrashReport.makeCrashReport(var9, "Ticking entity");
        ...
        for (i1 = 0; i1 < this.unloadedEntityList.size(); ++i1) {
            ...
            if (!entity2.addedToChunk || !this.isChunkLoaded(k1, i2, true)) continue;
    ...
    public List<AxisAlignedBB> getCollisionBoxes(AxisAlignedBB p_getCollisionBoxes_1_) {
        ArrayList<AxisAlignedBB> list = Lists.newArrayList();
        int i = MathHelper.floor_double(p_getCollisionBoxes_1_.minX);
        int j = MathHelper.floor_double(p_getCollisionBoxes_1_.maxX + 1.0);
        int k = MathHelper.floor_double(p_getCollisionBoxes_1_.minY);
        int l = MathHelper.floor_double(p_getCollisionBoxes_1_.maxY + 1.0);
        int i1 = MathHelper.floor_double(p_getCollisionBoxes_1_.minZ);
        int j1 = MathHelper.floor_double(p_getCollisionBoxes_1_.maxZ + 1.0);
        BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos();
        for (int k1 = i; k1 < j; ++k1) {
            for (int l1 = i1; l1 < j1; ++l1) {
                if (!this.isBlockLoaded(blockpos$mutableblockpos.func_181079_c(k1, 64, l1))) continue;
                for (int i2 = k - 1; i2 < l; ++i2) {
                    blockpos$mutableblockpos.func_181079_c(k1, i2, l1);
                    IBlockState iblockstate = k1 >= -30000000 && k1 < 30000000 && l1 >= -30000000 && l1 < 30000000 ? this.getBlockState(blockpos$mutableblockpos) : Blocks.bedrock.getDefaultState();
                    iblockstate.getBlock().addCollisionBoxesToList(this, blockpos$mutableblockpos, iblockstate, p_getCollisionBoxes_1_, list, null);
                }
            }
        }
        return list;
