...
implements IChunkProvider {
    ...
    public ChunkProviderFlat(World worldIn, long seed, boolean generateStructures, String flatGeneratorSettings) {
        ...
        if (generateStructures) {
            Map<String, Map<String, String>> map2 = this.flatWorldGenInfo.getWorldFeatures();
            if (map2.containsKey("village")) {
                Map<String, String> map1 = map2.get("village");
                if (!map1.containsKey("size")) {
                    map1.put("size", "1");
                }
                this.structureGenerators.add(new MapGenVillage(map1));
            }
            if (map2.containsKey("biome_1")) {
                this.structureGenerators.add(new MapGenScatteredFeature(map2.get("biome_1")));
            }
            if (map2.containsKey("mineshaft")) {
                this.structureGenerators.add(new MapGenMineshaft(map2.get("mineshaft")));
            }
            if (map2.containsKey("stronghold")) {
                this.structureGenerators.add(new MapGenStronghold(map2.get("stronghold")));
            }
            if (map2.containsKey("oceanmonument")) {
                this.structureGenerators.add(new StructureOceanMonument(map2.get("oceanmonument")));
            }
        }
