...
extends UserListEntry<T> {
    ...
    protected void onSerialization(JsonObject data2) {
        data2.addProperty("created", dateFormat.format(this.banStartDate));
        data2.addProperty("source", this.bannedBy);
        data2.addProperty("expires", this.banEndDate == null ? "forever" : dateFormat.format(this.banEndDate));
        data2.addProperty("reason", this.reason);
    }
}
