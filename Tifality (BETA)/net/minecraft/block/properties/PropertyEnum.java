...
extends PropertyHelper<T> {
    ...
    public static <T extends Enum<T>> PropertyEnum<T> create(String name, Class<T> clazz, T ... values2) {
        return PropertyEnum.create(name, clazz, Lists.newArrayList(values2));
    }

    public static <T extends Enum<T>> PropertyEnum<T> create(String name, Class<T> clazz, Collection<T> values2) {
        return new PropertyEnum<T>(name, clazz, values2);
    }
}
