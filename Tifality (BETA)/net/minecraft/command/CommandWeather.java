...
extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args2) throws CommandException {
        if (args2.length >= 1 && args2.length <= 2) {
            ...
            if (args2.length >= 2) {
                i = CommandWeather.parseInt(args2[1], 1, 1000000) * 20;
            }
            WorldServer world = MinecraftServer.getServer().worldServers[0];
            WorldInfo worldinfo = world.getWorldInfo();
            if ("clear".equalsIgnoreCase(args2[0])) {
            ...
            } else if ("rain".equalsIgnoreCase(args2[0])) {
                ...
                if (!"thunder".equalsIgnoreCase(args2[0])) {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args2, BlockPos pos) {
        return args2.length == 1 ? CommandWeather.getListOfStringsMatchingLastWord(args2, "clear", "rain", "thunder") : null;
    }
}
