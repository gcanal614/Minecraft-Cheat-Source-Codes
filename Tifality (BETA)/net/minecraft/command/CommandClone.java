...
extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args2) throws CommandException {
        ...
        BlockPos blockpos = CommandClone.parseBlockPos(sender, args2, 0, false);
        BlockPos blockpos1 = CommandClone.parseBlockPos(sender, args2, 3, false);
        BlockPos blockpos2 = CommandClone.parseBlockPos(sender, args2, 6, false);
        ...
        Block block = null;
        int j = -1;
        if ((args2.length < 11 || !args2[10].equals("force") && !args2[10].equals("move")) && structureboundingbox.intersectsWith(structureboundingbox1)) {
            throw new CommandException("commands.clone.noOverlap", new Object[0]);
        }
        if (args2.length >= 11 && args2[10].equals("move")) {
            flag = true;
        }
        ...
        boolean flag1 = false;
        if (args2.length >= 10) {
            if (args2[9].equals("masked")) {
                flag1 = true;
            } else if (args2[9].equals("filtered")) {
                if (args2.length < 12) {
                    throw new WrongUsageException("commands.clone.usage", new Object[0]);
                }
                block = CommandClone.getBlockByText(sender, args2[11]);
                if (args2.length >= 13) {
                    j = CommandClone.parseInt(args2[12], 0, 15);
                }
            }
        }
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args2, BlockPos pos) {
        return args2.length > 0 && args2.length <= 3 ? CommandClone.func_175771_a(args2, 0, pos) : (args2.length > 3 && args2.length <= 6 ? CommandClone.func_175771_a(args2, 3, pos) : (args2.length > 6 && args2.length <= 9 ? CommandClone.func_175771_a(args2, 6, pos) : (args2.length == 10 ? CommandClone.getListOfStringsMatchingLastWord(args2, "replace", "masked", "filtered") : (args2.length == 11 ? CommandClone.getListOfStringsMatchingLastWord(args2, "normal", "force", "move") : (args2.length == 12 && "filtered".equals(args2[9]) ? CommandClone.getListOfStringsMatchingLastWord(args2, Block.blockRegistry.getKeys()) : null)))));
    }
