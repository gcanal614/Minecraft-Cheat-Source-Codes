...
extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args2) throws CommandException {
        ...
        if (args2[0].equals("set")) {
            ...
            double d2 = CommandWorldBorder.parseDouble(args2[1], 1.0, 6.0E7);
            long l = i = args2.length > 2 ? CommandWorldBorder.parseLong(args2[2], 0L, 9223372036854775L) * 1000L : 0L;
            if (i > 0L) {
            ...
            double d8 = d4 + CommandWorldBorder.parseDouble(args2[1], -d4, 6.0E7 - d4);
            long i1 = worldborder.getTimeUntilTarget() + (args2.length > 2 ? CommandWorldBorder.parseLong(args2[2], 0L, 9223372036854775L) * 1000L : 0L);
            if (i1 > 0L) {
        ...
        } else if (args2[0].equals("center")) {
            if (args2.length != 3) {
                throw new WrongUsageException("commands.worldborder.center.usage", new Object[0]);
            }
            BlockPos blockpos = sender.getPosition();
            double d1 = CommandWorldBorder.parseDouble((double)blockpos.getX() + 0.5, args2[1], true);
            double d3 = CommandWorldBorder.parseDouble((double)blockpos.getZ() + 0.5, args2[2], true);
        ...
        } else if (args2[0].equals("warning")) {
            if (args2.length < 2) {
                throw new WrongUsageException("commands.worldborder.warning.usage", new Object[0]);
            }
            int j = CommandWorldBorder.parseInt(args2[2], 0);
            if (args2[1].equals("time")) {
                if (args2.length != 3) {
            ...
            } else if (args2[1].equals("distance")) {
                if (args2.length != 3) {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args2, BlockPos pos) {
        return args2.length == 1 ? CommandWorldBorder.getListOfStringsMatchingLastWord(args2, "set", "center", "damage", "warning", "add", "get") : (args2.length == 2 && args2[0].equals("damage") ? CommandWorldBorder.getListOfStringsMatchingLastWord(args2, "buffer", "amount") : (args2.length >= 2 && args2.length <= 3 && args2[0].equals("center") ? CommandWorldBorder.func_181043_b(args2, 1, pos) : (args2.length == 2 && args2[0].equals("warning") ? CommandWorldBorder.getListOfStringsMatchingLastWord(args2, "time", "distance") : null)));
    }
}
