...
implements ICommandManager {
    ...
    protected boolean tryExecute(ICommandSender sender, String[] args2, ICommand command, String input) {
        try {
            command.processCommand(sender, args2);
            return true;
        }
    ...
    private int getUsernameIndex(ICommand command, String[] args2) {
        if (command == null) {
            return -1;
        }
        for (int i = 0; i < args2.length; ++i) {
            if (!command.isUsernameIndex(args2, i) || !PlayerSelector.matchesMultiplePlayers(args2[i])) continue;
            return i;
        }
        return -1;
    }
