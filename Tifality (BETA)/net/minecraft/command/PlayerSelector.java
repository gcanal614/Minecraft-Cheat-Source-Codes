...
public class PlayerSelector {
    ...
    public static <T extends Entity> List<T> matchEntities(ICommandSender sender, String token, Class<? extends T> targetClass) {
        ...
        if (matcher.matches() && sender.canCommandSenderUseCommand(1, "@")) {
            Map<String, String> map2 = PlayerSelector.getArgumentMap(matcher.group(2));
            if (!PlayerSelector.isEntityTypeValid(sender, map2)) {
                return Collections.emptyList();
            }
            String s = matcher.group(1);
            BlockPos blockpos = PlayerSelector.func_179664_b(map2, sender.getPosition());
            List<World> list = PlayerSelector.getWorlds(sender, map2);
            ArrayList<? extends T> list1 = Lists.newArrayList();
            for (World world : list) {
                if (world == null) continue;
                ArrayList<Predicate<Entity>> list2 = Lists.newArrayList();
                list2.addAll(PlayerSelector.func_179663_a(map2, s));
                list2.addAll(PlayerSelector.func_179648_b(map2));
                list2.addAll(PlayerSelector.func_179649_c(map2));
                list2.addAll(PlayerSelector.func_179659_d(map2));
                list2.addAll(PlayerSelector.func_179657_e(map2));
                list2.addAll(PlayerSelector.func_179647_f(map2));
                list2.addAll(PlayerSelector.func_180698_a(map2, blockpos));
                list2.addAll(PlayerSelector.func_179662_g(map2));
                list1.addAll(PlayerSelector.filterResults(map2, targetClass, list2, s, world, blockpos));
            }
            return PlayerSelector.func_179658_a(list1, map2, sender, targetClass, s, blockpos);
        }
        return Collections.emptyList();
    }
    ...
    private static List<Predicate<Entity>> func_179657_e(Map<String, String> p_179657_0_) {
        ArrayList<Predicate<Entity>> list = Lists.newArrayList();
        final Map<String, Integer> map2 = PlayerSelector.func_96560_a(p_179657_0_);
    ...
    public static Map<String, Integer> func_96560_a(Map<String, String> p_96560_0_) {
        ...
        for (String s : p_96560_0_.keySet()) {
            ...
            map2.put(s.substring("score_".length()), MathHelper.parseIntWithDefault(p_96560_0_.get(s), 1));
        }
        return map2;
    }
    ...
    public static boolean matchesMultiplePlayers(String p_82377_0_) {
        ...
        return PlayerSelector.parseIntWithDefault(map2, "c", i) != 1;
    }
    ...
    private static Map<String, String> getArgumentMap(String argumentString) {
        ...
        if (j < argumentString.length()) {
            ...
            while (matcher1.find()) {
                map2.put(matcher1.group(1), matcher1.group(2));
            }
        }
        return map2;
    }
