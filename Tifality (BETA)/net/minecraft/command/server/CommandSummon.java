...
extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args2) throws CommandException {
        ...
        if (args2.length >= 4) {
            d0 = CommandSummon.parseDouble(d0, args2[1], true);
            d1 = CommandSummon.parseDouble(d1, args2[2], false);
            d2 = CommandSummon.parseDouble(d2, args2[3], true);
        ...
        if ("LightningBolt".equals(s)) {
            ...
            boolean flag = false;
            if (args2.length >= 5) {
                IChatComponent ichatcomponent = CommandSummon.getChatComponentFromNthArg(sender, args2, 4);
                try {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args2, BlockPos pos) {
        return args2.length == 1 ? CommandSummon.getListOfStringsMatchingLastWord(args2, EntityList.getEntityNameList()) : (args2.length > 1 && args2.length <= 4 ? CommandSummon.func_175771_a(args2, 1, pos) : null);
    }
}
