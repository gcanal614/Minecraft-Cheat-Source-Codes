...
extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args2) throws CommandException {
        ...
        BlockPos blockpos = CommandTestForBlock.parseBlockPos(sender, args2, 0, false);
        Block block = Block.getBlockFromName(args2[3]);
        if (block == null) {
            throw new NumberInvalidException("commands.setblock.notFound", args2[3]);
        }
        int i = -1;
        if (args2.length >= 5) {
            i = CommandTestForBlock.parseInt(args2[4], -1, 15);
        }
        ...
        boolean flag = false;
        if (args2.length >= 6 && block.hasTileEntity()) {
            String s = CommandTestForBlock.getChatComponentFromNthArg(sender, args2, 5).getUnformattedText();
            try {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args2, BlockPos pos) {
        return args2.length > 0 && args2.length <= 3 ? CommandTestForBlock.func_175771_a(args2, 0, pos) : (args2.length == 4 ? CommandTestForBlock.getListOfStringsMatchingLastWord(args2, Block.blockRegistry.getKeys()) : null);
    }
}
