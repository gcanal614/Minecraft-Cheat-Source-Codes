...
extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args2) throws CommandException {
        ...
        int i = 0;
        if (args2.length != 2 && args2.length != 4 && args2.length != 6) {
            entity = CommandTeleport.getCommandSenderAsPlayer(sender);
        } else {
            entity = CommandTeleport.func_175768_b(sender, args2[0]);
            i = 1;
        }
        if (args2.length != 1 && args2.length != 2) {
            ...
            if (entity.worldObj != null) {
                int lvt_5_2_ = i + 1;
                CommandBase.CoordinateArg commandbase$coordinatearg = CommandTeleport.parseCoordinate(entity.posX, args2[i], true);
                CommandBase.CoordinateArg commandbase$coordinatearg1 = CommandTeleport.parseCoordinate(entity.posY, args2[lvt_5_2_++], 0, 0, false);
                CommandBase.CoordinateArg commandbase$coordinatearg2 = CommandTeleport.parseCoordinate(entity.posZ, args2[lvt_5_2_++], true);
                CommandBase.CoordinateArg commandbase$coordinatearg3 = CommandTeleport.parseCoordinate(entity.rotationYaw, args2.length > lvt_5_2_ ? args2[lvt_5_2_++] : "~", false);
                CommandBase.CoordinateArg commandbase$coordinatearg4 = CommandTeleport.parseCoordinate(entity.rotationPitch, args2.length > lvt_5_2_ ? args2[lvt_5_2_] : "~", false);
            ...
            Entity entity1 = CommandTeleport.func_175768_b(sender, args2[args2.length - 1]);
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args2, BlockPos pos) {
        return args2.length != 1 && args2.length != 2 ? null : CommandTeleport.getListOfStringsMatchingLastWord(args2, MinecraftServer.getServer().getAllUsernames());
    }
