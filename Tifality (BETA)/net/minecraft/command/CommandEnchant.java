...
extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args2) throws CommandException {
        ...
        EntityPlayerMP entityplayer = CommandEnchant.getPlayer(sender, args2[0]);
        sender.setCommandStat(CommandResultStats.Type.AFFECTED_ITEMS, 0);
        try {
            i = CommandEnchant.parseInt(args2[1], 0);
        }
        catch (NumberInvalidException numberinvalidexception) {
            Enchantment enchantment = Enchantment.getEnchantmentByLocation(args2[1]);
            if (enchantment == null) {
        ...
        if (args2.length >= 3) {
            j = CommandEnchant.parseInt(args2[2], enchantment1.getMinLevel(), enchantment1.getMaxLevel());
        }
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args2, BlockPos pos) {
        return args2.length == 1 ? CommandEnchant.getListOfStringsMatchingLastWord(args2, this.getListOfPlayers()) : (args2.length == 2 ? CommandEnchant.getListOfStringsMatchingLastWord(args2, Enchantment.func_181077_c()) : null);
    }
