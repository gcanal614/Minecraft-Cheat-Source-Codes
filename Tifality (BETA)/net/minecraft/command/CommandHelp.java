...
extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args2) throws CommandException {
        ...
        int k = 0;
        try {
            k = args2.length == 0 ? 0 : CommandHelp.parseInt(args2[0], 1, j + 1) - 1;
        }
        catch (NumberInvalidException numberinvalidexception) {
            Map<String, ICommand> map2 = this.getCommands();
            ICommand icommand = map2.get(args2[0]);
            if (icommand != null) {
                throw new WrongUsageException(icommand.getCommandUsage(sender), new Object[0]);
            }
            if (MathHelper.parseIntWithDefault(args2[0], -1) != -1) {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args2, BlockPos pos) {
        if (args2.length == 1) {
            ...
            return CommandHelp.getListOfStringsMatchingLastWord(args2, set.toArray(new String[set.size()]));
        }
        return null;
    }
}
