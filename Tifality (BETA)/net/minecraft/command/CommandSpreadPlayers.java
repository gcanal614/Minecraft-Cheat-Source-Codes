...
extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args2) throws CommandException {
        ...
        double d0 = CommandSpreadPlayers.parseDouble(blockpos.getX(), args2[i++], true);
        double d1 = CommandSpreadPlayers.parseDouble(blockpos.getZ(), args2[i++], true);
        double d2 = CommandSpreadPlayers.parseDouble(args2[i++], 0.0);
        double d3 = CommandSpreadPlayers.parseDouble(args2[i++], d2 + 1.0);
        boolean flag = CommandSpreadPlayers.parseBoolean(args2[i++]);
        ArrayList<Entity> list = Lists.newArrayList();
        while (i < args2.length) {
            String s;
            if (PlayerSelector.hasArguments(s = args2[i++])) {
    ...
    private double func_110671_a(List<Entity> p_110671_1_, World worldIn, Position[] p_110671_3_, boolean p_110671_4_) {
        double d0 = 0.0;
        int i = 0;
        HashMap<Team, Position> map2 = Maps.newHashMap();
        for (int j = 0; j < p_110671_1_.size(); ++j) {
            ...
            if (p_110671_4_) {
                ...
                if (!map2.containsKey(team)) {
                    map2.put(team, p_110671_3_[i++]);
                }
                commandspreadplayers$position = (Position)map2.get(team);
            } else {
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args2, BlockPos pos) {
        return args2.length >= 1 && args2.length <= 2 ? CommandSpreadPlayers.func_181043_b(args2, 0, pos) : null;
    }
