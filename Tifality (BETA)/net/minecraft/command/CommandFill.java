...
extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args2) throws CommandException {
        ...
        block24: {
            block19: {
                ...
                BlockPos blockpos = CommandFill.parseBlockPos(sender, args2, 0, false);
                BlockPos blockpos1 = CommandFill.parseBlockPos(sender, args2, 3, false);
                Block block = CommandBase.getBlockByText(sender, args2[6]);
                int i = 0;
                if (args2.length >= 8) {
                    i = CommandFill.parseInt(args2[7], 0, 15);
                }
                ...
                boolean flag = false;
                if (args2.length >= 10 && block.hasTileEntity()) {
                    String s = CommandFill.getChatComponentFromNthArg(sender, args2, 9).getUnformattedText();
                    try {
                        ...
                        for (int k1 = blockpos2.getX(); k1 <= blockpos3.getX(); ++k1) {
                            ...
                            block20: {
                                block21: {
                                    block23: {
                                        block22: {
                                            ...
                                            if (args2.length < 9) break block20;
                                            if (args2[8].equals("outline") || args2[8].equals("hollow")) break block21;
                                            if (!args2[8].equals("destroy")) break block22;
                                            world.destroyBlock(blockpos4, true);
                                            break block20;
                                        }
                                        if (!args2[8].equals("keep")) break block23;
                                        if (!world.isAirBlock(blockpos4)) {
                                            continue;
                                        }
                                        break block20;
                                    }
                                    if (!args2[8].equals("replace") || block.hasTileEntity()) break block20;
                                    if (args2.length > 9) {
                                        Block block1 = CommandBase.getBlockByText(sender, args2[9]);
                                        if (world.getBlockState(blockpos4).getBlock() != block1) continue;
                                    }
                                    if (args2.length <= 10) break block20;
                                    int l1 = CommandBase.parseInt(args2[10]);
                                ...
                                if (k1 != blockpos2.getX() && k1 != blockpos3.getX() && j1 != blockpos2.getY() && j1 != blockpos3.getY() && i1 != blockpos2.getZ() && i1 != blockpos3.getZ()) {
                                    if (!args2[8].equals("hollow")) continue;
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args2, BlockPos pos) {
        return args2.length > 0 && args2.length <= 3 ? CommandFill.func_175771_a(args2, 0, pos) : (args2.length > 3 && args2.length <= 6 ? CommandFill.func_175771_a(args2, 3, pos) : (args2.length == 7 ? CommandFill.getListOfStringsMatchingLastWord(args2, Block.blockRegistry.getKeys()) : (args2.length == 9 ? CommandFill.getListOfStringsMatchingLastWord(args2, "replace", "destroy", "keep", "hollow", "outline") : (args2.length == 10 && "replace".equals(args2[8]) ? CommandFill.getListOfStringsMatchingLastWord(args2, Block.blockRegistry.getKeys()) : null))));
    }
}
