...
import club.tifality.manager.event.impl.player.MoveFlyingEvent;
...
implements ICommandSender {
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            boolean bl = flag = this.onGround && (this.isSneaking() || event.isCancelled()) && this instanceof EntityPlayer;
            if (flag) {
                double d6 = 0.05;
            ...
            if (this.stepHeight > 0.0f && flag1 && (d3 != x || d5 != z)) {
                boolean bl2 = this instanceof EntityPlayerSP;
                StepEvent stepEvent = new StepEvent(this.stepHeight);
                if (bl2) {
                    Tifality.getInstance().getEventBus().post(stepEvent);
                }
                double d11 = x;
                double d7 = y;
                double d8 = z;
                ...
                if (bl2 && y > -0.5) {
                    ...
                    if (blockHeight % 0.5 == 0.0 || blockHeights.contains(blockHeight)) {
                ...
                if (d11 * d11 + d8 * d8 >= x * x + z * z) {
                    ...
                    Tifality.getInstance().getEventBus().post(new StepConfirmEvent());
                }
            }
            this.resetPositionToBB();
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        if (f >= 1.0E-4f) {
            ...
            strafe *= f;
            forward *= f;
            MoveFlyingEvent event = new MoveFlyingEvent(this.rotationYaw);
    ...
    public void setCommandStat(CommandResultStats.Type type2, int amount) {
        this.cmdResultStats.func_179672_a(this, type2, amount);
    }
