...
public class SmartLeaves {
    ...
    static IBakedModel getModelCull(String type2, List updatedTypes) {
        ModelManager modelmanager = Config.getModelManager();
        if (modelmanager == null) {
            return null;
        }
        ResourceLocation resourcelocation = new ResourceLocation("blockstates/" + type2 + "_leaves.json");
        if (Config.getDefiningResourcePack(resourcelocation) != Config.getDefaultResourcePack()) {
            return null;
        }
        ResourceLocation resourcelocation1 = new ResourceLocation("models/block/" + type2 + "_leaves.json");
        if (Config.getDefiningResourcePack(resourcelocation1) != Config.getDefaultResourcePack()) {
            return null;
        }
        ModelResourceLocation modelresourcelocation = new ModelResourceLocation(type2 + "_leaves", "normal");
        ...
        if (ibakedmodel != null && ibakedmodel != modelmanager.getMissingModel()) {
            ...
            list.clear();
            updatedTypes.add(type2 + "_leaves");
            return ibakedmodel;
        }
        return null;
    }
