...
public class Shaders {
    ...
    private static void loadShaderPackProperties() {
        ...
        if (shaderPack != null) {
            ...
            try {
                ...
                inputstream.close();
                shaderPackClouds.loadFrom(properties2);
                shaderPackOldHandLight.loadFrom(properties2);
                shaderPackDynamicHandLight.loadFrom(properties2);
                shaderPackOldLighting.loadFrom(properties2);
                shaderPackShadowTranslucent.loadFrom(properties2);
                shaderPackUnderwaterOverlay.loadFrom(properties2);
                shaderPackSun.loadFrom(properties2);
                shaderPackVignette.loadFrom(properties2);
                shaderPackMoon.loadFrom(properties2);
                shaderPackBackFaceSolid.loadFrom(properties2);
                shaderPackBackFaceCutout.loadFrom(properties2);
                shaderPackBackFaceCutoutMipped.loadFrom(properties2);
                shaderPackBackFaceTranslucent.loadFrom(properties2);
                shaderPackRainDepth.loadFrom(properties2);
                shaderPackBeaconBeamDepth.loadFrom(properties2);
                shaderPackSeparateAo.loadFrom(properties2);
                shaderPackFrustumCulling.loadFrom(properties2);
                shaderPackOptionSliders = ShaderPackParser.parseOptionSliders(properties2, shaderPackOptions);
                shaderPackProfiles = ShaderPackParser.parseProfiles(properties2, shaderPackOptions);
                shaderPackGuiScreens = ShaderPackParser.parseGuiScreens(properties2, shaderPackProfiles, shaderPackOptions);
                shaderPackProgramConditions = ShaderPackParser.parseProgramConditions(properties2, shaderPackOptions);
                customTexturesGbuffers = Shaders.loadCustomTextures(properties2, 0);
                customTexturesComposite = Shaders.loadCustomTextures(properties2, 1);
                customTexturesDeferred = Shaders.loadCustomTextures(properties2, 2);
                noiseTexturePath = properties2.getProperty("texture.noise");
                if (noiseTexturePath != null) {
                    noiseTextureEnabled = true;
                }
                customUniforms = ShaderPackParser.parseCustomUniforms(properties2);
                ShaderPackParser.parseAlphaStates(properties2);
                ShaderPackParser.parseBlendStates(properties2);
                ShaderPackParser.parseRenderScales(properties2);
                ShaderPackParser.parseBuffersFlip(properties2);
            }
    ...
    private static ICustomTexture loadCustomTextureRaw(int textureUnit, String line, String path, TextureType type2, InternalFormat internalFormat, int width, int height, int depth, PixelFormat pixelFormat, PixelType pixelType) {
        try {
            ...
            bytebuffer.flip();
            CustomTextureRaw customtextureraw = new CustomTextureRaw(type2, internalFormat, width, height, depth, pixelFormat, pixelType, bytebuffer, textureUnit);
    ...
    private static void saveShaderPackOptions(ShaderOption[] sos, IShaderPack sp) {
        ...
        if (shaderPackOptions != null) {
            for (int i = 0; i < sos.length; ++i) {
                ...
                properties2.setProperty(shaderoption.getName(), shaderoption.getValue());
            }
        }
        try {
            Shaders.saveOptionProperties(sp, properties2);
        }
        catch (IOException ioexception) {
    ...
    private static ShaderOption[] loadShaderPackOptions() {
        try {
            ...
            Properties properties2 = Shaders.loadOptionProperties(shaderPack);
            for (int i = 0; i < ashaderoption.length; ++i) {
                ShaderOption shaderoption = ashaderoption[i];
                String s = properties2.getProperty(shaderoption.getName());
                if (s == null) continue;
    ...
    public static void beginRender(Minecraft minecraft, float partialTicks) {
        ...
        ShadersRender.renderShadowMap(entityRenderer, 0, partialTicks);
        EXTFramebufferObject.glBindFramebufferEXT(36160, dfb);
        for (int j = 0; j < usedColorBuffers; ++j) {
            EXTFramebufferObject.glFramebufferTexture2DEXT(36160, 36064 + j, 3553, dfbColorTexturesFlip.getA(j), 0);
        }
        ...
        if (shaderPackLoaded) {
            ...
            try {
                for (String s3 : list) {
                    ...
                    PropertiesOrdered properties2 = new PropertiesOrdered();
                    Lang.loadLocaleData(inputstream, properties2);
                    inputstream.close();
                    for (Object o : ((Hashtable)properties2).keySet()) {
                        String s4 = (String)o;
                        String s5 = properties2.getProperty(s4);
