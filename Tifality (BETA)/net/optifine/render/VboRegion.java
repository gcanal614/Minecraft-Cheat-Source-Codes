...
public class VboRegion {
    ...
    public void bufferData(ByteBuffer data2, VboRange range) {
        int i = range.getPosition();
        int j = range.getSize();
        int k = this.toVertex(data2.limit());
        if (k <= 0) {
            if (i >= 0) {
                range.setPosition(-1);
                range.setSize(0);
            ...
            this.bindBuffer();
            OpenGlHelper.glBufferSubData(OpenGlHelper.GL_ARRAY_BUFFER, l, data2);
            this.unbindBuffer();
