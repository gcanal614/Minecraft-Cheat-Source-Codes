...
public class ExpressionParser {
    ...
    private IExpression makeFunction(FunctionType type2, Deque<Token> deque) throws ParseException {
        Token token;
        if (type2.getParameterCount(new IExpression[0]) == 0 && ((token = deque.peek()) == null || token.getType() != TokenType.BRACKET_OPEN)) {
    ...
    private static IExpression makeFunction(FunctionType type2, IExpression[] args2) throws ParseException {
        ExpressionType[] aexpressiontype = type2.getParameterTypes(args2);
        if (args2.length != aexpressiontype.length) {
            throw new ParseException("Invalid number of arguments, function: \"" + type2.getName() + "\", count arguments: " + args2.length + ", should be: " + aexpressiontype.length);
        }
        for (int i = 0; i < args2.length; ++i) {
            ExpressionType expressiontype1;
            IExpression iexpression = args2[i];
            ExpressionType expressiontype = iexpression.getExpressionType();
            if (expressiontype == (expressiontype1 = aexpressiontype[i])) continue;
            throw new ParseException("Invalid argument type, function: \"" + type2.getName() + "\", index: " + i + ", type: " + (Object)((Object)expressiontype) + ", should be: " + (Object)((Object)expressiontype1));
        }
        if (type2.getExpressionType() == ExpressionType.FLOAT) {
            return new FunctionFloat(type2, args2);
        }
        if (type2.getExpressionType() == ExpressionType.BOOL) {
            return new FunctionBool(type2, args2);
        }
        if (type2.getExpressionType() == ExpressionType.FLOAT_ARRAY) {
            return new FunctionFloatArray(type2, args2);
        }
        throw new ParseException("Unknown function type: " + (Object)((Object)type2.getExpressionType()) + ", function: " + type2.getName());
    }
    ...
    private static Deque<Token> getGroup(Deque<Token> deque, TokenType tokenTypeEnd, boolean tokenEndRequired) throws ParseException {
        ...
        int i = 0;
        Iterator<Token> iterator2 = deque.iterator();
