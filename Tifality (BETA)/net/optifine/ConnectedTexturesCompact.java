...
public class ConnectedTexturesCompact {
    ...
    private static BakedQuad getQuadCompact(TextureAtlasSprite sprite, Dir dir, int x1, int y1, int x2, int y2, int side, BakedQuad quadIn, RenderEnv renderEnv) {
        ...
        if (i >= 0 && i < amap.length) {
            BakedQuad bakedquad;
            IdentityHashMap<BakedQuad, BakedQuad> map2;
            Map[] amap1 = amap[i];
            if (amap1 == null) {
                amap1 = new Map[Dir.VALUES.length];
                amap[i] = amap1;
            }
            if ((map2 = amap1[dir.ordinal()]) == null) {
                amap1[dir.ordinal()] = map2 = new IdentityHashMap<BakedQuad, BakedQuad>(1);
            }
    ...
    private static void fixVertexCompact(int[] data2, int vertex, TextureAtlasSprite spriteFrom, TextureAtlasSprite spriteTo, int side, int x1, int y1, int x2, int y2) {
        float f6;
        float f5;
        ...
        float f2 = Float.intBitsToFloat(data2[j + 0]);
        float f3 = Float.intBitsToFloat(data2[j + 1]);
        float f4 = Float.intBitsToFloat(data2[j + 2]);
        switch (side) {
            case 0: {
                f5 = f2;
                f6 = 1.0f - f4;
        ...
        data2[j + 0] = Float.floatToRawIntBits(f2);
        data2[j + 1] = Float.floatToRawIntBits(f3);
        data2[j + 2] = Float.floatToRawIntBits(f4);
    }
