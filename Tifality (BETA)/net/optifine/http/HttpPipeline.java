...
public class HttpPipeline {
    ...
    public static HttpRequest makeRequest(String urlStr, Proxy proxy) throws IOException {
        ...
        map2.put(HEADER_USER_AGENT, "Java/" + System.getProperty("java.version"));
        map2.put(HEADER_HOST, s1);
        map2.put(HEADER_ACCEPT, "text/html, image/gif, image/png");
        map2.put(HEADER_CONNECTION, HEADER_VALUE_KEEP_ALIVE);
        byte[] abyte = new byte[]{};
        HttpRequest httprequest = new HttpRequest(s1, i, proxy, s2, s, s3, map2, abyte);
        return httprequest;
    }
    ...
    public static HttpResponse executeRequest(HttpRequest req) throws IOException {
        ...
        synchronized (hashMap) {
            ...
            Exception exception = (Exception)map2.get("Exception");
            if (exception != null) {
