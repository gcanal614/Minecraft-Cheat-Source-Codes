...
public class CustomColors {
    ...
    private static String getValidProperty(String fileName, String key, String[] validValues, String valDef) {
        try {
            ...
            inputstream.close();
            String s = properties2.getProperty(key);
            if (s == null) {
                return valDef;
            }
    ...
    private static void readColorProperties(String fileName) {
        try {
            ...
            properties2.load(inputstream);
            inputstream.close();
            particleWaterColor = CustomColors.readColor((Properties)properties2, new String[]{"particle.water", "drop.water"});
            particlePortalColor = CustomColors.readColor((Properties)properties2, "particle.portal");
            lilyPadColor = CustomColors.readColor((Properties)properties2, "lilypad");
            expBarTextColor = CustomColors.readColor((Properties)properties2, "text.xpbar");
            bossTextColor = CustomColors.readColor((Properties)properties2, "text.boss");
            signTextColor = CustomColors.readColor((Properties)properties2, "text.sign");
            fogColorNether = CustomColors.readColorVec3(properties2, "fog.nether");
            fogColorEnd = CustomColors.readColorVec3(properties2, "fog.end");
            skyColorEnd = CustomColors.readColorVec3(properties2, "sky.end");
            colorsBlockColormaps = CustomColors.readCustomColormaps(properties2, fileName);
            spawnEggPrimaryColors = CustomColors.readSpawnEggColors(properties2, fileName, "egg.shell.", "Spawn egg shell");
            spawnEggSecondaryColors = CustomColors.readSpawnEggColors(properties2, fileName, "egg.spots.", "Spawn egg spot");
            wolfCollarColors = CustomColors.readDyeColors(properties2, fileName, "collar.", "Wolf collar");
            sheepColors = CustomColors.readDyeColors(properties2, fileName, "sheep.", "Sheep");
            textColors = CustomColors.readTextColors(properties2, fileName, "text.code.", "Text");
            int[] aint = CustomColors.readMapColors(properties2, fileName, "map.", "Map");
            if (aint != null) {
            ...
            potionColors = CustomColors.readPotionColors(properties2, fileName, "potion.", "Potion");
            xpOrbTime = Config.parseInt(properties2.getProperty("xporb.time"), -1);
        }
    ...
    private static CustomColormap[] readCustomColormaps(Properties props, String fileName) {
        ...
        String[] astring = map2.keySet().toArray(new String[map2.size()]);
    ...
    private static CustomColormap[][] readBlockColormaps(String[] basePaths, CustomColormap[] basePalettes, int width, int height) {
        ...
        for (int i = 0; i < astring.length; ++i) {
            ...
            try {
                ...
                PropertiesOrdered properties2 = new PropertiesOrdered();
                properties2.load(inputstream);
                CustomColormap customcolormap = new CustomColormap(properties2, s, width, height, paletteFormatDefault);
    ...
    public static CustomColormap getCustomColors(String pathImage, int width, int height) {
        try {
            ...
            if (Config.hasResource(resourcelocation1)) {
                ...
                ((Hashtable)properties2).put("format", paletteFormatDefault);
                ((Hashtable)properties2).put("source", pathImage);
                s = pathImage;
            }
            CustomColormap customcolormap = new CustomColormap(properties2, s, width, height, paletteFormatDefault);
    ...
    private static float[][] readDyeColors(Properties props, String fileName, String prefix, String logName) {
        ...
        HashMap<String, EnumDyeColor> map2 = new HashMap<String, EnumDyeColor>();
        for (int i = 0; i < aenumdyecolor.length; ++i) {
            EnumDyeColor enumdyecolor = aenumdyecolor[i];
            map2.put(enumdyecolor.getName(), enumdyecolor);
        }
        ...
        for (Object e : props.keySet()) {
            ...
            EnumDyeColor enumdyecolor1 = (EnumDyeColor)map2.get(s2);
