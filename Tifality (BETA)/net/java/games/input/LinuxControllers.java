...
final class LinuxControllers {
    ...
    public static final synchronized boolean getNextDeviceEvent(Event event, LinuxEventDevice device) throws IOException {
        while (device.getNextEvent(linux_event)) {
            LinuxAxisDescriptor descriptor2 = linux_event.getDescriptor();
            LinuxComponent component = device.mapDescriptor(descriptor2);
            if (component == null) continue;
            float value = component.convertValue(linux_event.getValue(), descriptor2);
