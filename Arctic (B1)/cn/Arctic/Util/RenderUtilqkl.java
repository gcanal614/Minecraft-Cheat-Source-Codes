...
//		GL11.glDisable(3042);
	...
	public static void setupClientState(final GLClientState state, final boolean enabled) {
		RenderUtilqkl.csBuffer.clear();
		if (state.ordinal() > 0) {
			RenderUtilqkl.csBuffer.add(state.getCap());
		}
		RenderUtilqkl.csBuffer.add(32884);
		RenderUtilqkl.csBuffer.forEach(enabled ? RenderUtilqkl.ENABLE_CLIENT_STATE : RenderUtilqkl.DISABLE_CLIENT_STATE);
	}
...
//							R2DUtils.drawRect(x2, y2, x1, y1, color);
	...
	public static void drawOutline(double x, double y, double width, double height, double lineWidth, int color) {
		RenderUtil.drawRect((float) x, y, x + width, y + lineWidth, color);
		RenderUtil.drawRect((float) x, y, x + lineWidth, y + height, color);
		RenderUtil.drawRect((float) x, y + height - lineWidth, x + width, y + height, color);
		RenderUtil.drawRect((float) (x + width - lineWidth), y, x + width, y + height, color);
	}
