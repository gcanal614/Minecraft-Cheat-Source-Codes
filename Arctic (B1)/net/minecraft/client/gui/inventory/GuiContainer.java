...
import cn.Arctic.Module.modules.PLAYER.ChestStealer;
...
public abstract class GuiContainer extends GuiScreen {
    ...
    public void initGui() {
        ...
        this.buttonList.add(new GuiButton(6969, 5, 5, 85, 20, "Disable KillAura"));
        this.buttonList.add(new GuiButton(6971, 5, 25, 100, 20, "Disable InvCleaner"));
        this.buttonList.add(new GuiButton(6970, 5, 45, 110, 20,"Disable Cheststealer"));
        this.buttonList.add(new GuiButton(6972, 5, 65, 110, 20,"/lobby"));
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    	...
    	if(ChestStealer.slient.getValue()&&Client.instance.getModuleManager().getModuleByClass(ChestStealer.class).isEnabled()&&mc.player != null && mc.player.openContainer != null
                && mc.player.openContainer instanceof ContainerChest) {
    		GuiScreen guiScreen=mc.currentScreen;
    		if(ChestStealer.slient.getValue()&&Client.instance.getModuleManager().getModuleByClass(ChestStealer.class).isEnabled()&&mc.player != null && mc.player.openContainer != null
                    && mc.player.openContainer instanceof ContainerChest) {
    			String tipString = "拿取箱子中...";
    			FontLoaders2.msFont16.drawString(tipString,(width/2) - (FontLoaders2.msFont16.getStringWidth(tipString)/2),(height/2)+30,new Color(HUD.r.getValue().intValue(),HUD.g.getValue().intValue(),HUD.b.getValue().intValue(),255).getRGB());
    		}
    		mc.setIngameFocus();
        ...
        translate.interpolate(this.width,this.height,10);
        super.drawScreen(mouseX, mouseY, partialTicks);
        int var4 = this.guiLeft;
        int var5 = this.guiTop;
        //TODO
        ParticleUtils.drawParticles(Mouse.getX() * width / mc.displayWidth, height - Mouse.getY() * height / mc.displayHeight - 1);
        this.drawGradientRect(0, 0, this.width, this.height, new Color(HUD.r.getValue().intValue(), HUD.g.getValue().intValue(), HUD.b.getValue().intValue(),0).getRGB(), new Color(HUD.r.getValue().intValue(), HUD.g.getValue().intValue(), HUD.b.getValue().intValue()).getRGB());
        RenderUtil.drawBorderedRect(0, 0, RenderUtil.width(), RenderUtil.height(), 1, new Color(1,1,1,0).getRGB(), new Color(26,24,25,85).getRGB());
       double xmod = this.width/2-(translate.getX()/2);
        ...
        //this.drawGradientRect(0, 0, this.width, this.height, new Color(HUD.r.getValue().intValue(), HUD.g.getValue().intValue(), HUD.b.getValue().intValue(),0).getRGB(), new Color(HUD.r.getValue().intValue(), HUD.g.getValue().intValue(), HUD.b.getValue().intValue(),alpha).getRGB());
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        for (Object aButtonList : this.buttonList) {
            ...
            GuiButton var53 = (GuiButton) aButtonList;
            GuiButton var54 = (GuiButton) aButtonList;
            GuiButton var55 = (GuiButton) aButtonList;
            if (var52.mousePressed(this.mc, mouseX, mouseY) && var52.id == 6969) {
                Client.getModuleManager().getModuleByClass(Aura.class).setEnabled(false);
            }
            if (var54.mousePressed(this.mc, mouseX, mouseY) && var54.id == 6971) {
                Client.getModuleManager().getModuleByClass(InvCleaner.class).setEnabled(false);
            }
            if (var53.mousePressed(this.mc, mouseX, mouseY) && var53.id == 6970) {
                Client.getModuleManager().getModuleByClass(ChestStealer.class).setEnabled(false);
            }
            if (var53.mousePressed(this.mc, mouseX, mouseY) && var55.id == 6972) {
