					...
					if (chatline != null) {
						...
						if (j1 < 200 || flag) {
							...
							if (l1 > 3) {
								...
								float width = FontLoaders2.msFont15.getStringWidth(s);
								if(l1 == 255) {
									chatline.x =(float) chatline.animationUtils.animate(width, chatline.x,0.02f);
								}else {
									chatline.x =(float) chatline.animationUtils.animate(0, chatline.x,0.02f);
								}
								FontLoaders2.msFont15.drawString(s, (float) i2 + chatline.x-width, (float) (j2 - 10.5),
										16777215 + (l1 << 24));
