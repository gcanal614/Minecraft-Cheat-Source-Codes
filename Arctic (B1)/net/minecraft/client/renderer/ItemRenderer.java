...
public class ItemRenderer {
	...
	private void setLightMapFromPlayer(AbstractClientPlayer clientPlayer) {
		int i = this.mc.world.getCombinedLight(new BlockPos(clientPlayer.posX,
	 ...
	 */
	private void rotateWithPlayerRotations(ClientPlayerEntity entityplayerspIn, float partialTicks) {
	...
	private void doItemUsedTransformations(float swingProgress) {
		 float f = -0.4F * MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float)Math.PI);
	        float f1 = 0.2F * MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float)Math.PI * 2.0F);
	        float f2 = -0.2F * MathHelper.sin(swingProgress * (float)Math.PI);
	        if(ModuleManager.getModuleByClass(HUD.class).isEnabled()&&HUD.smooth.getValue().booleanValue()) {
	                   GlStateManager.translate(f-0.06, f1+0.3, f2+0.05);
	                   GlStateManager.scale(2f,1.6f,3f);
	...
	private void transformFirstPersonItem(float equipProgress, float swingProgress) {
		if (this.mc != null && this.mc.player != null && this.mc.player.getItemInUse() != null
				&& this.mc.player.getItemInUse().getItem() != null
				&& Item.getIdFromItem(this.mc.player.getItemInUse().getItem()) == 261) {
			GlStateManager.translate(0.0f, 0.05f, 0.04f);
			GlStateManager.scale(0.93f, 1.0f, 1.0f);
		}
		if (this.mc != null && this.mc.player != null && this.mc.player.getCurrentEquippedItem() != null
				&& this.mc.player.getCurrentEquippedItem().getItem() != null
				&& Item.getIdFromItem(this.mc.player.getCurrentEquippedItem().getItem()) == 346) {
	...
	public void renderItemInFirstPerson(float partialTicks) {
		if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
			...
			this.rotateWithPlayerRotations((ClientPlayerEntity) abstractclientplayer, partialTicks);
...
//						float f114514 = abstractclientplayer.getSwingProgress(partialTicks);
						
						this.animation(f, 0.0f);
						this.doBlockTransformations();
						
//                        final float var8 = MathHelper.sin(f1 * f1 * (float) Math.PI);
//                        final float var9 = MathHelper.sin(MathHelper.sqrt_float(f1) * (float) Math.PI);
//                        GlStateManager.translate(-0.2f, 0.2f, 0.1f);
//                        GlStateManager.rotate(-var9 * (float) 40.0 / 2.0f, -8.0f, -0.0f, 9.0f);
//                        GlStateManager.rotate(-var9 * (float) 40.0, 1.0f, -0.4f, -0.0f);
						float var8 = MathHelper.sin(f1 * f1 * 3.1415927f);
						float var9 = MathHelper.sin(MathHelper.sqrt_float(f1) * 3.1415927f);
						GlStateManager.translate(-0.05f, 0.38f, 0.1f);
						GlStateManager.rotate(-var9 * (float) 80.0 / 2.0f, -4.0f, -0.0f, 9.0f);
						GlStateManager.rotate(-var9 * (float) 80.0, 1.0f, -0.4f, -0.0f);
	...
	private void animation(float equipProgress, float swingProgress) {
		GlStateManager.translate(0.56F, -0.52F, -0.71999997F);
		GlStateManager.translate(0.0F, equipProgress * -0.6F, 0.0F);
		GlStateManager.rotate(45.0F, 0.0F, 1.0F, 0.0F);
		float f = MathHelper.sin(swingProgress * swingProgress * (float) Math.PI);
		...
		if (this.mc.player.isEntityInsideOpaqueBlock()) {
			...
			if (iblockstate.getBlock().getRenderType() != -1) {
				...
				if (!Reflector.callBoolean(Reflector.ForgeEventFactory_renderBlockOverlay,
						new Object[] { this.mc.player, Float.valueOf(partialTicks), object, iblockstate, blockpos })) {
		...
		if (!this.mc.player.isSpectator()) {
			if (this.mc.player.isInsideOfMaterial(Material.water)
					&& !Reflector.callBoolean(Reflector.ForgeEventFactory_renderWaterOverlay,
							new Object[] { this.mc.player, Float.valueOf(partialTicks) })) {
