...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdate()
    {
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ)))
        {
        	...
        	EventC.dispatch(eventUpdate);
        	if(eventUpdate.isCancelled()) return;
            super.onUpdate();
    ...
    {
    	EventUpdate event = new EventUpdate(posX, getEntityBoundingBox().minY, posZ, rotationYaw, rotationPitch, this.lastReportedYaw, this.lastReportedPitch, onGround);
    	EventC.dispatch(event);
    	
    	ScriptManager.instance.onUpdateScript();
    	Hime.instance.serverData = mc.getCurrentServerData();
    	//System.out.println(Hime.instance.serverData + "");
    	//  if (this.loaded.size() > 0) {
        //if (this.loaded.get(this.index) == null)
    	
    	if(!Hime.instance.moduleManager.getModule("Killaura").isToggled()) {
    	  if(!Hime.instance.moduleManager.getModule("Scaffold").isToggled()){
    		if(!Hime.instance.moduleManager.getModule("AntiAim").isToggled()) {
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            if (this.ridingEntity == null)
            {
                if (flag2 && flag3)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(event.getX(), event.getY(), event.getZ(), event.getYaw(), event.getPitch(), event.getGroundState()));
                }
                else if (flag2)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(event.getX(), event.getY(), event.getZ(), event.getGroundState()));
                }
                else if (flag3)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(event.getYaw(), event.getPitch(), event.getGroundState()));
                }
                else
                {
    ...
    public void sendChatMessage(String message)
    {
       if(message.startsWith(".")) {
    	   Hime.instance.commandManager.callCommand(message.substring(1));
       }else{
    ...
    public void displayGUIChest(IInventory chestInventory)
    {
        ...
        {
        	//TODO open chest event
        	
        	//if(Hime.instance.moduleManager.getModule("NoChestGui").isToggled())
        		//return;
        	
            this.mc.displayGuiScreen(new GuiChest(this.inventory, chestInventory));
            ContainerChest chest = (ContainerChest) mc.thePlayer.openContainer;
            
            EventOpenChest eventOpen = new EventOpenChest(chest);
        	EventC.dispatch(eventOpen);
        }
    ...
    public void onLivingUpdate()
    {
        ...
        if (isUsingItem() && !isRiding() && 
        	      !(Hime.instance.moduleManager.getModule("NoSlow").isToggled())) {
