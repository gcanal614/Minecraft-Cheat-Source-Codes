...
public class TTFFontRenderer {
    ...
    private void renderString(String text, float x, float y2, final int color, final boolean shadow) {
        if (text == "" || text.length() == 0) {
        	...
        	if(Hime.instance.moduleManager.getModule("NameProtect").isToggled() && Minecraft.getMinecraft().theWorld != null && Minecraft.getMinecraft().thePlayer != null) {
              if(Hime.instance.settingsManager.getSettingByName("Streamer Mode").getValBoolean()) {
                ...
                for(NetworkPlayerInfo info : list){
                 text = text.replaceAll(info.getGameProfile().getName(), EnumChatFormatting.OBFUSCATED + info.getGameProfile().getName());
                }
              }else {
                text = text.replaceAll(Minecraft.getMinecraft().thePlayer.getName(), Hime.instance.nameprotect);
              }
            }
        	if(Minecraft.getMinecraft().theWorld != null && Minecraft.getMinecraft().thePlayer != null) {
           //TODO TEST
             if(Hime.instance.user.equals("")) {
            	 text = text.replaceAll(Minecraft.getMinecraft().thePlayer.getName(), Minecraft.getMinecraft().thePlayer.getName() + " (§b" + "Default" + "§f)"); 
             }else {
        		text = text.replaceAll(Minecraft.getMinecraft().thePlayer.getName(), Minecraft.getMinecraft().thePlayer.getName() + " (§b" + Hime.instance.user + "§f)");
             }
        	}
        GL11.glPushMatrix();
    ...
    public float getWidth(final String text) {
        ...
        for (int i = 0; i < length; ++i) {
            ...
            if (character == '\u00a7' && i < length) {
                final int index = "0123456789abcdefklmnor".indexOf(text.charAt(i + 1));
                if (index == 17) {
                    characterData = this.boldData;
                    continue;
                }
