...
import net.minecraft.util.ResourceLocation;
	...
	public class BlurUtil {
		...
		public static void blurArea(float x, float y, int width, int height, float intensity, float blurWidth,
				float blurHeight) {
			...
			if (OpenGlHelper.isFramebufferEnabled()) {
				...
				GL11.glScissor(((int)x) * factor, (mc.displayHeight - (((int)y) * factor) - height * factor), width * factor,
						height * factor);
		...
		public static void blurArea(float changeLogY, float changeLogX, int width, int height, float intensity) {
			ScaledResolution scale = new ScaledResolution(mc);
			int factor = scale.getScaleFactor();
			int factor2 = scale.getScaledWidth();
			int factor3 = scale.getScaledHeight();
			...
			GL11.glScissor(((int)changeLogY) * factor, (mc.displayHeight - ((int) (changeLogX * factor)) - height * factor), width * factor,
					(height) * factor);
			GL11.glEnable(GL11.GL_SCISSOR_TEST);
			buffer.bindFramebuffer(true);
			blurShader.loadShaderGroup(mc.timer.renderPartialTicks);
