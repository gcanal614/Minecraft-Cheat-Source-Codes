...
public class TargetStrafe extends Module {
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        Color aaaa = new Color(hud.Start.getColor().getRed(), hud.Start.getColor().getGreen(), hud.Start.getColor().getBlue());
        Color aaaa2 = new Color(hud.Finish.getColor().getRed(), hud.Finish.getColor().getGreen(), hud.Finish.getColor().getBlue());
        ...
        for (Entity entity : getMc().theWorld.getLoadedEntityList()) {
            if (entity instanceof EntityLivingBase) {
                ...
                if (entityLivingBase.isDead || entityLivingBase == getMc().thePlayer || !isTargetable(entityLivingBase, getMc().thePlayer))
                    continue;
                drawCirle(entity, target != null && entity == target ? aaaa : aaaa2, event.getPartialTicks());
            }
        }
    }
    ...
    public void onUpdate(UpdateEvent event) {
        if (!Clarinet.INSTANCE.getModuleManager().getModule("speed").isEnabled() || (!getMc().gameSettings.keyBindJump.isKeyDown() && TargetStrafe.spaceOnly.isEnabled())) {target = null; set = false;return;}
        target = getTarget();
        if (target != null) {
