...
import non.asset.utils.value.impl.StringValue;
...
public class CommandManager {
    ...
    public void initialize() {
        ...
        register(Clientinfo.class);
        register(Panic.class);
    ...
    public void dispatch(final String s) {
        ...
        if (command.length > 1) {
            Module m = Clarinet.INSTANCE.getModuleManager().getModule(command[0]);
            if (m != null) {
                if (command[1].equalsIgnoreCase("help")) {
                    ...
                    Printer.print(m.getLabel() + " bounded to " + Keyboard.getKeyName(m.getKeybind()) + ".");
                    return;
                }
                if (command.length > 2) {
        ...
        Clarinet.INSTANCE.getCommandManager().getCommandMap().values().forEach(c -> {
