            ...
            for (int j = position.getY(); j <= position.getY() + 1 + i; ++j)
            {
                ...
                for (int l = position.getX() - k; l <= position.getX() + k && flag; ++l)
                {
                    for (int i1 = position.getZ() - k; i1 <= position.getZ() + k && flag; ++i1)
                    {
                        if (j >= 0 && j < 256)
                        {
                            Block block = worldIn.getBlockState(blockposmutableblockpos.func_181079_c(l, j, i1)).getBlock();
                ...
                if ((block1 == Blocks.grass || block1 == Blocks.dirt) && position.getY() < 256 - i - 1)
                {
                    ...
                    for (int j2 = position.getY() - 3 + i; j2 <= position.getY() + i; ++j2)
                    {
                        ...
                        for (int i4 = position.getX() - j3; i4 <= position.getX() + j3; ++i4)
                        {
                            ...
                            {
                                blockposmutableblockpos1.func_181079_c(i4, j2, j4);

                                if (worldIn.getBlockState(blockposmutableblockpos1).getBlock().getMaterial() == Material.leaves)
                                {
