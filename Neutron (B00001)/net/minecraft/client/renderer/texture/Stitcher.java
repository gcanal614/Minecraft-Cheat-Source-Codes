...
public class Stitcher
{
    ...
    {
        Stitcher.Holder stitcherholder = new Stitcher.Holder(p_110934_1_, this.mipmapLevelStitcher);
    ...
    {
        Stitcher.Holder[] astitcherholder = (Stitcher.Holder[])((Stitcher.Holder[])this.setStitchHolders.toArray(new Stitcher.Holder[this.setStitchHolders.size()]));
        Arrays.sort((Object[])astitcherholder);

        for (Stitcher.Holder stitcherholder : astitcherholder)
        {
            if (!this.allocateSlot(stitcherholder))
            {
                String s = String.format("Unable to fit: %s, size: %dx%d, atlas: %dx%d, atlasMax: %dx%d - Maybe try a lower resolution resourcepack?", new Object[] {stitcherholder.getAtlasSprite().getIconName(), Integer.valueOf(stitcherholder.getAtlasSprite().getIconWidth()), Integer.valueOf(stitcherholder.getAtlasSprite().getIconHeight()), Integer.valueOf(this.currentWidth), Integer.valueOf(this.currentHeight), Integer.valueOf(this.maxWidth), Integer.valueOf(this.maxHeight)});
                throw new StitcherException(stitcherholder, s);
            }
        }
    ...
    public List getStichSlots()
    {
        ...
        for (Object stitcherslot : this.stitchSlots)
        {
            ((Slot) stitcherslot).getAllStitchSlots(arraylist);
        }

        ArrayList arraylist1 = Lists.newArrayList();

        for (Object stitcherslot10 : arraylist)
        {
            Slot stitcherslot1 = (Slot) stitcherslot10;
            Stitcher.Holder stitcherholder = stitcherslot1.getStitchHolder();
            TextureAtlasSprite textureatlassprite = stitcherholder.getAtlasSprite();
            textureatlassprite.initSprite(this.currentWidth, this.currentHeight, stitcherslot1.getOriginX(), stitcherslot1.getOriginY(), stitcherholder.isRotated());
    ...
    private boolean expandAndAllocateSlot(Stitcher.Holder p_94311_1_)
    {
        ...
        else
        {
            ...
            if (flag1)
            {
                ...
                stitcherslot = new Stitcher.Slot(this.currentWidth, 0, p_94311_1_.getWidth(), this.currentHeight);
                this.currentWidth += p_94311_1_.getWidth();
            }
            else
            {
                stitcherslot = new Stitcher.Slot(0, this.currentHeight, this.currentWidth, p_94311_1_.getHeight());
                this.currentHeight += p_94311_1_.getHeight();
            }

            stitcherslot.addSlot(p_94311_1_);
    ...
    public static class Slot
    {
            ...
            else
            {
                ...
                if (i <= this.width && j <= this.height)
                {
                    ...
                    else
                    {
                        ...
                        for (Object stitcherslot : this.subSlots)
                        {
                            if (((Slot) stitcherslot).addSlot(holderIn))
                            {
                                return true;
                            }
                        }
        ...
        public void getAllStitchSlots(List p_94184_1_)
        {
            ...
            else if (this.subSlots != null)
            {
                for (Object stitcherslot : this.subSlots)
                {
                    ((Slot) stitcherslot).getAllStitchSlots(p_94184_1_);
                }
            }
        }
