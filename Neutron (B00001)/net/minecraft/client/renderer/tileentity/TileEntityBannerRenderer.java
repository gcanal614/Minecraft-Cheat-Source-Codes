...
public class TileEntityBannerRenderer extends TileEntitySpecialRenderer<TileEntityBanner>
{
    ...
    private ResourceLocation func_178463_a(TileEntityBanner bannerObj)
    {
        ...
        {
            TileEntityBannerRenderer.TimedBannerTexture tileentitybannerrenderertimedbannertexture = (TileEntityBannerRenderer.TimedBannerTexture)DESIGNS.get(s);
            ...
            if (tileentitybannerrenderertimedbannertexture == null)
            {
                if (DESIGNS.size() >= 256)
                {
                    ...
                    while (iterator.hasNext())
                    {
                        ...
                        TileEntityBannerRenderer.TimedBannerTexture tileentitybannerrenderertimedbannertexture1 = (TileEntityBannerRenderer.TimedBannerTexture)DESIGNS.get(s1);

                        if (i - tileentitybannerrenderertimedbannertexture1.systemTime > 60000L)
                        {
                            Minecraft.getMinecraft().getTextureManager().deleteTexture(tileentitybannerrenderertimedbannertexture1.bannerTexture);
                            iterator.remove();
                        }
                    }
                ...
                for (TileEntityBanner.EnumBannerPattern tileentitybannerenumbannerpattern : list1)
                {
                    list2.add("textures/entity/banner/" + tileentitybannerenumbannerpattern.getPatternName() + ".png");
                }

                tileentitybannerrenderertimedbannertexture = new TileEntityBannerRenderer.TimedBannerTexture();
                tileentitybannerrenderertimedbannertexture.bannerTexture = new ResourceLocation(s);
                Minecraft.getMinecraft().getTextureManager().loadTexture(tileentitybannerrenderertimedbannertexture.bannerTexture, new LayeredColorMaskTexture(BANNERTEXTURES, list2, list));
                DESIGNS.put(s, tileentitybannerrenderertimedbannertexture);
            }
