...
public class EntityZombie extends EntityMob
{
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, IEntityLivingData livingdata)
    {
        ...
        {
            EntityZombie.GroupData entityzombiegroupdata = (EntityZombie.GroupData)livingdata;

            if (entityzombiegroupdata.isVillager)
            {
                this.setVillager(true);
            }

            if (entityzombiegroupdata.isChild)
            {
                this.setChild(true);
        ...
        if (this.rand.nextFloat() < 0.01F)
        {
            ...
            for (int k = (int)this.posX - 4; k < (int)this.posX + 4 && j < 14; ++k)
            {
                for (int l = (int)this.posY - 4; l < (int)this.posY + 4 && j < 14; ++l)
                {
                    ...
                    {
                        Block block = this.worldObj.getBlockState(blockposmutableblockpos.func_181079_c(k, l, i1)).getBlock();
