...
public abstract class EntityLivingBase extends Entity {
    ...
    public void fall(float distance, float damageMultiplier) {
        ...
        if (i > 0) {
            ...
            if (block.getMaterial() != Material.air) {
                Block.SoundType blocksoundtype = block.stepSound;
                this.playSound(blocksoundtype.getStepSound(), blocksoundtype.getVolume() * 0.5F, blocksoundtype.getFrequency() * 0.75F);
            }
        }
    }
    ...
    private int getArmSwingAnimationEnd() {
        boolean animations = (NeutronMain.instance.moduleManager.getModuleOrNull(Animations.class).isEnabled());
        if (animations) {
            if (this == (Minecraft.getMinecraft()).thePlayer)
                return (int)(Animations.swingSpeed.getValue() * (Minecraft.getMinecraft()).timer.getTimerSpeed());
