...
public class EntityPackets1_14 extends LegacyEntityRewriter<Protocol1_13_2To1_14>
{
            ...
            public void registerMap() {
                ...
                this.handler(new PacketHandler() {
                    ...
                    public void handle(final PacketWrapper wrapper) throws Exception {
                        ...
                        final int entitiyId = wrapper.get((Type<Integer>)Type.INT, 0);
                        final StoredEntityData storedEntity = ((Protocol1_13_2To1_14)EntityPackets1_14.this.protocol).getEntityRewriter().tracker(wrapper.user()).entityData(entitiyId);
    ...
    protected void registerRewrites() {
        ...
        final MetaHandler villagerDataHandler = (event, meta) -> {
            villagerData = (VillagerData)meta.getValue();
        ...
        this.filter().filterFamily(Entity1_14Types.LIVINGENTITY).handler((event, meta) -> {
            index2 = event.index();
            if (index2 == 12) {
                position = (Position)meta.getValue();
                if (position != null) {
                    wrapper = PacketWrapper.create(ClientboundPackets1_13.USE_BED, null, event.user());
