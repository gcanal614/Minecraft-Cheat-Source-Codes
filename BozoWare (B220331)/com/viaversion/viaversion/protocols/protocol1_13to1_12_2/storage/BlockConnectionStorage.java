...
public class BlockConnectionStorage implements StorableObject
{
    ...
    public void store(final int x, final int y, final int z, int blockState) {
        ...
        final NibbleArray nibbleArray = map.getValue();
        if (nibbleArray != null) {
    ...
    public int get(final int x, final int y, final int z) {
        ...
        final NibbleArray nibbleArray = map.getValue();
        return WorldPackets.toNewId((map.getKey()[blockPosition] & 0xFF) << 4 | ((nibbleArray == null) ? 0 : nibbleArray.get(blockPosition)));
    }
    ...
    public void remove(final int x, final int y, final int z) {
        ...
        for (final short entry : map.getKey()) {
            if (entry != 0) {
                return;
            }
        }
