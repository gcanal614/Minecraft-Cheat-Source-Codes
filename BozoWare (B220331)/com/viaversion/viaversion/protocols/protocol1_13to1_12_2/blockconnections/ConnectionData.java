...
public class ConnectionData
{
    ...
    public static void update(final UserConnection user, final Position position) {
        for (final BlockFace face : BlockFace.values()) {
            ...
            final int blockState = ConnectionData.blockConnectionProvider.getBlockData(user, pos.getX(), pos.getY(), pos.getZ());
    ...
    public static void init() {
        ...
        if (!Via.getConfig().isReduceBlockStorageMemory()) {
            ...
            for (final Map.Entry<String, JsonElement> entry : mappingBlockConnections.entrySet()) {
                final int id2 = ConnectionData.keyToId.get(entry.getKey());
