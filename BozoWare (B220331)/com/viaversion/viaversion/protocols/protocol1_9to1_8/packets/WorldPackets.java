    ...
    public static void register(final Protocol protocol) {
        ...
        protocol.registerClientbound(ClientboundPackets1_8.MAP_BULK_CHUNK, null, new PacketRemapper() {
            ...
            public void registerMap() {
                ...
                final int count;
                final ChunkBulkSection[] chunks;
                int i;
                final ClientChunks clientChunks;
                final ChunkBulkSection[] array;
                int length;
                int j = 0;
                ChunkBulkSection chunk;
                CustomByteType customByteType;
                ByteBuf buffer;
                PacketWrapper chunkPacket;
                this.handler(wrapper -> {
                    wrapper.cancel();
                    skyLight = wrapper.read((Type<Boolean>)Type.BOOLEAN);
                    count = wrapper.read((Type<Integer>)Type.VAR_INT);
                    chunks = new ChunkBulkSection[count];
                    for (i = 0; i < count; ++i) {
                    ...
                    for (length = array.length; j < length; ++j) {
                        chunk = array[j];
                        customByteType = new CustomByteType(chunk.getLength());
                        ...
                        try {
                            ...
                            chunkPacket = PacketWrapper.create(ClientboundPackets1_8.CHUNK_DATA, buffer, wrapper.user());
