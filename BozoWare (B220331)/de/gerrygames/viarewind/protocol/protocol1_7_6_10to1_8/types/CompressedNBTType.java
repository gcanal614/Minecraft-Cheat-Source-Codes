...
public class CompressedNBTType extends Type<CompoundTag>
{
    ...
    public CompoundTag read(final ByteBuf buffer) throws IOException {
        ...
        final ByteBuf compressed = buffer.readSlice((int)length);
        final GZIPInputStream gzipStream = new GZIPInputStream((InputStream)new ByteBufInputStream(compressed));
        try {
    ...
    public void write(final ByteBuf buffer, final CompoundTag nbt) throws Exception {
        ...
        try {
            final GZIPOutputStream gzipStream = new GZIPOutputStream((OutputStream)new ByteBufOutputStream(compressedBuf));
            try {
