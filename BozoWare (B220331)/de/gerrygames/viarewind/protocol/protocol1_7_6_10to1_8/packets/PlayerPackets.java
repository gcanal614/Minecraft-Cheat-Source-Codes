...
public class PlayerPackets
{
    public static void register(final Protocol1_7_6_10TO1_8 protocol) {
        ...
        ((Protocol<ClientboundPackets1_8, ClientboundPackets1_7, S1, S2>)protocol).registerClientbound(ClientboundPackets1_8.RESOURCE_PACK, null, new PacketRemapper() {
            @Override
            public void registerMap() {
                final PacketWrapper response;
                this.handler(packetWrapper -> {
                    packetWrapper.cancel();
                    response = packetWrapper.create(ServerboundPackets1_8.RESOURCE_PACK_STATUS);
                    response.write(Type.STRING, (String)packetWrapper.read((Type<T>)Type.STRING));
                    response.write(Type.VAR_INT, 2);
        ...
        ((AbstractProtocol<C1, C2, S1, ServerboundPackets1_7>)protocol).registerServerbound(ServerboundPackets1_7.PLUGIN_MESSAGE, new PacketRemapper() {
            ...
            public void registerMap() {
                ...
                this.handler(packetWrapper -> {
                    channel = packetWrapper.get(Type.STRING, 0);
                    length = packetWrapper.read((Type<Short>)Type.SHORT);
                    if (channel.equalsIgnoreCase("MC|ItemName")) {
                        customByteType = new CustomByteType(length);
                        data = packetWrapper.read((Type<byte[]>)customByteType);
                        name = new String(data, StandardCharsets.UTF_8);
                        buf = packetWrapper.user().getChannel().alloc().buffer();
                        Type.STRING.write(buf, name);
                        data2 = new byte[buf.readableBytes()];
                        buf.readBytes(data2);
                        buf.release();
                        packetWrapper.write(Type.REMAINING_BYTES, data2);
                        ...
                        book = packetWrapper.read(Types1_7_6_10.COMPRESSED_NBT_ITEM);
                        tag = book.tag();
                        if (tag != null && tag.contains("pages")) {
                            for (pages = tag.get("pages"), i = 0; i < pages.size(); ++i) {
                                page = pages.get(i);
                                value = page.getValue();
                                value2 = ChatUtil.legacyToJson(value);
