...
public class Block
{
    ...
    public boolean isVisuallyOpaque() {
        if (BozoWare.getInstance().getModuleManager().getModuleByClass.apply(XRay.class).isModuleToggled()) {
            return getIdFromBlock(this) == 16;
        }
        return this.blockMaterial.blocksMovement() && this.isFullCube();
    }
    ...
    public boolean shouldSideBeRendered(final IBlockAccess worldIn, final BlockPos pos, final EnumFacing side) {
        if (BozoWare.getInstance().getModuleManager().getModuleByClass.apply(XRay.class).isModuleToggled()) {
            final XRay xray = BozoWare.getInstance().getModuleManager().getModuleByClass.apply(XRay.class);
            final int myid = getIdFromBlock(this);
            for (final Integer block : xray.getBlocks()) {
                if (myid == block) {
                    return true;
                }
                if (myid == 1 || myid == 3 || myid == 13) {
                    return false;
                }
            }
        }
    ...
    public void addCollisionBoxesToList(final World worldIn, final BlockPos pos, final IBlockState state, final AxisAlignedBB mask, final List<AxisAlignedBB> list, final Entity collidingEntity) {
        ...
        final EventAABB event = new EventAABB(this, pos, axisalignedbb);
