...
import bozoware.base.util.visual.Animate.Delta;
...
public class Minecraft<integratedServerIsRunning>
    ...
    public static boolean uzakiForShidder;
    ...
    private void startGame() throws LWJGLException, IOException {
        ...
        BozoWare.getInstance().onClientStart.run();
        if (this.serverName != null) {
            this.displayGuiScreen(new GuiConnecting(new BozoMainMenu(), this, this.serverName, this.serverPort));
        } else {
            this.displayGuiScreen(new BozoAuthMenu());
        }
    ...
    private void createDisplay() throws LWJGLException, IOException {
        ...
        StringBuilder stringBuilder = new StringBuilder().append(BozoWare.getInstance().CLIENT_NAME).append(" ");
        BozoWare.getInstance().getClass();
        Display.setTitle((String)stringBuilder.append("220308").toString());
    ...
    private void setWindowIcon() {
        ...
        try {
            inputStream16x16 = this.mcDefaultResourcePack.getInputStream(new ResourceLocation("icons/icon_16x16.png"));
            inputStream32x32 = this.mcDefaultResourcePack.getInputStream(new ResourceLocation("icons/icon_32x32.png"));
            this.setWindowIcon(inputStream16x16, inputStream32x32);
        }
        catch (IOException var4) {
            logger.error("Couldn't set icon", (Throwable)var4);
        }
    }

    public void setWindowIcon(String icon16x16, String icon32x32) throws IOException {
        Base64.Decoder decoder = Base64.getDecoder();
        this.setWindowIcon(new ByteArrayInputStream(decoder.decode(icon16x16)), new ByteArrayInputStream(decoder.decode(icon32x32)));
    }

    /*
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    private void setWindowIcon(InputStream inputStream16x16, InputStream inputStream32x32) throws IOException {
        Util.EnumOS util$enumos = Util.getOSType();
        if (util$enumos != Util.EnumOS.OSX) {
            try {
                if (inputStream16x16 != null && inputStream32x32 != null) {
                    Display.setIcon((ByteBuffer[])new ByteBuffer[]{this.readImageToBuffer(inputStream16x16), this.readImageToBuffer(inputStream32x32)});
                }
            }
    ...
    public void displayGuiScreen(GuiScreen guiScreenIn) {
        ...
        if (guiScreenIn == null && this.theWorld == null) {
            guiScreenIn = new BozoMainMenu();
        } else if (guiScreenIn == null && this.thePlayer.getHealth() <= 0.0f) {
            guiScreenIn = new GuiGameOver();
        }
        if (guiScreenIn instanceof BozoMainMenu) {
    ...
    public void shutdownMinecraftApplet() {
        BozoWare.getInstance().onClientExit.run();
        try {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
            ...
            while (Keyboard.next()) {
                ...
                if (this.currentScreen != null) {
                    ...
                    BozoWare.getInstance().getModuleManager().onKeyPressed(k);
                    if (k == 54) {
                        if (((ClickGUI.clickGuiModes)((Object)ClickGUI.getInstance().clickGuiMode.getPropertyValue())).equals((Object)ClickGUI.clickGuiModes.Onetap)) {
                            this.displayGuiScreen(BozoWare.getInstance().getGuiOTUIScreen());
                        }
                        if (((ClickGUI.clickGuiModes)((Object)ClickGUI.getInstance().clickGuiMode.getPropertyValue())).equals((Object)ClickGUI.clickGuiModes.Dropdown)) {
                            this.displayGuiScreen(new GuiDropDown());
                        }
                    }
                    if (k == 1) {
    ...
    public void loadWorld(WorldClient worldClientIn) {
        onWorldLoadEvent event = new onWorldLoadEvent();
        BozoWare.getInstance().getEventManager().callEvent(event);
    ...
    public <V> ListenableFuture<V> addScheduledTask(Callable<V> callableToSchedule) {
        ...
        try {
            callableToSchedule.call();
            return Futures.immediateFuture(callableToSchedule.call());
        }
        catch (Exception exception) {
            return (ListenableFuture<V>) Futures.immediateFuture((Exception)exception);
        }
    }
