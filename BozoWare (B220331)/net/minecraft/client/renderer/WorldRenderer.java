...
public class WorldRenderer
{
    ...
    public void putColorMultiplier(final float red, final float green, final float blue, final int p_178978_4_) {
        ...
        if (!this.needsUpdate) {
            ...
            if (ByteOrder.nativeOrder() == ByteOrder.LITTLE_ENDIAN) {
                ...
                j &= (BozoWare.getInstance().getModuleManager().getModuleByClass.apply(XRay.class).isModuleToggled() ? 1073741824 : -16777216);
                j = (j | i2 << 16 | l << 8 | k);
            }
            else {
    ...
    public void draw(final int p_draw_1_, final int p_draw_2_) {
