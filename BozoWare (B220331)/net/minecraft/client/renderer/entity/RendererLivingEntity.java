...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
    ...
    protected void renderModel(final T entitylivingbaseIn, final float p_77036_2_, final float p_77036_3_, final float p_77036_4_, final float p_77036_5_, final float p_77036_6_, final float p_77036_7_) {
        ...
        if (flag || flag2) {
            ...
            if (BozoWare.getInstance().getModuleManager().getModuleByClass.apply(Chams.class).isModuleToggled() && Chams.getInstance().chamsXD.getPropertyValue().equals(Chams.chamsMode.Colored) && entitylivingbaseIn.isEntityAlive() && !entitylivingbaseIn.isInvisible() && entitylivingbaseIn instanceof EntityPlayer && entitylivingbaseIn != Minecraft.getMinecraft().thePlayer) {
                GL11.glPushAttrib(1048575);
                GL11.glDisable(3008);
                GL11.glDisable(3553);
                GL11.glDisable(2896);
                if (!Chams.getInstance().hudColors.getPropertyValue()) {
                    GlStateManager.color(Objects.requireNonNull(Chams.getInstance().color.getPropertyValue()).getRed() / 255.0f, Chams.getInstance().color.getPropertyValue().getGreen() / 255.0f, Chams.getInstance().color.getPropertyValue().getBlue() / 255.0f);
                }
                else {
                    GlStateManager.color(Objects.requireNonNull(new Color(HUD.getInstance().bozoColor).getRed() / 255.0f), new Color(HUD.getInstance().bozoColor).getGreen() / 255.0f, new Color(HUD.getInstance().bozoColor).getBlue() / 255.0f);
                }
                GL11.glEnable(3042);
    ...
    public void renderName(final T entity, final double x, final double y, final double z) {
        if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, entity, this, x, y, z)) {
            if (this.canRenderName(entity)) {
                ...
                if (d0 < f * f) {
                    final RenderNametagEvent event = new RenderNametagEvent(entity, entity.getDisplayName().getFormattedText());
