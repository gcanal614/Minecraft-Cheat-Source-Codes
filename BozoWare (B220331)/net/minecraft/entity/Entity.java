...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            final double d5 = y;
            double d6 = z;
            final boolean flag = (BozoWare.getInstance().getModuleManager().getModuleByClass.apply(BlockFly.class).isModuleToggled() && BlockFly.getInstance().swBool.getPropertyValue() && this.onGround) || (this.onGround && this.isSneaking() && this instanceof EntityPlayer);
            if (flag) {
            ...
            if (this.stepHeight > 0.0f && flag2 && (d4 != x || d6 != z)) {
                final PlayerStepEvent playerStepEvent = new PlayerStepEvent(this.stepHeight);
                if (this instanceof EntityPlayerSP) {
                    BozoWare.getInstance().getEventManager().callEvent(playerStepEvent);
                }
                final double d8 = x;
                final double d9 = y;
                final double d10 = z;
                ...
                this.setEntityBoundingBox(this.getEntityBoundingBox().offset(0.0, y, 0.0));
                if (this instanceof EntityPlayerSP) {
                    final double blockHeight = playerStepEvent.getStepHeight() + y;
                    if (blockHeight % 0.015625 == 0.0) {
                        playerStepEvent.setHeightStepped(blockHeight);
                        playerStepEvent.setPost();
                        BozoWare.getInstance().getEventManager().callEvent(playerStepEvent);
                    }
                }
