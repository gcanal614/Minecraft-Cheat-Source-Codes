...
public class MCPViaPlatform implements ViaPlatform<UUID>
{
    ...
    public PlatformTask runSync(final Runnable runnable, final long ticks) {
        return new FutureTaskId((Future<?>)ViaMCP.getInstance().getEventLoop().schedule(() -> this.runSync(runnable), ticks * 50L, TimeUnit.MILLISECONDS).addListener((GenericFutureListener)this.errorLogger()));
    }
    
    @Override
    public PlatformTask runRepeatingSync(final Runnable runnable, final long ticks) {
        return new FutureTaskId((Future<?>)ViaMCP.getInstance().getEventLoop().scheduleAtFixedRate(() -> this.runSync(runnable), 0L, ticks * 50L, TimeUnit.MILLISECONDS).addListener((GenericFutureListener)this.errorLogger()));
    }
