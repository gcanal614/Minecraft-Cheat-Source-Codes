...
public class Hud extends ModuleBase {
	...
	public static class sortCustomFont implements Comparator<ModuleBase> {
		@Override
		public int compare(ModuleBase arg0, ModuleBase arg1) {
				if (mc.customFont.getStringWidth(arg0.getDisplayName() + (arg0.getSuffix().length() > 1 ? (" " + arg0.getSuffix()) : "")) > mc.customFont.getStringWidth(arg1.getDisplayName() + (arg1.getSuffix().length() > 1 ? (" " + arg1.getSuffix()) : ""))) {
					return -1;
				} else if (mc.customFont.getStringWidth(arg0.getDisplayName() + (arg0.getSuffix().length() > 1 ? (" " + arg0.getSuffix()) : "")) < mc.customFont.getStringWidth(arg1.getDisplayName() + (arg1.getSuffix().length() > 1 ? (" " + arg1.getSuffix()) : "")))
					return 1;
			return 0;
		}
	}
	...
	public void onEvent(Event event) {
		...
		if(event instanceof EventRender2D) {
			...
			for(ModuleBase mod: Thunderware.moduleManager.getModules()) {
				if(mod.isToggled()) {
					modules.add(mod);
				}
			}
			...
			Color fix = new Color(161, 121, 161);
			Gui.drawRect(0,0,0,0, fix.getRGB());
			mc.fontRendererObj.drawStringWithShadow("T\247fhunderware", 4, 4, getColorInt(5));
			mc.fontRendererObj.drawStringWithShadow("v\247fersion: " + Thunderware.instance.buildVersion, 4, 14, getColorInt(20));
