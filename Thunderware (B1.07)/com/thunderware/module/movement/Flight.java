...
import com.thunderware.events.listeners.EventBoundingBox;
...
import com.thunderware.events.listeners.EventStep;
...
public class Flight extends ModuleBase {
	...
	public void onEvent(Event e) {
		...
		if(e instanceof EventMove) {
			...
			switch(mode.getCurrentValue()) {
				case "Test":
					if(mc.thePlayer.ticksExisted % 3 == 0 && mc.thePlayer.isMoveKeysDown()) {
						double[] boost = MovementUtils.getMotion(speed.getValue().floatValue());
						event.setX(boost[0]);
						event.setZ(boost[1]);
					}
					break;
