            ...
            if(message.startsWith(prefix)) {
                try {
                    ...
                    if (paramArray[0].contains("bind")) {
                        Thunderware.moduleManager.getModuleByName(paramArray[1]).setKey(Keyboard.getKeyIndex(paramArray[2].toUpperCase(Locale.ROOT)));
                    }
			                ...
			                if (message.contains("config")) {
								...
								if(commandmsg.length >= 1) {
									if(commandmsg[1].equalsIgnoreCase("save")) {
										File folder = new File("Thunderware");
										if(!folder.exists()) {
											folder.mkdir();
										}
										File config = new File("Thunderware/" + commandmsg[2] + ".txt");
										config.delete();
										System.out.println(config.getAbsolutePath());
										try {
											if (config.createNewFile()) {
												FileWriter configWrite = new FileWriter(config);
												for(ModuleBase m : Thunderware.moduleManager.getModules()) {
										...
										if (config.exists()) {
											...
											try {
												while((line=br.readLine()) != null)
												{
													...
													ModuleBase module = Thunderware.moduleManager.getModuleByName(splited[0]);
