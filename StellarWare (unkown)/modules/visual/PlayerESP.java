...
import stellar.skid.events.events.UpdateModelEvent;
...
public final class PlayerESP extends AbstractModule {
    ...
    public void onRenderTag(RenderNameTagEvent event) {
        event.setCancelled(filter.contains("Name") && (!onlyTargets.get() || stellarWare.getPlayerManager().hasType(event.getEntity().getName(),
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        for (EntityPlayer entity : this.collectedEntities) {
            if (onlyTargets.get() && !stellarWare.getPlayerManager().hasType(entity.getName(), PlayerManager.EnumPlayerType.TARGET))
                continue;
                    ...
                    if (filter.contains("Name")) {
                        ...
                        if (nameColorType.get().equals("Team")) {
                            ...
                            if (onlyTargets.get()) {
                                ...
                                if (stellarWare.playerManager.hasType(entity.getName().toLowerCase(), PlayerManager.EnumPlayerType.FRIEND)) {
                                    color = FRIEND.getRGB();
                                } else if (stellarWare.playerManager.hasType(entity.getName().toLowerCase(), PlayerManager.EnumPlayerType.TARGET)) {
                                    color = TARGET.getRGB();
...
//                        UserEntity user = stellarWare.getIRCUser(name);
//
