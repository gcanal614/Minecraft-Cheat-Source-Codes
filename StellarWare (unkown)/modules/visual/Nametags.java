...
import stellar.skid.events.events.RenderNameTagEvent;
...
import static stellar.skid.gui.screen.setting.SettingType.COMBOBOX;
import static stellar.skid.modules.PlayerManager.EnumPlayerType.FRIEND;
...
import static stellar.skid.utils.PlayerUtils.inTeamWithMinecraftPlayer;
import static stellar.skid.utils.fonts.impl.Fonts.SFBOLD.SFBOLD_18.SFBOLD_18;
...
public final class Nametags extends AbstractModule {
    ...
    public void onNameTagRender(RenderNameTagEvent event) {
        ...
        event.setCancelled(this.stellarWare.getPlayerManager().getType(event.getEntity().getName()) == TARGET  //
    ...
    private void onRender(Render3DEvent event) {
        ...
        this.validEntities.forEach(player -> {
            if (!player.entity.isEntityAlive() || this.mc.player.getDistanceToEntity(player.entity) > this.renderDistance.get() && StellarWare.getInstance().getPlayerManager().getType(player.entity.getName()) != TARGET) {
                this.validEntities.remove(player);
            }

            if (!this.mc.world.getLoadedEntityList().contains(player.entity) || this.onlyTargets.get() && StellarWare.getInstance().getPlayerManager().getType(player.entity.getName()) != TARGET || player.entity.getEntityID() == this.mc.player.getEntityID() || player.entity.getDisplayName().getFormattedText().contains("NPC") || player.entity.getDisplayName().getUnformattedText().equalsIgnoreCase(player.entity.getName()))
                this.validEntities.remove(player);
    ...
    private class Player {
        ...
        void render() {
            ...
            final PlayerManager.EnumPlayerType type = StellarWare.getInstance().getPlayerManager().getType(this.entity.getName());
...
//            UserEntity user = stellarWare.getIRC().getUserManager().findByNickname(this.entity.getName());
//
