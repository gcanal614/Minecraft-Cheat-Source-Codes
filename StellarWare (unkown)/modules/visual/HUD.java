...
import stellar.skid.utils.notifications.NotificationRenderer;
...
import static stellar.skid.modules.PlayerManager.EnumPlayerType.TARGET;
...
public final class HUD extends AbstractModule {
    ...
    private stellar.skid.utils.fonts.api.FontRenderer font = Fonts.SF.SF_20.SF_20;
    private boolean setFont;

    /* properties @off */
    @Property("client-name")
    private final StringProperty clientName = createString("StellarWare");
    @Property("client-info")
    private final StringProperty clientInfo = createString(StellarWare.getInstance().version);
    @Property("scoreboard")
    ...
    private final ListProperty<String> hudElements = createList("ModuleList", "UserInfo", "ArmorHUD", "Scoreboard", "Bossbar", "PotionHUD", "Coords", "Name", "Info", "Time", "FPS")
            .acceptableValues("ModuleList", "UserInfo", "HotBar", "TargetsList", "ArmorHUD", "Scoreboard", "Bossbar", "PotionHUD", "Coords", "Speed", "Name", "Info", "Time", "FPS", "Inventory");
    @Property("hud-font")
    ...
    @Property("color")
    private final ColorProperty color = createColor(new Color(255, 0, 255).getRGB());
    @Property("inv-x")
    ...
    @Property("notifications-module")
    public static final BooleanProperty nModules = PropertyFactory.booleanFalse();
    ...
    public HUD(@NonNull ModuleManager moduleManager) {
        ...
        Manager.put(new Setting("CLIENT_NAME", "Client Name", TEXTBOX, this, "Client name", clientNameclientName, () -> hudElements.get().contains("Name")));
        Manager.put(new Setting("CLIENT_INFO", "Client Info", TEXTBOX, this, "Client info", clientInfo, () -> hudElements.get().contains("Info")));
        ...
        Manager.put(new Setting("NOTIFICATION-MODULES", "Notification modules", CHECKBOX, this, nModules));
    }
    ...
    public List<AbstractModule> getModules() {
        ...
        Stream<AbstractModule> stream = stellarWare.getModuleManager().getAbstractModules().stream();
    ...
    public void onUpdate(Render2DEvent eventUpdate) {
        ...
        for (AbstractModule m : getModules()) {
            ...
            if (m.isEnabled()) {
                ...
                stellarWare.getModuleManager().getAbstractModules().remove(m);
            } else if (m.offsetX > -2) {
    ...
    private void drawArrayList(Render2DEvent event) {
        ...
        float y;
        final stellar.skid.utils.fonts.api.FontRenderer cFont = font;
        final String hudFont = this.hudFont.get();
            ...
            for (AbstractModule module : modules) {
                ...
                if (backgroundMode.get().equalsIgnoreCase("outline")) {
                    ...
                    if (modules.indexOf(module) == modules.size() - 1) {
                        // Ã�Â½Ã�Â¸Ã�Â¶Ã�Â½Ã‘ï¿½Ã‘ï¿½ Ã‘â€¡Ã�Â°Ã‘ï¿½Ã‘â€šÃ‘Å’
                        Gui.drawRect(
                                width - (module.offsetX + 6) - bgMargin,
                                y + fontHeight + 2,
                                width,
    ...
    private List<Entity> getLoadedTargets() {
        final List<String> players = stellarWare.getPlayerManager().whoHas(TARGET);
