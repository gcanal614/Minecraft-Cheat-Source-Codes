...
import stellar.skid.modules.player.BedBreaker;
import stellar.skid.modules.player.ChestStealer;
import stellar.skid.modules.player.InvManager;
import stellar.skid.utils.Channels;
...
import stellar.skid.utils.pathfinding.AStarCustomPathfinder;
import stellar.skid.utils.pathfinding.Vec3;
import stellar.skid.utils.tasks.FutureTask;
...
import static stellar.skid.utils.notifications.NotificationType.WARNING;
...
public final class ClickGUI extends AbstractModule {
    ...
    @Property("ClickGui")
    private final StringProperty ClickGuiModes = PropertyFactory.createString("DropDown").acceptableValues("Discord", "DropDown");
    ...
    public ClickGUI(@NonNull ModuleManager moduleManager) {
        ...
        Manager.put(new Setting("ClickGuiModes", "Modes ", SettingType.COMBOBOX, this, ClickGuiModes));
    ...
    public void onEnable() {
        if(ClickGuiModes.get().equalsIgnoreCase("DropDown")){
       mc.displayGuiScreen(stellarWare.getDropDownGUI());

            toggle();


        } else if (ClickGuiModes.get().equalsIgnoreCase("Discord")){
            mc.displayGuiScreen(stellarWare.getDiscordGUI());
        ...
        if (event.getState().equals(PacketEvent.State.OUTGOING)) {
            ...
            if (notify) {
                if (event.getPacket() instanceof C03PacketPlayer) {
                    if (mc.player != null) {
                        setLanguage = true;
                        stellarWare.getNotificationManager().pop("StellarWare", "Hypixel bypasses enabled!", NotificationType.INFO);
                    } else {
                        stellarWare.getNotificationManager().pop("StellarWare", "Hypixel bypasses disabled! Contact staff", 6_000, NotificationType.ERROR);
                    }
    ...
    public void onSpawn(SpawnCheckEvent event) {
        if (event.getEntity() instanceof EntityPlayer && stellarWare.getPlayerManager().hasType(event.getEntity().getName(), PlayerManager.EnumPlayerType.TARGET)) {
            stellarWare.getNotificationManager().pop("target detected: " + event.getEntity().getName(), 3000, NotificationType.WARNING);
        }
    }
        ...
        switch (setting.getSettingName()) {
            ...
            case "FLIGHT_HYPIXEL_MODE": {
                if (setting.getStringProperty().equals("Normal")) {
                    StellarWare.getInstance().getNotificationManager().pop("Unsafe fly mode!", "Please consider using safe mode to avoid setting off staff alerts!", 10000, WARNING);
                }
            ...
            case "FLIGHT_SW_BOOST": {
                if (setting.getSettingType() == SettingType.CHECKBOX && setting.getBooleanProperty().get()) {
                    stellarWare.getNotificationManager().pop(setting.getModule().getDisplayName(), "You need bow and arrows in order to use " + setting.getDisplayName(), 3_000, NotificationType.INFO);
                }
    ...
    public void onReceive(PacketEvent event) {
        if (event.getState().equals(PacketEvent.State.INCOMING)) {
            ...
            if (event.getPacket() instanceof S00PacketDisconnect && !stellarWare.getTaskManager().getFutureTasks().isEmpty()) {
                stellarWare.getTaskManager().getFutureTasks().clear();
            }
    ...
    public void onUpdate(PlayerUpdateEvent event) {
        ...
        if (ServerUtils.serverIs(Servers.PRE)) {
            ...
            stellarWare.getNotificationManager().pop("Bedwars", "Using fly may result in a ban, wait", 15000, WARNING);
            getModule(BedBreaker.class).setWhiteListed(null);

            StellarWare.getInstance().getTaskManager().queue(new FutureTask(500) {
                @Override
                public void execute() {
                    for (int x = -20; x < 21; x++) {
                        for (int z = -20; z < 21; z++) {
                            for (int y = -10; y < 12; y++) {
                                ...
                                if (mc.world.getBlockState(pos).getBlock() == Blocks.bed && mc.world.getBlockState(pos).getValue(BlockBed.PART) == BlockBed.EnumPartType.HEAD) {
                                    ...
                                    stellarWare.getNotificationManager().pop("Whitelisted your own bed!", "Whitelisted bed at " + pos.toString(), 3000, NotificationType.INFO);
                                }
                            }
                        }
                    }
    ...
    public void onAutoDisable(PacketEvent event) {
        if (event.getState().equals(PacketEvent.State.OUTGOING)) {
            if (ServerUtils.isHypixel()) {
                ...
                if (ServerUtils.isHypixel()) {
                    if (ViaFabric.clientSideVersion == stellarWare.viaVersion()) {
            ...
            if (ServerUtils.isHypixel()) {
                if (event.getPacket() instanceof S02PacketChat) {
                    ...
                    if (formattedText.equals("\u00A7r\u00A7aSelected language: \u00A7r\u00A7eEnglish\u00A7r")) {
                        ...
                        channel = Channels.PM;
                        stellarWare.getNotificationManager().pop("Hypixel", "You are now in the " + channelName() + " channel", 3_000, NotificationType.INFO);
                        event.setCancelled(true);
                    ...
                    if (packet.getType().equals(S45PacketTitle.Type.TITLE)) {
                        ...
                        if (text.equals("VICTORY!") || text.equals("GAME OVER!")) {
                            if (isEnabled(KillAura.class) && getModule(KillAura.class).getAutoDisable().contains("Game End")) {
                                StellarWare.getInstance().getNotificationManager().pop(getModule(KillAura.class).getName() + " was disabled, because game has ended", 1_500, WARNING);
                                getModule(KillAura.class).toggle();
                            }

                            if (isEnabled(InvManager.class) && getModule(InvManager.class).getAutoDisable().contains("Game End")) {
                                StellarWare.getInstance().getNotificationManager().pop(getModule(InvManager.class).getName() +
                            ...
                            if (isEnabled(ChestStealer.class) && getModule(ChestStealer.class).getAutoDisable().contains("Game End")) {
                                StellarWare.getInstance().getNotificationManager().pop(getModule(ChestStealer.class).getName() +
                                        " was disabled, because game has ended", 1_500, WARNING);
                ...
                if (event.getPacket() instanceof S2FPacketSetSlot) {
                    ...
                    if (packet.getItem().getDisplayName().contains("\u00A7") && packet.getItem().getDisplayName().contains("Spectator")) {
                        if (isEnabled(KillAura.class) && getModule(KillAura.class).getAutoDisable().contains("Death")) {
                            StellarWare.getInstance().getNotificationManager().pop(getModule(KillAura.class).getName() +
    ...
    public void onLoadWorld(LoadWorldEvent event) {
        ...
        if (getModule(KillAura.class).isEnabled() && getModule(KillAura.class).getAutoDisable().contains("World Change")) {
            stellarWare.getNotificationManager().pop(getModule(KillAura.class).getName() + " was disabled, because world changed", 1_500, WARNING);
            getModule(KillAura.class).toggle();
        }

        if (getModule(InvManager.class).isEnabled() && getModule(InvManager.class).getAutoDisable().contains("World Change")) {
            stellarWare.getNotificationManager().pop(getModule(InvManager.class).getName() + " was disabled, because world changed", 1_500, WARNING);
            getModule(InvManager.class).toggle();
        }

        if (getModule(ChestStealer.class).isEnabled() && getModule(ChestStealer.class).getAutoDisable().contains("World Change")) {
            stellarWare.getNotificationManager().pop(getModule(ChestStealer.class).getName() + " was disabled, because world changed", 1_500, WARNING);
            getModule(ChestStealer.class).toggle();
        }
        ...
        if (hubbed) {
            ...
            hubbed = false;
            stellarWare.getTaskManager().queue(new FutureTask(2000) {
                @Override
                public void execute() {
                    ArrayList<Vec3> vec3s = computePath(new Vec3(mc.player.posX, mc.player.posY, mc.player.posZ), new Vec3(coords[0], coords[1], coords[2]));
                    ...
                    stellarWare.getNotificationManager().pop("Done", "Teleporting to " + coords[0] + "/" + coords[1] + "/" + coords[2], 3000, NotificationType.INFO);
                }
