        ...
        if (event.getState().equals(MotionUpdateEvent.State.PRE)) {
            for (Entity playerEntity : entities) {
                if (!mc.world.getPlayerEntities().contains(playerEntity)) {
                    ...
                    stellarWare.getPlayerManager().removeType(playerEntity.getName(), PlayerManager.EnumPlayerType.TARGET);
                }
            }
                ...
                for (int slotIndex = 0; slotIndex < 36; slotIndex++) {
                    ...
                    if (stack != null) {
                        if (Item.getIdFromItem(stack.getItem()) == 17 || Item.getIdFromItem(stack.getItem()) == 162) {
                            ...
                            mc.playerController.windowClick(mc.player.inventoryContainer.windowId, 3, 0, 0, mc.player); //ÐºÑ€Ð°Ñ„Ñ‚ Ñ�Ð»Ð¾Ñ‚ 1-4
            ...
            if (auto_pit.get()) {
                if (auto_tar.get()) {
                            ...
                            if (displayName.contains("\u00A76\u00A7l")) {
                                ...
                                if (split.length > 2) {
                                    ...
                                    if (bounty >= min_bounty.get()) {
                                        if (!entities.contains(playerEntity) && !stellarWare.getPlayerManager().hasType(playerEntity.getName(), PlayerManager.EnumPlayerType.FRIEND)) {
                                            entities.add(playerEntity);
                                            stellarWare.getNotificationManager().pop("Found a bountied player", playerEntity.getName() + " has " + EnumChatFormatting.GOLD + bounty + "g" + EnumChatFormatting.RESET + " on him!", 7000, NotificationType.INFO);
                                        }
                                    }
                                }
                            }
                    ...
                    for (Entity playerEntity : entities) {
                        if (stellarWare.getPlayerManager().hasType(playerEntity.getName(), PlayerManager.EnumPlayerType.FRIEND)) {
                            entities.remove(playerEntity);
                            stellarWare.getPlayerManager().removeType(playerEntity.getName(), PlayerManager.EnumPlayerType.TARGET);
                            break;
                        }
                        ...
                        stellarWare.getPlayerManager().setType(playerEntity.getName(), PlayerManager.EnumPlayerType.TARGET);
    ...
    private void autoplay(PacketEvent packetEventReceive) {
        if (packetEventReceive.getPacket() instanceof S02PacketChat) {
            ...
            if (command.startsWith("/play ")) {
                ...
                StellarWare.getInstance().getNotificationManager().pop("Sending you to the next game in ", delay.get() * 1000, NotificationType.INFO);

                StellarWare.getInstance().getTaskManager().queue(new FutureTask(this.delay.get() * 1_000) {
        ...
        if (ServerUtils.isHypixel()) {
            if (event.getState().equals(PacketEvent.State.INCOMING)) {
                if (ServerUtils.serverIs(Servers.PIT)) {
                    ...
                    if (send) {
                        stellarWare.getTaskManager().queue(new FutureTask(5100) {
                            @Override
                            public void execute() {
                                mc.player.sendChatMessage(".tp " + tpX + " " + tpY + " " + tpZ + " " + "goteleportnigga");
                                queued = false;
