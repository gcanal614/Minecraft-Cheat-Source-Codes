...
public final class KillAura extends AbstractModule {
    ...
    public KillAura(@NonNull ModuleManager moduleManager) {
        ...
        Manager.put(new Setting("BO_COLOR", "Ranger Color", COLOR_PICKER, this, color, null));
    ...
    public void onEnable() {
    	//StellarWare.getInstance().getNotificationManager().pop("Enabled Killaura", NotificationType.INFO);
        setSuffix(mode.get());
        alpha = 0;
            ...
            if (targetList != null) {
                ...
                if (!isContainsTarget(mode().equalsIgnoreCase("Switch"))) {
                    ...
                    for (Entity entity : targetList) {
                        if (stellarWare.getPlayerManager().hasType(entity.getName(), PlayerManager.EnumPlayerType.TARGET)) {
                            target = entity;
                        }
                    }
                }
    ...
    private boolean rayTraycing(Entity target) {
        return !raytrace.get() || stellarWare.getPlayerManager().hasType(target.getName(), PlayerManager.EnumPlayerType.TARGET) || getPosition() != null || mc.playerController.isBreakingBlock();
    }

    private void attackEntity(Entity target) {
        if (timerAttack.delay(1000L / aps.get().longValue())) {
            EventManager.call(new AttackEvent());
            ArrayList<stellar.skid.utils.pathfinding.Vec3> vec3s = computePath(new stellar.skid.utils.pathfinding.Vec3(mc.player.posX, mc.player.posY, mc.player.posZ), new stellar.skid.utils.pathfinding.Vec3(target.posX, target.posY, target.posZ));

            if (mode.equals("MW Infinite")) {
                for (stellar.skid.utils.pathfinding.Vec3 vec3 : vec3s) {
        ...
        if (event.getState().equals(PacketEvent.State.OUTGOING)) {
            if (blinking) {
                ...
                if (toDispatch.size() > 60) {
                    dispatchPackets();
                    stellarWare.getNotificationManager().pop(getDisplayName(), "Packet buffer overloaded!", 2000, NotificationType.ERROR);
                    blinking = false;
                }
            }
    ...
    public boolean isValidEntity(@NonNull Entity target) {
        ...
        if (target.isEntityAlive() && mc.player.getHealth() > 0.0F && !stellarWare.getPlayerManager()
    ...
    public void onRender3D(Render3DEvent event) {
        if (visRange.get()) {
            ...
            GL11.glColor4f(color.getAwtColor().getRed() , color.getAwtColor().getGreen() , color.getAwtColor().getBlue() , alpha);
            ...
            if (ServerUtils.serverIs(Servers.DUELS) && event.getPacket() instanceof S45PacketTitle) {
                ...
                if (packet.getType().equals(S45PacketTitle.Type.TITLE)) {
                    ...
                    if (text.equals("VICTORY!")) {
                        stellarWare.getNotificationManager().pop("Check chat for stats", 1000, NotificationType.INFO);
    ...
    private ArrayList<stellar.skid.utils.pathfinding.Vec3> computePath(stellar.skid.utils.pathfinding.Vec3 topFrom, stellar.skid.utils.pathfinding.Vec3 to) {
        ...
        int i = 0;
        stellar.skid.utils.pathfinding.Vec3 lastLoc = null;
        stellar.skid.utils.pathfinding.Vec3 lastDashLoc = null;
        ArrayList<stellar.skid.utils.pathfinding.Vec3> path = new ArrayList<>();
        ArrayList<stellar.skid.utils.pathfinding.Vec3> pathFinderPath = pathfinder.getPath();
        for (stellar.skid.utils.pathfinding.Vec3 pathElm : pathFinderPath) {
