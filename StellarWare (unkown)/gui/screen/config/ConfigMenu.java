...
import stellar.skid.commands.impl.ConfigCommand;
import stellar.skid.gui.screen.click.DiscordGUI;
import stellar.skid.gui.screen.click.Scroll;
...
public final class ConfigMenu {
    ...
    public static void drawScreen(int mouseX, int mouseY) {
        final StellarWare stellarWare = StellarWare.getInstance();
        final DiscordGUI discordGUI = stellarWare.getDiscordGUI();
        ...
        final int guiColor = stellarWare.getModuleManager().getModule(ClickGUI.class).getGUIColor();
    ...
    public static void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        if (mouseButton == 0) {
            final StellarWare stellarWare = StellarWare.getInstance();
            final ModuleManager moduleManager = stellarWare.getModuleManager();
            ...
            if (!b && isSave(mouseX, mouseY)) {
                if (message.length() > 25) {
                    StellarWare.getInstance().getNotificationManager().pop("Name is too long!", 2_000, NotificationType.ERROR);
                    break a;
                }
    ...
    public static void keyTyped(char typedChar, int keyCode) {
        if (keyCode == Keyboard.KEY_F5) {
            StellarWare.getInstance().getDiscordGUI().initConfigs();
            return;
        }
    ...
    public static boolean isLoad(int mouseX, int mouseY) {
        final DiscordGUI gui = StellarWare.getInstance().getDiscordGUI();
