...
public final class FriendCommand extends NovoCommand {
    ...
    /* constructors */
    public FriendCommand(@NonNull StellarWare stellarWare) {
        super(stellarWare, "f", "Adds friend", "friend");
    }
    ...
    public void process(String[] args) {
        ...
        if (args.length == 1) {
            ...
            switch (arg.toLowerCase()) {
                case "l":
                case "list": {
                    final List<String> friends = this.stellarWare.getPlayerManager().whoHas(FRIEND);
                ...
                case "clear": {
                    if (this.stellarWare.getPlayerManager().removeType(FRIEND, e -> true)) {
                ...
                default: {
                    final PlayerManager.EnumPlayerType type = this.stellarWare.getPlayerManager().getType(arg);
    ...
    public void add(String name) {
        ...
        final PlayerManager manager = this.stellarWare.getPlayerManager();
        ...
        switch (args.length) { // @off
            ...
            case 2:
                if (args[0].equalsIgnoreCase("add")) {
                    return completeTab(NetHandlerPlayClient.playerInfoMap.values().stream().map(NetworkPlayerInfo::getGameProfile).map(GameProfile::getName).filter(s -> !this.stellarWare.getPlayerManager().hasType(s, FRIEND)), args[1], true);
                } else if (args[0].equalsIgnoreCase("remove")) {
                    return completeTab(this.stellarWare.getPlayerManager().whoHas(FRIEND), args[1], true);
                } else {
                    return null;
                }
            default:
