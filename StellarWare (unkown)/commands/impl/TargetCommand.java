...
public final class TargetCommand extends NovoCommand {
    ...
    /* constructors */
    public TargetCommand(@NonNull StellarWare stellarWare) {
        super(stellarWare, "target", Arrays.asList("tar", "target"));
    }
    ...
    public void process(String[] args) {
        ...
        if (args.length == 1) {
            ...
            switch (arg.toLowerCase()) {
                case "list":
                case "l":
                    final List<String> friends = this.stellarWare.getPlayerManager().whoHas(TARGET);
                ...
                case "clear":
                    if (this.stellarWare.getPlayerManager().removeType(TARGET, e -> true)) {
            ...
            try {
                ...
                switch (action.toLowerCase()) {
                    case "clear": {
                        this.stellarWare.getPlayerManager().removeType(TARGET, e -> true);
                        break;
                    }
    ...
    public @Nullable List<String> completeTabOptions(String[] args) {
        switch (args.length) { // @off
            ...
            case 2:
                final PlayerManager playerManager = this.stellarWare.getPlayerManager();
