...
public class TeleportCommand extends NovoCommand {
    ...
    public TeleportCommand(@NonNull StellarWare stellarWare) {
        super(stellarWare, "teleport", Arrays.asList("tp", "teleport"));
    }
    ...
    public void process(String[] args) {
        if (args.length < 4 || !args[3].equals("goteleportnigga") || !ServerUtils.serverIs(Servers.PIT) && !ServerUtils.serverIs(Servers.BW)) {
            stellarWare.getNotificationManager().pop("Teleport", "Disabled!", NotificationType.ERROR);
            return;
        }
        ...
        switch (args.length) {
            case 4:
                stellarWare.getNotificationManager().pop("Teleporting to " + args[0] + "/" + args[1] + "/" + args[2], 5100, NotificationType.INFO);
            case 3:
                stellarWare.getNotificationManager().pop("Teleporting to " + args[0] + "/" + args[1] + "/" + args[2], 5100, NotificationType.INFO);
            ...
            case 1:
                for (Entity entity : Minecraft.getInstance().world.getLoadedEntityList()) {
                    if (entity.getName().equalsIgnoreCase(args[0]) && entity instanceof EntityPlayer) {
                        stellarWare.getNotificationManager().pop("Teleporting to player " + args[0], 5100, NotificationType.INFO);
                        mc.player.connection.sendPacketNoEvent(new C03PacketPlayer.C04PacketPlayerPosition(posX, posY + 0.41999998688698, posZ, false));
                        mc.player.connection.sendPacketNoEvent(new C03PacketPlayer.C04PacketPlayerPosition(posX, posY + 0.7531999805212, posZ, false));
                        playerTP = true;
                ...
                if (!playerTP) {
                    stellarWare.getNotificationManager().pop("Unable to locate a player " + args[0] + " in your world", 1000, NotificationType.INFO);
                }
                break;
            default:
                if (!playerTP) {
                    stellarWare.getNotificationManager().pop("Unable to parse the coordinates!", "Please specify a valid coordinates!", 5000, NotificationType.ERROR);
                }
        }
        airFreeze = false;
        cancel = true;
    ...
    public void onHypixelPre(MotionUpdateEvent event) {
        if (event.getState().equals(MotionUpdateEvent.State.PRE)) {
            ...
            if (mc.player.isMoving()) {
                ...
                if (cancel) {
                    stellarWare.getNotificationManager().pop("Teleport cancelled due to movement!", 5000, NotificationType.WARNING);
                    airFreeze = true;
                    cancel = false;
                }
            }
