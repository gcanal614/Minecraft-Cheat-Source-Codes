...
public class CustomColors {
    ...
    public static void update() {
        ...
        potionColors = null;
        paletteFormatDefault = CustomColors.getValidProperty();
        ...
        if (Config.isCustomColors()) {
            ...
            CustomColors.readColorProperties();
            blockColormaps = CustomColors.readBlockColormaps(new String[]{s + "custom/", s + "blocks/"}, colorsBlockColormaps);
            CustomColors.updateUseDefaultGrassFoliageColors();
        }
    }
    ...
    private static String getValidProperty() {
        try {
            ...
            if (!list.contains(s)) {
                CustomColors.warn("Invalid value: palette.format=" + s);
                CustomColors.warn("Expected values: " + Config.arrayToString(CustomColormap.FORMAT_STRINGS));
                return "vanilla";
            }
            CustomColors.dbg("palette.format=" + s);
            return s;
        }
    ...
    private static void readColorProperties() {
        try {
            ...
            CustomColors.dbg("Loading mcpatcher/color.properties");
            PropertiesOrdered properties = new PropertiesOrdered();
            properties.load(inputstream);
            inputstream.close();
            ...
            colorsBlockColormaps = CustomColors.readCustomColormaps(properties);
            spawnEggPrimaryColors = CustomColors.readSpawnEggColors(properties, "egg.shell.", "Spawn egg shell");
            spawnEggSecondaryColors = CustomColors.readSpawnEggColors(properties, "egg.spots.", "Spawn egg spot");
            wolfCollarColors = CustomColors.readDyeColors(properties, "collar.", "Wolf collar");
            sheepColors = CustomColors.readDyeColors(properties, "sheep.", "Sheep");
            textColors = CustomColors.readTextColors(properties);
            int[] aint = CustomColors.readMapColors(properties);
            if (aint != null) {
            ...
            potionColors = CustomColors.readPotionColors(properties);
    ...
    private static CustomColormap[] readCustomColormaps(Properties props) {
        ...
        for (String s6 : astring = map.keySet().toArray(new String[0])) {
    ...
    private static CustomColormap[][] readBlockColormaps(String[] basePaths, CustomColormap[] basePalettes) {
        ...
        for (Object s : astring) {
            ...
            try {
                ...
                PropertiesOrdered properties = new PropertiesOrdered();
                properties.load(inputstream);
                CustomColormap customcolormap = new CustomColormap(properties, (String)s, 256, 256, paletteFormatDefault);
        ...
        return CustomColors.blockListToArray(list);
    }
    ...
    private static CustomColormap getBlockColormap(IBlockState blockState) {
        CustomColormap[] acustomcolormap;
        if (blockColormaps == null) {
            return null;
        }
        ...
        if (i >= 0 && i < blockColormaps.length && (acustomcolormap = blockColormaps[i]) != null) {
    ...
    private static int getRedstoneColor(IBlockState blockState) {
        ...
        int i = CustomColors.getRedstoneLevel(blockState);
    ...
    public static void updateReddustFX(EntityFX fx, IBlockAccess blockAccess, double x, double y, double z) {
        if (redstoneColors != null) {
            ...
            int i = CustomColors.getRedstoneLevel(iblockstate);
    ...
    private static int[] readTextColors(Properties props) {
        ...
        for (Object e : props.keySet()) {
            String s = (String)e;
            String s1 = props.getProperty(s);
            if (!s.startsWith("text.code.")) continue;
        ...
        CustomColors.dbg("Text colors: " + i);
        return aint;
    }
    ...
    private static int[] readPotionColors(Properties props) {
        ...
        for (Object e : props.keySet()) {
            String s = (String)e;
            String s1 = props.getProperty(s);
            if (!s.startsWith("potion.")) continue;
        ...
        CustomColors.dbg("Potion colors: " + i);
        return aint;
    }
    ...
    private static int getPotionId(String name) {
        ...
        for (Potion potion : apotion = Potion.potionTypes) {
