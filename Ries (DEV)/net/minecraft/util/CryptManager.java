...
public class CryptManager {
    ...
    public static byte[] getServerIdHash(String serverId, PublicKey publicKey, SecretKey secretKey) {
        try {
            return CryptManager.digestOperation(serverId.getBytes("ISO_8859_1"), secretKey.getEncoded(), publicKey.getEncoded());
        }
    ...
    private static byte[] cipherOperation(int opMode, Key key, byte[] data) {
        try {
            return Objects.requireNonNull(CryptManager.createTheCipherInstance(opMode, key.getAlgorithm(), key)).doFinal(data);
        }
        catch (BadPaddingException | IllegalBlockSizeException illegalblocksizeexception) {
