...
ITickable {
    ...
    private static boolean isInventoryEmpty(IInventory inventoryIn, EnumFacing side) {
        if (inventoryIn instanceof ISidedInventory) {
            int[] aint;
            ISidedInventory isidedinventory = (ISidedInventory)inventoryIn;
            for (int j : aint = isidedinventory.getSlotsForFace(side)) {
                if (isidedinventory.getStackInSlot(j) == null) continue;
                return false;
            }
        } else {
    ...
    public static boolean captureDroppedItems(IHopper p_145891_0_) {
        ...
        if (iinventory != null) {
            ...
            if (iinventory instanceof ISidedInventory) {
                ...
                for (int j : aint = isidedinventory.getSlotsForFace(enumfacing)) {
                    if (!TileEntityHopper.pullItemFromSlot(p_145891_0_, iinventory, j, enumfacing)) continue;
                    return true;
                }
            } else {
