...
implements IInventory {
    ...
    private int storePartialItemStack(ItemStack itemStackIn) {
        ...
        if (j >= 0) {
            ...
            if ((k = Math.min(i, this.mainInventory[j].getMaxStackSize() - this.mainInventory[j].stackSize)) > this.getInventoryStackLimit() - this.mainInventory[j].stackSize) {
    ...
    public boolean hasItemStack(ItemStack itemStackIn) {
        for (ItemStack value : this.armorInventory) {
            if (value == null || !value.isItemEqual(itemStackIn)) continue;
            return true;
        }
        for (ItemStack stack : this.mainInventory) {
            if (stack == null || !stack.isItemEqual(itemStackIn)) continue;
            return true;
        }
        return false;
    }
