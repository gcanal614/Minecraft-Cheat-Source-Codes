...
public class Chunk {
    ...
    private void relightBlock(int x, int y, int z) {
        ...
        for (j = Math.max(y, i); j > 0 && this.getBlockLightOpacity(x, j - 1, z) == 0; --j) {
        }
        if (j != i) {
    ...
    public Random getRandomWithSeed(long seed) {
        return new Random(this.worldObj.getSeed() + (long)this.xPosition * (long)this.xPosition * 4987142L + (long)this.xPosition * 5947611L + (long)this.zPosition * (long)this.zPosition * 4392871L + (long)this.zPosition * 389711L ^ seed);
    }
