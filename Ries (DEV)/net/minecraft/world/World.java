...
implements IBlockAccess {
    ...
    public boolean canBlockFreeze(BlockPos pos, boolean noWaterAdj) {
        ...
        if (!(f > 0.15f || pos.getY() < 0 || pos.getY() >= 256 || this.getLightFor(EnumSkyBlock.BLOCK, pos) >= 10 || (block = (iblockstate = this.getBlockState(pos)).getBlock()) != Blocks.water && block != Blocks.flowing_water || iblockstate.getValue(BlockLiquid.LEVEL) != 0)) {
            if (!noWaterAdj) {
                return true;
            }
