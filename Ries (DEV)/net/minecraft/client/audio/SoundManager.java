    ...
    public void playSound(ISound sound) {
        if (this.loaded) {
            if (this.sndSystem.getMasterVolume() <= 0.0f) {
                logger.debug(LOG_MARKER, "Skipped playing soundEvent: {}, master volume was zero", (Object)sound.getSoundLocation());
            } else {
                SoundEventAccessorComposite soundeventaccessorcomposite = this.sndHandler.getSound(sound.getSoundLocation());
                if (soundeventaccessorcomposite == null) {
                    logger.warn(LOG_MARKER, "Unable to play unknown soundEvent: {}", (Object)sound.getSoundLocation());
                } else {
                    SoundPoolEntry soundpoolentry = soundeventaccessorcomposite.cloneEntry();
                    if (soundpoolentry == SoundHandler.missing_sound) {
                        logger.warn(LOG_MARKER, "Unable to play empty soundEvent: {}", (Object)soundeventaccessorcomposite.getSoundEventLocation());
                    } else {
                        ...
                        if (f2 == 0.0f) {
                            logger.debug(LOG_MARKER, "Skipped playing sound {}, volume was zero.", (Object)resourcelocation);
                        } else {
                            ...
                            logger.debug(LOG_MARKER, "Playing sound {} for event {} as channel {}", (Object)soundpoolentry.getSoundPoolEntryLocation(), (Object)soundeventaccessorcomposite.getSoundEventLocation(), (Object)s);
