    ...
    public void tryAcceptPlayer() {
        ...
        if ((s = this.server.getConfigurationManager().allowUserToConnect(this.networkManager.getRemoteAddress(), this.loginGameProfile)) != null) {
            ...
            if (this.server.getNetworkCompressionTreshold() >= 0 && !this.networkManager.isLocalChannel()) {
                this.networkManager.sendPacket(new S03PacketEnableCompression(this.server.getNetworkCompressionTreshold()), (GenericFutureListener<? extends Future<? super Void>>)((ChannelFutureListener)p_operationComplete_1_ -> this.networkManager.setCompressionTreshold(this.server.getNetworkCompressionTreshold())), new GenericFutureListener[0]);
            }
    ...
    public void processEncryptionResponse(C01PacketEncryptionResponse packetIn) {
        ...
        new Thread("User Authenticator #" + AUTHENTICATOR_THREAD_ID.incrementAndGet()){
            ...
            public void run() {
                ...
                try {
                    String s = new BigInteger(Objects.requireNonNull(CryptManager.getServerIdHash("", NetHandlerLoginServer.this.server.getKeyPair().getPublic(), NetHandlerLoginServer.this.secretKey))).toString(16);
