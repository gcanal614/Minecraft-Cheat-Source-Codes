...
@ModuleInfo(internalName="Timer", name="Timer", desc="Allows you to change the general speed of the game.", category=Module.Category.MISCELLANEOUS)
public class Timer
extends Module {
    ...
    public Timer(int key, boolean enabled) {
        super(key, enabled);
        Ries.INSTANCE.getSettingManager().addSlider(this, INTERNAL_GAMESPEED_VALUE, GAMESPEED_VALUE_SETTING_NAME, 2.0, 0.01, 20.0);
    }

    @Override
    public void onEnable() {
        this.oldGameSpeed = Timer.MC.timer.getTimerSpeed();
    }

    @Override
    public void onDisable() {
        Timer.MC.timer.setTimerSpeed(this.oldGameSpeed);
    }
    ...
    public void onMotion(EventMotionUpdate e) {
        ...
        if (e.getState() == EventMotionUpdate.State.PRE && (double)Timer.MC.timer.getTimerSpeed() != gameSpeed) {
            Timer.MC.timer.setTimerSpeed((float)gameSpeed);
        }
    }
}
