...
@ModuleInfo(internalName="ChatBot", name="Chat Bot", desc="Allows you to host a chat bot AI with commands.", category=Module.Category.MISCELLANEOUS, exp=true)
...
extends Module {
    ...
    public void onPacket(EventPacket e) {
        if (e.getPacket() instanceof S02PacketChat) {
            S02PacketChat packet = (S02PacketChat)PacketUtils.getPacket(e.getPacket());
            ...
            for (NetworkPlayerInfo networkplayerinfo : EntityUtils.getPlayerList()) {
                if (!message.toLowerCase().contains(EntityUtils.getPlayerName(networkplayerinfo).toLowerCase())) continue;
