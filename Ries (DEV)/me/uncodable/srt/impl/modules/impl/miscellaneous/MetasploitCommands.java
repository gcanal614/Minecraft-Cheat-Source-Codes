...
@ModuleInfo(internalName="MetasploitCommands", name="Metasploit Commands", desc="Allows you to use Metasploit modules in SRT.\nExperience the true hacker-man experience.", category=Module.Category.MISCELLANEOUS, legit=true)
public class MetasploitCommands
extends Module {
    ...
    public void onChat(EventSendChatMessage e) {
        if (e.getMessage().startsWith("->")) {
            ...
            for (MetasploitCommand command : Ries.INSTANCE.getMetasploitCommandManager().getMetasploitCommands()) {
                if (!e.getMessage().toLowerCase().split("->")[1].startsWith(command.getInfo().name().toLowerCase()) || hit) continue;
