...
@ModuleInfo(internalName="ClickGUIEXPERIMENTAL", name="Click GUI New", desc="Allows you to view a clickable GUI that you can click on.\nWhat else is there to say?", category=Module.Category.VISUAL, legit=true)
...
extends Module {
    ...
    public void onEnable() {
        ...
        if (this.renderTabs.isEmpty()) {
            int i;
            for (i = 0; i < halfCategoryValues; ++i) {
                this.renderTabs.add(new RenderTab(Module.Category.values()[i], 10 + 94 * i, 10, 100 + 94 * i, 30));
            }
            for (i = halfCategoryValues; i < Module.Category.values().length; ++i) {
