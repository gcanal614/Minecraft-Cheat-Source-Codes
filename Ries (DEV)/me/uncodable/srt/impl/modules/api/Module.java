...
implements IListener {
    ...
    public void toggle() {
        ...
        if (Ries.INSTANCE.getModuleManager().getModuleByName("Safeguard").isEnabled() && !this.getInfo().legit()) {
            Ries.INSTANCE.msg(String.format("Module \"%s\" was disabled to provide a fair gameplay experience. You cannot toggle illegitimate modules while \u00a7bsafeguard\u00a77 is enabled.", this.getInfo().name()));
            this.enabled = false;
            return;
        }
        if (this.enabled) {
