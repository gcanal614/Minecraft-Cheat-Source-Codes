...
public class RenderTab {
    ...
    public void setup() {
        if (!this.setup) {
            for (int moduleIndex = 0; moduleIndex < Ries.INSTANCE.getModuleManager().getModulesInCategory(this.category).size(); ++moduleIndex) {
                this.renderModules.add(new RenderModule(this, Ries.INSTANCE.getModuleManager().getModulesInCategory(this.category).get(moduleIndex), this.startX + 2, this.startY + 20 * (moduleIndex + 1) + 2, this.endX - 2, this.endY + 20 * (moduleIndex + 1) - 2));
            }
    ...
    public void render(int mouseX, int mouseY, int width, int height) {
        ...
        RenderTab.MC.fontRendererObj.drawStringWithShadow(this.category.getCategoryName(), this.startX + 4, this.startY + 6, 0xFFFFFF);
        GL11.glPushMatrix();
        GL11.glTranslatef((float)(this.endX - 8), (float)(this.startY + 6), (float)0.0f);
        if (this.dropped) {
            GL11.glTranslatef((float)5.0f, (float)2.0f, (float)0.0f);
            GL11.glRotatef((float)90.0f, (float)0.0f, (float)0.0f, (float)1.0f);
        }
        RenderTab.MC.fontRendererObj.drawStringWithShadow(">", 0.0f, 0.0f, 0xFFFFFF);
        GL11.glPopMatrix();
        if (this.dropped) {
            Gui.drawRect(this.firstModule.getStartX() - 2, this.firstModule.getStartY() - 2, this.lastModule.getEndX() + 2, this.lastModule.getEndY() + 2, Integer.MIN_VALUE);
    ...
    public void onClicked(int mouseX, int mouseY, int mouseButton) {
        switch (mouseButton) {
            case 0: {
                if (!this.hoverOver(mouseX, mouseY)) break;
