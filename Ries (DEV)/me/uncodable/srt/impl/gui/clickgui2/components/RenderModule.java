...
import me.uncodable.srt.impl.gui.clickgui2.components.RenderSetting;
...
public class RenderModule {
    ...
    public void setup() {
        if (!this.setup) {
            for (int settingIndex = 0; settingIndex < Ries.INSTANCE.getSettingManager().getAllSettings(this.module).size(); ++settingIndex) {
                this.renderSettings.add(new RenderSetting(this, Ries.INSTANCE.getSettingManager().getAllSettings(this.module).get(settingIndex), this.endX + 4, this.startY + 16 * settingIndex + 2, this.endX + 90 - 4, this.endY + 16 * settingIndex - 2));
            }
    ...
    public void render(int mouseX, int mouseY) {
        ...
        if (this.binding) {
            RenderModule.MC.fontRendererObj.drawStringWithShadow("Press a key...", this.startX + 4, this.startY + 5, 0xFFFFFF);
        } else {
            RenderModule.MC.fontRendererObj.drawStringWithShadow(String.format("%s %s", this.module.getInfo().name(), this.module.getPrimaryKey() == 0 ? "" : String.format("(%s)", Keyboard.getKeyName((int)this.module.getPrimaryKey()))), this.startX + 4, this.startY + 5, 0xFFFFFF);
        }
        if (!Ries.INSTANCE.getSettingManager().getAllSettings(this.module).isEmpty()) {
            GL11.glPushMatrix();
            GL11.glTranslatef((float)(this.endX - 8), (float)(this.startY + 5), (float)0.0f);
            if (this.dropped) {
                GL11.glTranslatef((float)5.0f, (float)2.0f, (float)0.0f);
                GL11.glRotatef((float)90.0f, (float)0.0f, (float)0.0f, (float)1.0f);
            }
            RenderModule.MC.fontRendererObj.drawStringWithShadow(">", 0.0f, 0.0f, 0xFFFFFF);
            GL11.glPopMatrix();
        }
        if (this.dropped) {
            Gui.drawRect(this.firstSetting.getStartX() - 2, this.firstSetting.getStartY() - 4, this.lastSetting.getEndX() + 2, this.lastSetting.getEndY() + 4, Integer.MIN_VALUE);
