...
public class CraftingManager {
    ...
    public ShapedRecipes addRecipe(ItemStack stack, Object ... recipeComponents) {
        block9: {
            ...
            j = 0;
            k = 0;
            if (!(recipeComponents[i] instanceof String[])) ** GOTO lbl20
            astring = (String[])recipeComponents[i++];
            l = 0;
            while (l < astring.length) {
                s2 = astring[l];
                ++k;
                j = s2.length();
...
lbl20:
        ...
        while (i1 < j * k) {
            c0 = s.charAt(i1);
            aitemstack[i1] = map.containsKey(Character.valueOf(c0)) != false ? ((ItemStack)map.get(Character.valueOf(c0))).copy() : null;
            ++i1;
        }
        shapedrecipes = new ShapedRecipes(j, k, aitemstack, stack);
