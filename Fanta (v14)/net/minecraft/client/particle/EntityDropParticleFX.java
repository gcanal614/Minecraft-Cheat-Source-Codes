...
extends EntityFX {
    ...
    public void onUpdate() {
        ...
        if (this.onGround) {
            if (this.materialType == Material.water) {
                this.setDead();
                worldObj.spawnParticle(EnumParticleTypes.WATER_SPLASH, this.posX, this.posY, this.posZ, 0.0, 0.0, 0.0, new int[0]);
            } else {
        ...
        if ((material = (iblockstate = worldObj.getBlockState(blockpos = new BlockPos(this))).getBlock().getMaterial()).isLiquid() || material.isSolid()) {
            double d1;
            double d0 = 0.0;
