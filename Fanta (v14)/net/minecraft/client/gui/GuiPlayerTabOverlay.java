...
extends Gui {
    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, ScoreObjective scoreObjectiveIn) {
        boolean flag;
        int l3;
        this.translate.interpolate(width, GuiContainer.height, 4.0);
        NetHandlerPlayClient nethandlerplayclient = this.mc.thePlayer.sendQueue;
        List list = field_175252_a.sortedCopy(nethandlerplayclient.getPlayerInfoMap());
        int i = 0;
        int j = 0;
        for (NetworkPlayerInfo networkplayerinfo : list) {
            String prefix = GuiPlayerTabOverlay.getIrcPrefix(this.getPlayerName(networkplayerinfo), networkplayerinfo.getGameProfile());
            int k = this.mc.fontRendererObj.getStringWidth(String.valueOf(prefix) + this.getPlayerName(networkplayerinfo));
            i = Math.max(i, k);
        ...
        if (list1 != null) {
            GuiPlayerTabOverlay.drawRect(width / 2 - l1 / 2 - 1, k1 - 1, width / 2 + l1 / 2 + 1, k1 + list1.size() * this.mc.fontRendererObj.getFontHeight(), Integer.MIN_VALUE);
            for (String s3 : list1) {
        ...
        while (k4 < l3) {
            ...
            if (k4 < list.size()) {
                ...
                s1 = String.valueOf(GuiPlayerTabOverlay.getIrcPrefix(s1, gameprofile)) + s1;
                if (flag) {
        ...
        if (list2 != null) {
            k1 = k1 + i4 * 9 + 1;
            GuiPlayerTabOverlay.drawRect(width / 2 - l1 / 2 - 1, k1 - 1, width / 2 + l1 / 2 + 1, k1 + list2.size() * this.mc.fontRendererObj.getFontHeight(), Integer.MIN_VALUE);
            for (String s4 : list2) {
