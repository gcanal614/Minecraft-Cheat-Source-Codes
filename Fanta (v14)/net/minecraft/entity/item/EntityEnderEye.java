...
extends Entity {
    ...
    public void moveTowards(BlockPos p_180465_1_) {
        ...
        this.despawnTimer = 0;
        this.shatterOrDrop = rand.nextInt(5) > 0;
    }
    ...
    public void onUpdate() {
        ...
        if (!EntityEnderEye.worldObj.isRemote) {
        ...
        if (this.isInWater()) {
            ...
            worldObj.spawnParticle(EnumParticleTypes.PORTAL, this.posX - this.motionX * (double)f3 + rand.nextDouble() * 0.6 - 0.3, this.posY - this.motionY * (double)f3 - 0.5, this.posZ - this.motionZ * (double)f3 + rand.nextDouble() * 0.6 - 0.3, this.motionX, this.motionY, this.motionZ, new int[0]);
        }
        if (!EntityEnderEye.worldObj.isRemote) {
            this.setPosition(this.posX, this.posY, this.posZ);
            ++this.despawnTimer;
            if (this.despawnTimer > 80 && !EntityEnderEye.worldObj.isRemote) {
                this.setDead();
                if (this.shatterOrDrop) {
                    worldObj.spawnEntityInWorld(new EntityItem(worldObj, this.posX, this.posY, this.posZ, new ItemStack(Items.ender_eye)));
                } else {
