...
implements IHopper {
    ...
    public boolean interactFirst(EntityPlayer playerIn) {
        if (!EntityMinecartHopper.worldObj.isRemote) {
    ...
    public void onUpdate() {
        super.onUpdate();
        if (!EntityMinecartHopper.worldObj.isRemote && this.isEntityAlive() && this.getBlocked()) {
    ...
    public boolean func_96112_aD() {
        ...
        List<Entity> list = worldObj.getEntitiesWithinAABB(EntityItem.class, this.getEntityBoundingBox().expand(0.25, 0.0, 0.25), EntitySelectors.selectAnything);
        if (list.size() > 0) {
