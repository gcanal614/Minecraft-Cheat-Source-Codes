...
extends Entity {
    ...
    public void onUpdate() {
        ...
        if (block.getMaterial() == Material.air) {
            ...
            if (this.fallTime++ == 0) {
                ...
                } else if (!EntityFallingBlock.worldObj.isRemote) {
                    this.setDead();
                    return;
                }
            }
            ...
            if (!EntityFallingBlock.worldObj.isRemote) {
                ...
                if (this.onGround) {
                    ...
                    if (worldObj.getBlockState(blockpos1).getBlock() != Blocks.piston_extension) {
                        ...
                        if (!this.canSetAsBlock) {
                            if (worldObj.canBlockBePlaced(block, blockpos1, true, EnumFacing.UP, null, null) && !BlockFalling.canFallInto(worldObj, blockpos1.down()) && worldObj.setBlockState(blockpos1, this.fallTile, 3)) {
                                TileEntity tileentity;
                                if (block instanceof BlockFalling) {
                                    ((BlockFalling)block).onEndFalling(worldObj, blockpos1);
                                }
                                if (this.tileEntityData != null && block instanceof ITileEntityProvider && (tileentity = worldObj.getTileEntity(blockpos1)) != null) {
                            ...
                            } else if (this.shouldDropItem && worldObj.getGameRules().getBoolean("doEntityDrops")) {
                                this.entityDropItem(new ItemStack(block, 1, block.damageDropped(this.fallTile)), 0.0f);
                            }
                        }
                    }
                } else if (this.fallTime > 100 && !EntityFallingBlock.worldObj.isRemote && (blockpos1.getY() < 1 || blockpos1.getY() > 256) || this.fallTime > 600) {
                    if (this.shouldDropItem && worldObj.getGameRules().getBoolean("doEntityDrops")) {
                        this.entityDropItem(new ItemStack(block, 1, block.damageDropped(this.fallTile)), 0.0f);
                    }
                    this.setDead();
                }
    ...
    public void fall(float distance, float damageMultiplier) {
        ...
        if (this.hurtEntities && (i = MathHelper.ceiling_float_int(distance - 1.0f)) > 0) {
            ArrayList list = Lists.newArrayList(worldObj.getEntitiesWithinAABBExcludingEntity(this, this.getEntityBoundingBox()));
            ...
            if (flag && (double)rand.nextFloat() < (double)0.05f + (double)i * 0.05) {
