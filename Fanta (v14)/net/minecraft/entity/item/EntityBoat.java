...
extends Entity {
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        ...
        if (!EntityBoat.worldObj.isRemote && !this.isDead) {
            boolean flag;
    ...
    public void onUpdate() {
        ...
        if (d9 > 0.2975) {
            ...
            while ((double)k < 1.0 + d9 * 60.0) {
                ...
                double d6 = (double)(rand.nextInt(2) * 2 - 1) * 0.7;
                if (rand.nextBoolean()) {
                    double d7 = this.posX - d2 * d5 * 0.8 + d4 * d6;
                    double d8 = this.posZ - d4 * d5 * 0.8 - d2 * d6;
                    worldObj.spawnParticle(EnumParticleTypes.WATER_SPLASH, d7, this.posY - 0.125, d8, this.motionX, this.motionY, this.motionZ, new int[0]);
                } else {
                    double d24 = this.posX + d2 + d4 * d5 * 0.7;
                    double d25 = this.posZ + d4 - d2 * d5 * 0.7;
                    worldObj.spawnParticle(EnumParticleTypes.WATER_SPLASH, d24, this.posY - 0.125, d25, this.motionX, this.motionY, this.motionZ, new int[0]);
                }
                ++k;
            }
        }
        if (EntityBoat.worldObj.isRemote && this.isBoatEmpty) {
            ...
            this.setRotation(this.rotationYaw, this.rotationPitch);
            if (!EntityBoat.worldObj.isRemote) {
                List<Entity> list = worldObj.getEntitiesWithinAABBExcludingEntity(this, this.getEntityBoundingBox().expand(0.2f, 0.0, 0.2f));
