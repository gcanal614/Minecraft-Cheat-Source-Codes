...
extends Entity {
    ...
    public void onUpdate() {
        ...
        if (EntityEnderCrystal.worldObj.provider instanceof WorldProviderEnd && worldObj.getBlockState(new BlockPos(i, j, k)).getBlock() != Blocks.fire) {
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        ...
        if (!this.isDead && !EntityEnderCrystal.worldObj.isRemote) {
            this.health = 0;
            if (this.health <= 0) {
                this.setDead();
                if (!EntityEnderCrystal.worldObj.isRemote) {
