...
extends EntityLivingBase {
            ...
            block22: {
                block23: {
                    block21: {
                        ...
                        if (EntityArmorStand.worldObj.isRemote || player.isSpectator()) break block20;
                        i = 0;
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        if (EntityArmorStand.worldObj.isRemote) {
            return false;
        }
    ...
    private void playParticles() {
        if (worldObj instanceof WorldServer) {
            ((WorldServer)worldObj).spawnParticle(EnumParticleTypes.BLOCK_DUST, this.posX, this.posY + (double)this.height / 1.5, this.posZ, 10, (double)(this.width / 4.0f), (double)(this.height / 4.0f), (double)(this.width / 4.0f), 0.05, Block.getStateId(Blocks.planks.getDefaultState()));
        }
    }
    ...
    private void dropContents() {
        ...
        while (i < this.contents.length) {
            if (this.contents[i] != null && this.contents[i].stackSize > 0) {
                if (this.contents[i] != null) {
                    Block.spawnAsEntity(worldObj, new BlockPos(this).up(), this.contents[i]);
                }
