...
IMob {
    ...
    public void onLivingUpdate() {
        if (EntityDragon.worldObj.isRemote) {
            ...
            if (f1 <= -0.3f && f >= -0.3f && !this.isSilent()) {
                worldObj.playSound(this.posX, this.posY, this.posZ, "mob.enderdragon.wings", 5.0f, 0.8f + rand.nextFloat() * 0.3f, false);
            }
        }
        ...
        if (this.getHealth() <= 0.0f) {
            ...
            worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, this.posX + (double)f11, this.posY + 2.0 + (double)f13, this.posZ + (double)f14, 0.0, 0.0, 0.0, new int[0]);
        } else {
            ...
            if (this.isAIDisabled()) {
                ...
                if (EntityDragon.worldObj.isRemote) {
                    ...
                    if (this.target != null) {
                        ...
                        this.targetX += rand.nextGaussian() * 2.0;
                        this.targetZ += rand.nextGaussian() * 2.0;
                    }
                ...
                if (!EntityDragon.worldObj.isRemote && this.hurtTime == 0) {
                    this.collideWithEntities(worldObj.getEntitiesWithinAABBExcludingEntity(this, this.dragonPartWing1.getEntityBoundingBox().expand(4.0, 2.0, 4.0).offset(0.0, -2.0, 0.0)));
                    this.collideWithEntities(worldObj.getEntitiesWithinAABBExcludingEntity(this, this.dragonPartWing2.getEntityBoundingBox().expand(4.0, 2.0, 4.0).offset(0.0, -2.0, 0.0)));
                    this.attackEntitiesInList(worldObj.getEntitiesWithinAABBExcludingEntity(this, this.dragonPartHead.getEntityBoundingBox().expand(1.0, 1.0, 1.0)));
                }
    ...
    private void updateDragonEnderCrystal() {
        ...
        if (rand.nextInt(10) == 0) {
            float f = 32.0f;
            List<EntityEnderCrystal> list = worldObj.getEntitiesWithinAABB(EntityEnderCrystal.class, this.getEntityBoundingBox().expand(f, f, f));
    ...
    private void setNewTarget() {
        ...
        ArrayList list = Lists.newArrayList(EntityDragon.worldObj.playerEntities);
        ...
        if (rand.nextInt(2) == 0 && !list.isEmpty()) {
            this.target = (Entity)list.get(rand.nextInt(list.size()));
        } else {
            double d2;
            double d1;
            double d0;
            ...
            do {
                this.targetX = 0.0;
                this.targetY = 70.0f + rand.nextFloat() * 50.0f;
                this.targetZ = 0.0;
                this.targetX += (double)(rand.nextFloat() * 120.0f - 60.0f);
                this.targetZ += (double)(rand.nextFloat() * 120.0f - 60.0f);
    ...
    private boolean destroyBlocksInAABB(AxisAlignedBB p_70972_1_) {
        ...
        if (flag1) {
            ...
            worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, d0, d1, d2, 0.0, 0.0, 0.0, new int[0]);
        }
        return flag;
    }
    ...
    public boolean attackEntityFromPart(EntityDragonPart dragonPart, DamageSource source, float p_70965_3_) {
        ...
        this.targetX = this.posX + (double)(f1 * 5.0f) + (double)((rand.nextFloat() - 0.5f) * 2.0f);
        this.targetY = this.posY + (double)(rand.nextFloat() * 3.0f) + 1.0;
        this.targetZ = this.posZ - (double)(f2 * 5.0f) + (double)((rand.nextFloat() - 0.5f) * 2.0f);
        this.target = null;
    ...
    protected void onDeathUpdate() {
        ...
        if (this.deathTicks >= 180 && this.deathTicks <= 200) {
            ...
            worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_HUGE, this.posX + (double)f, this.posY + 2.0 + (double)f1, this.posZ + (double)f2, 0.0, 0.0, 0.0, new int[0]);
        }
        ...
        if (!EntityDragon.worldObj.isRemote) {
            if (this.deathTicks > 150 && this.deathTicks % 5 == 0 && flag) {
                ...
                while (i > 0) {
                    ...
                    i -= k;
                    worldObj.spawnEntityInWorld(new EntityXPOrb(worldObj, this.posX, this.posY, this.posZ, k));
                }
            }
        ...
        if (this.deathTicks == 200 && !EntityDragon.worldObj.isRemote) {
            if (flag) {
                int j = 2000;
                while (j > 0) {
                    int l = EntityXPOrb.getXPSplit(j);
                    j -= l;
                    worldObj.spawnEntityInWorld(new EntityXPOrb(worldObj, this.posX, this.posY, this.posZ, l));
                }
            }
