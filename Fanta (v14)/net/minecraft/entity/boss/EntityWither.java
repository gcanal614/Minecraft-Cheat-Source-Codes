...
IRangedAttackMob {
    ...
    public void onLivingUpdate() {
        ...
        if (!EntityWither.worldObj.isRemote && this.getWatchedTargetId(0) > 0 && (entity = worldObj.getEntityByID(this.getWatchedTargetId(0))) != null) {
            double d1;
            double d0;
            double d3;
        ...
        while (l < 3) {
            ...
            worldObj.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, d10 + rand.nextGaussian() * (double)0.3f, d2 + rand.nextGaussian() * (double)0.3f, d4 + rand.nextGaussian() * (double)0.3f, 0.0, 0.0, 0.0, new int[0]);
            if (flag && EntityWither.worldObj.rand.nextInt(4) == 0) {
                worldObj.spawnParticle(EnumParticleTypes.SPELL_MOB, d10 + rand.nextGaussian() * (double)0.3f, d2 + rand.nextGaussian() * (double)0.3f, d4 + rand.nextGaussian() * (double)0.3f, (double)0.7f, (double)0.7f, 0.5, new int[0]);
            }
            ++l;
        }
        if (this.getInvulTime() > 0) {
            int i1 = 0;
            while (i1 < 3) {
                worldObj.spawnParticle(EnumParticleTypes.SPELL_MOB, this.posX + rand.nextGaussian() * 1.0, this.posY + (double)(rand.nextFloat() * 3.3f), this.posZ + rand.nextGaussian() * 1.0, (double)0.7f, (double)0.7f, (double)0.9f, new int[0]);
                ++i1;
            }
        }
    }
    ...
    protected void updateAITasks() {
        if (this.getInvulTime() > 0) {
            ...
            if (j1 <= 0) {
                worldObj.newExplosion(this, this.posX, this.posY + (double)this.getEyeHeight(), this.posZ, 7.0f, false, worldObj.getGameRules().getBoolean("mobGriefing"));
            ...
            while (i < 3) {
                if (this.ticksExisted >= this.field_82223_h[i - 1]) {
                    int k1;
                    this.field_82223_h[i - 1] = this.ticksExisted + 10 + rand.nextInt(10);
                    if (worldObj.getDifficulty() == EnumDifficulty.NORMAL || worldObj.getDifficulty() == EnumDifficulty.HARD) {
                        ...
                        if (k3 > 15) {
                            float f = 10.0f;
                            float f1 = 5.0f;
                            double d0 = MathHelper.getRandomDoubleInRange(rand, this.posX - (double)f, this.posX + (double)f);
                            double d1 = MathHelper.getRandomDoubleInRange(rand, this.posY - (double)f1, this.posY + (double)f1);
                            double d2 = MathHelper.getRandomDoubleInRange(rand, this.posZ - (double)f, this.posZ + (double)f);
                    ...
                    if ((k1 = this.getWatchedTargetId(i)) > 0) {
                        ...
                        if (entity != null && entity.isEntityAlive() && this.getDistanceSqToEntity(entity) <= 900.0 && this.canEntityBeSeen(entity)) {
                            if (entity instanceof EntityPlayer && ((EntityPlayer)entity).capabilities.disableDamage) {
                                ...
                                this.field_82223_h[i - 1] = this.ticksExisted + 40 + rand.nextInt(20);
                                this.field_82224_i[i - 1] = 0;
                            }
                        } else {
                            this.updateWatchedTargetId(i, 0);
                        ...
                        List<EntityLivingBase> list = worldObj.getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().expand(20.0, 8.0, 20.0), Predicates.and(attackEntitySelector, EntitySelectors.NOT_SPECTATING));
                        int j2 = 0;
                        while (j2 < 10 && !list.isEmpty()) {
            ...
            if (this.blockBreakCounter > 0) {
                ...
                if (this.blockBreakCounter == 0 && worldObj.getGameRules().getBoolean("mobGriefing")) {
    ...
    private void launchWitherSkullToEntity(int p_82216_1_, EntityLivingBase p_82216_2_) {
        this.launchWitherSkullToCoords(p_82216_1_, p_82216_2_.posX, p_82216_2_.posY + (double)p_82216_2_.getEyeHeight() * 0.5, p_82216_2_.posZ, p_82216_1_ == 0 && rand.nextFloat() < 0.001f);
    }
    ...
    protected void dropFewItems(boolean p_70628_1_, int p_70628_2_) {
        ...
        if (!EntityWither.worldObj.isRemote) {
            for (EntityPlayer entityplayer : worldObj.getEntitiesWithinAABB(EntityPlayer.class, this.getEntityBoundingBox().expand(50.0, 100.0, 50.0))) {
