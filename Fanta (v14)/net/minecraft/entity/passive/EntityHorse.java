...
implements IInvBasic {
    ...
    public void dropChests() {
        if (!EntityHorse.worldObj.isRemote && this.isChested()) {
    ...
    public void fall(float distance, float damageMultiplier) {
        ...
        if ((i = MathHelper.ceiling_float_int((distance * 0.5f - 3.0f) * damageMultiplier)) > 0) {
            ...
            if ((block = worldObj.getBlockState(new BlockPos(this.posX, this.posY - 0.2 - (double)this.prevRotationYaw, this.posZ)).getBlock()).getMaterial() != Material.air && !this.isSilent()) {
    ...
    private void updateHorseSlots() {
        if (!EntityHorse.worldObj.isRemote) {
    ...
    protected String getLivingSound() {
        int i;
        this.openHorseMouth();
        if (rand.nextInt(10) == 0 && !this.isMovementBlocked()) {
            this.makeHorseRear();
        }
    ...
    private void setHorseTexturePaths() {
        ...
        if (i == 0) {
            ...
            this.texturePrefix = String.valueOf(this.texturePrefix) + HORSE_TEXTURES_ABBR[k];
            if (l >= horseMarkingTextures.length) {
                this.field_175508_bO = false;
                return;
            }
            this.horseTexturesArray[1] = horseMarkingTextures[l];
            this.texturePrefix = String.valueOf(this.texturePrefix) + HORSE_MARKING_TEXTURES_ABBR[l];
        } else {
        ...
        if (i1 >= horseArmorTextures.length) {
            ...
            this.texturePrefix = String.valueOf(this.texturePrefix) + HORSE_ARMOR_TEXTURES_ABBR[i1];
    ...
    public void openGUI(EntityPlayer playerEntity) {
        if (!EntityHorse.worldObj.isRemote && (this.riddenByEntity == null || this.riddenByEntity == playerEntity) && this.isTame()) {
            this.horseChest.setCustomName(this.getName());
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (itemstack != null) {
            ...
            if (!flag && this.canCarryChest() && !this.isChested() && itemstack.getItem() == Item.getItemFromBlock(Blocks.chest)) {
                ...
                this.playSound("mob.chickenplop", 1.0f, (rand.nextFloat() - rand.nextFloat()) * 0.2f + 1.0f);
                flag = true;
                this.initHorseChest();
            }
    ...
    public void onLivingUpdate() {
        ...
        if (!EntityHorse.worldObj.isRemote) {
            ...
            if (rand.nextInt(900) == 0 && this.deathTime == 0) {
                this.heal(1.0f);
            }
            if (!this.isEatingHaystack() && this.riddenByEntity == null && rand.nextInt(300) == 0 && worldObj.getBlockState(new BlockPos(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY) - 1, MathHelper.floor_double(this.posZ))).getBlock() == Blocks.grass) {
    ...
    public void onUpdate() {
        super.onUpdate();
        if (EntityHorse.worldObj.isRemote && this.dataWatcher.hasObjectChanged()) {
        ...
        if (!EntityHorse.worldObj.isRemote && this.jumpRearingCounter > 0 && ++this.jumpRearingCounter > 20) {
    ...
    private void dropItemsInChest(Entity entityIn, AnimalChest animalChestIn) {
        if (animalChestIn != null && !EntityHorse.worldObj.isRemote) {
            int i = 0;
    ...
    public EntityAgeable createChild(EntityAgeable ageable) {
        ...
        if (k == 0) {
            ...
            int l = i1 < 4 ? this.getHorseVariant() & 0xFF : (i1 < 8 ? entityhorse.getHorseVariant() & 0xFF : rand.nextInt(7));
            int j1 = rand.nextInt(5);
            l = j1 < 2 ? (l |= this.getHorseVariant() & 0xFF00) : (j1 < 4 ? (l |= entityhorse.getHorseVariant() & 0xFF00) : (l |= rand.nextInt(5) << 8 & 0xFF00));
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, IEntityLivingData livingdata) {
        ...
        if (livingdata instanceof GroupData) {
            ...
            j = ((GroupData)livingdata).horseVariant & 0xFF | rand.nextInt(5) << 8;
        } else {
    ...
    protected void spawnHorseParticles(boolean p_110216_1_) {
        ...
        while (i < 7) {
            ...
            worldObj.spawnParticle(enumparticletypes, this.posX + (double)(rand.nextFloat() * this.width * 2.0f) - (double)this.width, this.posY + 0.5 + (double)(rand.nextFloat() * this.height), this.posZ + (double)(rand.nextFloat() * this.width * 2.0f) - (double)this.width, d0, d1, d2, new int[0]);
            ++i;
        }
    }
    ...
    private double getModifiedJumpStrength() {
        return (double)0.4f + rand.nextDouble() * 0.2 + rand.nextDouble() * 0.2 + rand.nextDouble() * 0.2;
    }

    private double getModifiedMovementSpeed() {
        return ((double)0.45f + rand.nextDouble() * 0.3 + rand.nextDouble() * 0.3 + rand.nextDouble() * 0.3) * 0.25;
    }
