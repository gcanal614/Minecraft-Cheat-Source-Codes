...
extends EntityCow {
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (itemstack != null && itemstack.getItem() == Items.shears && this.getGrowingAge() >= 0) {
            this.setDead();
            worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, this.posX, this.posY + (double)(this.height / 2.0f), this.posZ, 0.0, 0.0, 0.0, new int[0]);
            if (!EntityMooshroom.worldObj.isRemote) {
                ...
                int i = 0;
                while (i < 5) {
                    worldObj.spawnEntityInWorld(new EntityItem(worldObj, this.posX, this.posY + (double)this.height, this.posZ, new ItemStack(Blocks.red_mushroom)));
                    ++i;
                }
