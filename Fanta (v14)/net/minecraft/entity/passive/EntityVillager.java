...
INpc {
    ...
    protected void updateAITasks() {
        if (--this.randomTickDivider <= 0) {
            ...
            this.villageObj = worldObj.getVillageCollection().getNearestVillage(blockpos, 32);
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (!flag && this.isEntityAlive() && !this.isTrading() && !this.isChild()) {
            if (!(EntityVillager.worldObj.isRemote || this.buyingList != null && this.buyingList.size() <= 0)) {
    ...
    public void onDeath(DamageSource cause) {
        if (this.villageObj != null) {
            ...
            if (entity != null) {
                ...
                EntityPlayer entityplayer = worldObj.getClosestPlayerToEntity(this, 16.0);
    ...
    public void useRecipe(MerchantRecipe recipe) {
        ...
        if (recipe.getRewardsExp()) {
            worldObj.spawnEntityInWorld(new EntityXPOrb(worldObj, this.posX, this.posY + 0.5, this.posZ, i));
        }
    }

    @Override
    public void verifySellingItem(ItemStack stack) {
        if (!EntityVillager.worldObj.isRemote && this.livingSoundTime > -this.getTalkInterval() + 20) {
            this.livingSoundTime = -this.getTalkInterval();
            if (stack != null) {
                this.playSound("mob.villager.yes", this.getSoundVolume(), this.getSoundPitch());
            } else {
    ...
    private void populateBuyingList() {
        ...
        if (this.careerId != 0 && this.careerLevel != 0) {
            ...
            this.careerId = rand.nextInt(aentityvillager$itradelist.length) + 1;
            this.careerLevel = 1;
        }
        if (this.buyingList == null) {
        ...
        if (j >= 0 && j < aentityvillager$itradelist1.length) {
            ...
            int n = aentityvillager$itradelist2.length;
            int n2 = 0;
            while (n2 < n) {
                ITradeList entityvillager$itradelist = iTradeListArray[n2];
                entityvillager$itradelist.modifyMerchantRecipeList(this.buyingList, rand);
                ++n2;
            }
        }
    }
    ...
    private void spawnParticles(EnumParticleTypes particleType) {
        ...
        while (i < 5) {
            ...
            worldObj.spawnParticle(particleType, this.posX + (double)(rand.nextFloat() * this.width * 2.0f) - (double)this.width, this.posY + 1.0 + (double)(rand.nextFloat() * this.height), this.posZ + (double)(rand.nextFloat() * this.width * 2.0f) - (double)this.width, d0, d1, d2, new int[0]);
            ++i;
        }
    }
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, IEntityLivingData livingdata) {
        ...
        this.setProfession(EntityVillager.worldObj.rand.nextInt(5));
        this.setAdditionalAItasks();
        return livingdata;
    }
    ...
    public void onStruckByLightning(EntityLightningBolt lightningBolt) {
        if (!EntityVillager.worldObj.isRemote && !this.isDead) {
