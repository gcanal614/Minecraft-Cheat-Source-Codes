...
extends EntityTameable {
    ...
    protected String getLivingSound() {
        return this.isAngry() ? "mob.wolf.growl" : (rand.nextInt(3) == 0 ? (this.isTamed() && this.dataWatcher.getWatchableObjectFloat(18) < 10.0f ? "mob.wolf.whine" : "mob.wolf.panting") : "mob.wolf.bark");
    }
    ...
    public void onLivingUpdate() {
        ...
        if (!EntityWolf.worldObj.isRemote && this.isWet && !this.isShaking && !this.hasPath() && this.onGround) {
            this.isShaking = true;
            this.timeWolfIsShaking = 0.0f;
            this.prevTimeWolfIsShaking = 0.0f;
            worldObj.setEntityState(this, (byte)8);
        }
        if (!EntityWolf.worldObj.isRemote && this.getAttackTarget() == null && this.isAngry()) {
            this.setAngry(false);
        }
    }
    ...
    public void onUpdate() {
        ...
        if (this.isWet()) {
            ...
            if (this.timeWolfIsShaking == 0.0f) {
                this.playSound("mob.wolf.shake", this.getSoundVolume(), (rand.nextFloat() - rand.nextFloat()) * 0.2f + 1.0f);
            }
            ...
            if (this.timeWolfIsShaking > 0.4f) {
                ...
                int j = 0;
                while (j < i) {
                    float f1 = (rand.nextFloat() * 2.0f - 1.0f) * this.width * 0.5f;
                    float f2 = (rand.nextFloat() * 2.0f - 1.0f) * this.width * 0.5f;
                    worldObj.spawnParticle(EnumParticleTypes.WATER_SPLASH, this.posX + (double)f1, (double)(f + 0.8f), this.posZ + (double)f2, this.motionX, this.motionY, this.motionZ, new int[0]);
                    ++j;
                }
            }
        }
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (this.isTamed()) {
            ...
            if (this.isOwner(player) && !EntityWolf.worldObj.isRemote && !this.isBreedingItem(itemstack)) {
            ...
            if (!EntityWolf.worldObj.isRemote) {
                if (rand.nextInt(3) == 0) {
                    this.setTamed(true);
                    this.navigator.clearPathEntity();
                    this.setAttackTarget(null);
