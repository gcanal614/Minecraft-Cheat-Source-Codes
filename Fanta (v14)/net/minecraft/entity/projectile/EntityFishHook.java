...
extends Entity {
    ...
    public void handleHookCasting(double p_146035_1_, double p_146035_3_, double p_146035_5_, float p_146035_7_, float p_146035_8_) {
        ...
        p_146035_1_ += rand.nextGaussian() * (double)0.0075f * (double)p_146035_8_;
        p_146035_3_ += rand.nextGaussian() * (double)0.0075f * (double)p_146035_8_;
        p_146035_5_ += rand.nextGaussian() * (double)0.0075f * (double)p_146035_8_;
    ...
    public void onUpdate() {
        ...
        if (this.fishPosRotationIncrements > 0) {
            ...
            if (!EntityFishHook.worldObj.isRemote) {
            ...
            if (this.inGround) {
                if (worldObj.getBlockState(new BlockPos(this.xTile, this.yTile, this.zTile)).getBlock() == this.inTile) {
                    ++this.ticksInGround;
            ...
            Entity entity = null;
            List<Entity> list = worldObj.getEntitiesWithinAABBExcludingEntity(this, this.getEntityBoundingBox().addCoord(this.motionX, this.motionY, this.motionZ).expand(1.0, 1.0, 1.0));
            double d0 = 0.0;
            int i = 0;
            while (i < list.size()) {
                ...
                if (entity1.canBeCollidedWith() && (entity1 != this.angler || this.ticksInAir >= 5)) {
                    ...
                    if (movingobjectposition1 != null && ((d2 = vec31.squareDistanceTo(movingobjectposition1.hitVec)) < d0 || d0 == 0.0)) {
                        entity = entity1;
                        d0 = d2;
                    }
                }
            ...
            if (!this.inGround) {
                ...
                if (!EntityFishHook.worldObj.isRemote && d10 > 0.0) {
                    ...
                    if (this.ticksCatchable > 0) {
                        ...
                        if (this.ticksCatchableDelay <= 0) {
                            ...
                            this.playSound("random.splash", 0.25f, 1.0f + (rand.nextFloat() - rand.nextFloat()) * 0.4f);
                            ...
                            this.ticksCatchable = MathHelper.getRandomIntegerInRange(rand, 10, 30);
                        } else {
                            double d16;
                            this.fishApproachAngle = (float)((double)this.fishApproachAngle + rand.nextGaussian() * 4.0);
                        ...
                        if (rand.nextFloat() < f1) {
                            double d6;
                            double d14;
                            float f9 = MathHelper.randomFloatClamp(rand, 0.0f, 360.0f) * ((float)Math.PI / 180);
                            ...
                            if (block == Blocks.water || block == Blocks.flowing_water) {
                                worldserver.spawnParticle(EnumParticleTypes.WATER_SPLASH, d12, d14, d6, 2 + rand.nextInt(2), (double)0.1f, 0.0, (double)0.1f, 0.0, new int[0]);
                            }
                        }
                        if (this.ticksCaughtDelay <= 0) {
                            this.fishApproachAngle = MathHelper.randomFloatClamp(rand, 0.0f, 360.0f);
                            this.ticksCatchableDelay = MathHelper.getRandomIntegerInRange(rand, 20, 80);
                        }
                    } else {
                        this.ticksCaughtDelay = MathHelper.getRandomIntegerInRange(rand, 100, 900);
                        this.ticksCaughtDelay -= EnchantmentHelper.getLureModifier(this.angler) * 20 * 5;
                    }
                    if (this.ticksCatchable > 0) {
                        this.motionY -= (double)(rand.nextFloat() * rand.nextFloat() * rand.nextFloat()) * 0.2;
                    }
                }
    ...
    public int handleHookRetraction() {
        ...
        if (this.caughtEntity != null) {
            ...
            EntityItem entityitem = new EntityItem(worldObj, this.posX, this.posY, this.posZ, this.getFishingResult());
            ...
            EntityPlayer.worldObj.spawnEntityInWorld(new EntityXPOrb(EntityPlayer.worldObj, this.angler.posX, this.angler.posY + 0.5, this.angler.posZ + 0.5, rand.nextInt(6) + 1));
            i = 1;
        }
        if (this.inGround) {
            i = 2;
    ...
    private ItemStack getFishingResult() {
        float f = EntityFishHook.worldObj.rand.nextFloat();
