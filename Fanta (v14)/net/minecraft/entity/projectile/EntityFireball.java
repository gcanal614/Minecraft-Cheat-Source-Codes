...
extends Entity {
    ...
    public EntityFireball(World worldIn, EntityLivingBase shooter, double accelX, double accelY, double accelZ) {
        ...
        this.motionY = 0.0;
        this.motionX = 0.0;
        double d0 = MathHelper.sqrt_double((accelX += rand.nextGaussian() * 0.4) * accelX + (accelY += rand.nextGaussian() * 0.4) * accelY + (accelZ += rand.nextGaussian() * 0.4) * accelZ);
        this.accelerationX = accelX / d0 * 0.1;
        this.accelerationY = accelY / d0 * 0.1;
        this.accelerationZ = accelZ / d0 * 0.1;
    }
    ...
    public void onUpdate() {
        if (EntityFireball.worldObj.isRemote || (this.shootingEntity == null || !this.shootingEntity.isDead) && worldObj.isBlockLoaded(new BlockPos(this))) {
            super.onUpdate();
            this.setFire(1);
            if (this.inGround) {
            ...
            while (i < list.size()) {
                ...
                if (entity1.canBeCollidedWith() && (!entity1.isEntityEqual(this.shootingEntity) || this.ticksInAir >= 25)) {
                    ...
                    if (movingobjectposition1 != null && ((d1 = vec3.squareDistanceTo(movingobjectposition1.hitVec)) < d0 || d0 == 0.0)) {
                        entity = entity1;
                        d0 = d1;
                    }
                }
            ...
            if (this.isInWater()) {
                ...
                while (j < 4) {
                    float f3 = 0.25f;
                    worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX - this.motionX * (double)f3, this.posY - this.motionY * (double)f3, this.posZ - this.motionZ * (double)f3, this.motionX, this.motionY, this.motionZ, new int[0]);
                    ++j;
                }
                f2 = 0.8f;
            }
            ...
            worldObj.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, this.posX, this.posY + 0.5, this.posZ, 0.0, 0.0, 0.0, new int[0]);
            this.setPosition(this.posX, this.posY, this.posZ);
        } else {
            this.setDead();
        }
