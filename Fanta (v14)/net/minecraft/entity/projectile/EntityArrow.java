...
implements IProjectile {
    ...
    public void setThrowableHeading(double x, double y, double z, float velocity, float inaccuracy) {
        ...
        y /= (double)f;
        z /= (double)f;
        x += rand.nextGaussian() * (double)(rand.nextBoolean() ? -1 : 1) * (double)0.0075f * (double)inaccuracy;
        y += rand.nextGaussian() * (double)(rand.nextBoolean() ? -1 : 1) * (double)0.0075f * (double)inaccuracy;
        z += rand.nextGaussian() * (double)(rand.nextBoolean() ? -1 : 1) * (double)0.0075f * (double)inaccuracy;
    ...
    public void onUpdate() {
        ...
        if ((block = (iblockstate = worldObj.getBlockState(blockpos = new BlockPos(this.xTile, this.yTile, this.zTile))).getBlock()).getMaterial() != Material.air) {
        ...
        if (this.inGround) {
            ...
            while (i < list.size()) {
                ...
                if (entity1.canBeCollidedWith() && (entity1 != this.shootingEntity || this.ticksInAir >= 5)) {
                    ...
                    if (movingobjectposition1 != null && ((d1 = vec31.squareDistanceTo(movingobjectposition1.hitVec)) < d0 || d0 == 0.0)) {
                        entity = entity1;
                        d0 = d1;
                    }
                }
            ...
            if (movingobjectposition != null) {
                if (movingobjectposition.entityHit != null) {
                    ...
                    if (this.inTile.getMaterial() != Material.air) {
                        this.inTile.onEntityCollidedWithBlock(worldObj, blockpos1, iblockstate1, this);
                    }
                }
            }
            if (this.getIsCritical()) {
                int k = 0;
                while (k < 4) {
                    worldObj.spawnParticle(EnumParticleTypes.CRIT, this.posX + this.motionX * (double)k / 4.0, this.posY + this.motionY * (double)k / 4.0, this.posZ + this.motionZ * (double)k / 4.0, -this.motionX, -this.motionY + 0.2, -this.motionZ, new int[0]);
                    ++k;
                }
            }
            ...
            if (this.isInWater()) {
                ...
                while (i1 < 4) {
                    float f8 = 0.25f;
                    worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX - this.motionX * (double)f8, this.posY - this.motionY * (double)f8, this.posZ - this.motionZ * (double)f8, this.motionX, this.motionY, this.motionZ, new int[0]);
                    ++i1;
                }
                f4 = 0.6f;
            }
    ...
    public void onCollideWithPlayer(EntityPlayer entityIn) {
        if (!EntityArrow.worldObj.isRemote && this.inGround && this.arrowShake <= 0) {
            boolean flag;
