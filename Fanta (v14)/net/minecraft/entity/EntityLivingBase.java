...
extends Entity {
    ...
    protected void updateFallState(double y, boolean onGroundIn, Block blockIn, BlockPos pos) {
        ...
        if (!EntityLivingBase.worldObj.isRemote && this.fallDistance > 3.0f && onGroundIn) {
            ...
            if (block.getMaterial() != Material.air) {
                ...
                int i = (int)(150.0 * d0);
                ((WorldServer)worldObj).spawnParticle(EnumParticleTypes.BLOCK_DUST, this.posX, this.posY, this.posZ, i, 0.0, 0.0, 0.0, (double)0.15f, Block.getStateId(iblockstate));
            }
        }
    ...
    public void onEntityUpdate() {
        ...
        EntityLivingBase.worldObj.theProfiler.startSection("livingEntityBaseTick");
        boolean flag = this instanceof EntityPlayer;
        if (this.isEntityAlive()) {
            double d0;
            if (this.isEntityInsideOpaqueBlock()) {
                this.attackEntityFrom(DamageSource.inWall, 1.0f);
            } else if (flag && !worldObj.getWorldBorder().contains(this.getEntityBoundingBox()) && (d0 = worldObj.getWorldBorder().getClosestDistance(this) + worldObj.getWorldBorder().getDamageBuffer()) < 0.0) {
                this.attackEntityFrom(DamageSource.inWall, Math.max(1, MathHelper.floor_double(-d0 * worldObj.getWorldBorder().getDamageAmount())));
            }
        }
        if (this.isImmuneToFire() || EntityLivingBase.worldObj.isRemote) {
            this.extinguish();
        }
                ...
                if (!(this.canBreatheUnderwater() || this.isPotionActive(Potion.waterBreathing.id) || flag1)) {
                    ...
                    if (this.getAir() == -20) {
                        ...
                        while (i < 8) {
                            ...
                            worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX + (double)f, this.posY + (double)f1, this.posZ + (double)f2, this.motionX, this.motionY, this.motionZ, new int[0]);
                            ++i;
                        }
                        this.attackEntityFrom(DamageSource.drown, 2.0f);
                    }
                }
                if (!EntityLivingBase.worldObj.isRemote && this.isRiding() && this.ridingEntity instanceof EntityLivingBase) {
    ...
    protected void onDeathUpdate() {
        ...
        if (this.deathTime == 20) {
            if (!EntityLivingBase.worldObj.isRemote && (this.recentlyHit > 0 || this.isPlayer()) && this.canDropLoot() && worldObj.getGameRules().getBoolean("doMobLoot")) {
                int i = this.getExperiencePoints(this.attackingPlayer);
                while (i > 0) {
                    int j = EntityXPOrb.getXPSplit(i);
                    i -= j;
                    worldObj.spawnEntityInWorld(new EntityXPOrb(worldObj, this.posX, this.posY, this.posZ, j));
                }
            }
            this.setDead();
            int k = 0;
            while (k < 20) {
                ...
                worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, this.posX + (double)(rand.nextFloat() * this.width * 2.0f) - (double)this.width, this.posY + (double)(rand.nextFloat() * this.height), this.posZ + (double)(rand.nextFloat() * this.width * 2.0f) - (double)this.width, d2, d0, d1, new int[0]);
                ++k;
            }
        }
    }
    ...
    public void writeEntityToNBT(NBTTagCompound tagCompound) {
        ...
        n2 = 0;
        while (n2 < n) {
            ItemStack itemstack1 = itemStackArray[n2];
            if (itemstack1 != null) {
    ...
    public void readEntityFromNBT(NBTTagCompound tagCompund) {
        ...
        if (tagCompund.hasKey("Attributes", 9) && worldObj != null && !EntityLivingBase.worldObj.isRemote) {
    ...
    protected void updatePotionEffects() {
        ...
        while (iterator.hasNext()) {
            ...
            if (!potioneffect.onUpdate(this)) {
                if (EntityLivingBase.worldObj.isRemote) continue;
                iterator.remove();
        ...
        if (i > 0) {
            ...
            if (!this.isInvisible()) {
                ...
                boolean bl2 = flag = rand.nextInt(15) == 0;
            }
            if (flag1) {
            ...
            if (flag && i > 0) {
                ...
                worldObj.spawnParticle(flag1 ? EnumParticleTypes.SPELL_MOB_AMBIENT : EnumParticleTypes.SPELL_MOB, this.posX + (rand.nextDouble() - 0.5) * (double)this.width, this.posY + rand.nextDouble() * (double)this.height, this.posZ + (rand.nextDouble() - 0.5) * (double)this.width, d0, d1, d2, new int[0]);
            }
        }
    }
    ...
    protected void onChangedPotionEffect(PotionEffect id, boolean p_70695_2_) {
        this.potionsNeedUpdate = true;
        if (p_70695_2_ && !EntityLivingBase.worldObj.isRemote) {
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        ...
        if ((source == DamageSource.anvil || source == DamageSource.fallingBlock) && this.getEquipmentInSlot(4) != null) {
            this.getEquipmentInSlot(4).damageItem((int)(amount * 4.0f + rand.nextFloat() * amount * 2.0f), this);
            amount *= 0.75f;
        }
    ...
    public void renderBrokenItemStack(ItemStack stack) {
        this.playSound("random.break", 0.8f, 0.8f + EntityLivingBase.worldObj.rand.nextFloat() * 0.4f);
        int i = 0;
        while (i < 5) {
            ...
            Vec3 vec31 = new Vec3(((double)rand.nextFloat() - 0.5) * 0.3, d0, 0.6);
            ...
            worldObj.spawnParticle(EnumParticleTypes.ITEM_CRACK, vec31.xCoord, vec31.yCoord, vec31.zCoord, vec3.xCoord, vec3.yCoord + 0.05, vec3.zCoord, Item.getIdFromItem(stack.getItem()));
            ++i;
        }
    }
    ...
    public void onDeath(DamageSource cause) {
        ...
        if (!EntityLivingBase.worldObj.isRemote) {
            ...
            if (this.canDropLoot() && worldObj.getGameRules().getBoolean("doMobLoot")) {
                this.dropFewItems(this.recentlyHit > 0, i);
                this.dropEquipment(this.recentlyHit > 0, i);
                if (this.recentlyHit > 0 && rand.nextFloat() < 0.025f + (float)i * 0.01f) {
                    this.addRandomDrop();
                }
            }
        }
    ...
    public void knockBack(Entity entityIn, float p_70653_2_, double p_70653_3_, double p_70653_5_) {
        if (rand.nextDouble() >= this.getEntityAttribute(SharedMonsterAttributes.knockbackResistance).getAttributeValue()) {
    ...
    public boolean isOnLadder() {
        ...
        Block block = worldObj.getBlockState(new BlockPos(i, j = MathHelper.floor_double(this.getEntityBoundingBox().minY), k = MathHelper.floor_double(this.posZ))).getBlock();
    ...
    private int getArmSwingAnimationEnd() {
        if (Client.INSTANCE.moduleManager.getModule("HitSlow").isState()) {
            return (int)(this.isPotionActive(Potion.digSpeed) ? (double)(6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1) : (this.isPotionActive(Potion.digSlowdown) ? (double)(6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2) : HitSlow.hitSlowdown));
        }
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 2) {
            ...
            if (s != null) {
                this.playSound(this.getHurtSound(), this.getSoundVolume(), (rand.nextFloat() - rand.nextFloat()) * 0.2f + 1.0f);
            }
            this.attackEntityFrom(DamageSource.generic, 0.0f);
        } else if (id == 3) {
            String s1 = this.getDeathSound();
            if (s1 != null) {
                this.playSound(this.getDeathSound(), this.getSoundVolume(), (rand.nextFloat() - rand.nextFloat()) * 0.2f + 1.0f);
            }
            this.setHealth(0.0f);
    ...
    protected float getSoundPitch() {
        return this.isChild() ? (rand.nextFloat() - rand.nextFloat()) * 0.2f + 1.5f : (rand.nextFloat() - rand.nextFloat()) * 0.2f + 1.0f;
    }
            ...
            while (k < i) {
                if (j != 0 || k != 0) {
                    ...
                    if (worldObj.func_147461_a(axisalignedbb).isEmpty()) {
                        if (World.doesBlockHaveSolidTopSurface(worldObj, new BlockPos(l, (int)this.posY, i1))) {
                            this.setPositionAndUpdate(this.posX + (double)j, this.posY + 1.0, this.posZ + (double)k);
                            return;
                        }
                        if (World.doesBlockHaveSolidTopSurface(worldObj, new BlockPos(l, (int)this.posY - 1, i1)) || worldObj.getBlockState(new BlockPos(l, (int)this.posY - 1, i1)).getBlock().getMaterial() == Material.water) {
    ...
    protected void jump() {
        ...
        if (this.isSprinting()) {
            if (Client.INSTANCE.moduleManager.getModule((String)"TestAura").state && TestAura.target != null && ((CheckBox)Client.INSTANCE.moduleManager.getModule((String)"TestAura").getSetting((String)"JumpFix").getSetting()).state) {
                float f = Rotations.yaw * ((float)Math.PI / 180);
        ...
        if (this.isServerWorld()) {
            if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying) {
                if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying) {
                    float f4 = 0.91f;
                    if (this.onGround) {
                        f4 = EntityLivingBase.worldObj.getBlockState((BlockPos)new BlockPos((int)MathHelper.floor_double((double)this.posX), (int)(MathHelper.floor_double((double)this.getEntityBoundingBox().minY) - 1), (int)MathHelper.floor_double((double)this.posZ))).getBlock().slipperiness * 0.91f;
                    }
                    ...
                    this.motionY = !(!EntityLivingBase.worldObj.isRemote || worldObj.isBlockLoaded(new BlockPos((int)this.posX, 0, (int)this.posZ)) && worldObj.getChunkFromBlockCoords(new BlockPos((int)this.posX, 0, (int)this.posZ)).isLoaded()) ? (this.posY > 0.0 ? -0.1 : 0.0) : (this.motionY -= 0.08);
    ...
    public void onUpdate() {
        ...
        EntityLivingBase.worldObj.theProfiler.startSection("headTurn");
        f2 = this.func_110146_f(f1, f2);
        EntityLivingBase.worldObj.theProfiler.endSection();
        EntityLivingBase.worldObj.theProfiler.startSection("rangeChecks");
    ...
    public void onLivingUpdate() {
        ...
        EntityLivingBase.worldObj.theProfiler.startSection("ai");
        if (this.isMovementBlocked()) {
            ...
            EntityLivingBase.worldObj.theProfiler.startSection("newAi");
            this.updateEntityActionState();
            EntityLivingBase.worldObj.theProfiler.endSection();
        }
        EntityLivingBase.worldObj.theProfiler.endSection();
        EntityLivingBase.worldObj.theProfiler.startSection("jump");
        if (this.isJumping) {
            if (this.isInWater()) {
                this.updateAITick();
        ...
        EntityLivingBase.worldObj.theProfiler.endSection();
        EntityLivingBase.worldObj.theProfiler.startSection("travel");
        this.moveStrafing *= 0.98f;
        this.moveForward *= 0.98f;
        this.randomYawVelocity *= 0.9f;
        this.moveEntityWithHeading(this.moveStrafing, this.moveForward);
        EntityLivingBase.worldObj.theProfiler.endSection();
        EntityLivingBase.worldObj.theProfiler.startSection("push");
        if (!EntityLivingBase.worldObj.isRemote) {
    ...
    protected void collideWithNearbyEntities() {
        List<Entity> list = worldObj.getEntitiesInAABBexcluding(this, this.getEntityBoundingBox().expand(0.2f, 0.0, 0.2f), (Predicate<? super Entity>)Predicates.and(EntitySelectors.NOT_SPECTATING, (Predicate)new Predicate<Entity>(){
    ...
    public void onItemPickup(Entity p_71001_1_, int p_71001_2_) {
        if (!p_71001_1_.isDead && !EntityLivingBase.worldObj.isRemote) {
    ...
    public boolean canEntityBeSeen(Entity entityIn) {
        return worldObj.rayTraceBlocks(new Vec3(this.posX, this.posY + (double)this.getEyeHeight(), this.posZ), new Vec3(entityIn.posX, entityIn.posY + (double)entityIn.getEyeHeight(), entityIn.posZ)) == null;
    }
    ...
    public boolean isServerWorld() {
        return !EntityLivingBase.worldObj.isRemote;
    }
    ...
    protected void setBeenAttacked() {
        this.velocityChanged = rand.nextDouble() >= this.getEntityAttribute(SharedMonsterAttributes.knockbackResistance).getAttributeValue();
    }
    ...
    public Team getTeam() {
        return worldObj.getScoreboard().getPlayersTeam(this.getUniqueID().toString());
    }
