...
implements ICommandSender {
    ...
    public Entity(World worldIn) {
        ...
        this.entityUniqueID = MathHelper.getRandomUuid(rand);
    ...
    protected void preparePlayerToSpawn() {
        if (worldObj != null) {
            while (this.posY > 0.0 && this.posY < 256.0) {
                ...
                if (worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox()).isEmpty()) break;
                this.posY += 1.0;
            }
            this.motionZ = 0.0;
            this.motionY = 0.0;
    ...
    protected void setSize(float width, float height) {
        if (width != this.width || height != this.height) {
            ...
            if (this.width > f && !this.firstUpdate && !Entity.worldObj.isRemote) {
    ...
    public void onEntityUpdate() {
        ...
        if (!Entity.worldObj.isRemote && worldObj instanceof WorldServer) {
            ...
            if (this.inPortal) {
                if (minecraftserver.getAllowNether()) {
                    if (this.ridingEntity == null && this.portalCounter++ >= i) {
                        ...
                        int j = Entity.worldObj.provider.getDimensionId() == -1 ? 0 : -1;
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            if (flag) {
                double d6 = 0.05;
                while (x != 0.0 && worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(x, -1.0, 0.0)).isEmpty()) {
                    x = x < d6 && x >= -d6 ? 0.0 : (x > 0.0 ? (x -= d6) : (x += d6));
                    d3 = x;
                }
                while (z != 0.0 && worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(0.0, -1.0, z)).isEmpty()) {
                    z = z < d6 && z >= -d6 ? 0.0 : (z > 0.0 ? (z -= d6) : (z += d6));
                    d5 = z;
                }
                while (x != 0.0 && z != 0.0 && worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(x, -1.0, z)).isEmpty()) {
                    x = x < d6 && x >= -d6 ? 0.0 : (x > 0.0 ? (x -= d6) : (x += d6));
                    d3 = x;
                    z = z < d6 && z >= -d6 ? 0.0 : (z > 0.0 ? (z -= d6) : (z += d6));
                    d5 = z;
            ...
            List<AxisAlignedBB> list1 = worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().addCoord(x, y, z));
            ...
            if (this.stepHeight > 0.0f && flag1 && (d3 != x || d5 != z)) {
                ...
                y = this.stepHeight;
                List<AxisAlignedBB> list = worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().addCoord(d3, y, d5));
            ...
            if (block1.getMaterial() == Material.air && ((block = worldObj.getBlockState(blockpos.down()).getBlock()) instanceof BlockFence || block instanceof BlockWall || block instanceof BlockFenceGate)) {
                block1 = block;
            ...
            if (this.canTriggerWalking() && !flag && this.ridingEntity == null) {
                ...
                if (this.distanceWalkedOnStepModified > (float)this.nextStepDistance && block1.getMaterial() != Material.air) {
                    ...
                    if (this.isInWater()) {
                        ...
                        this.playSound(this.getSwimSound(), f, 1.0f + (rand.nextFloat() - rand.nextFloat()) * 0.4f);
                    }
    ...
    protected void updateFallState(double y, boolean onGroundIn, Block blockIn, BlockPos pos) {
        if (onGroundIn) {
            if (this.fallDistance > 0.0f) {
                if (blockIn != null) {
                    blockIn.onFallenUpon(worldObj, pos, this, this.fallDistance);
                } else {
    ...
    public boolean isWet() {
        return this.inWater || worldObj.canLightningStrike(new BlockPos(this.posX, this.posY, this.posZ)) || worldObj.canLightningStrike(new BlockPos(this.posX, this.posY + (double)this.height, this.posZ));
    }
    ...
    public boolean handleWaterMovement() {
        if (worldObj.handleMaterialAcceleration(this.getEntityBoundingBox().expand(0.0, -0.4f, 0.0).contract(0.001, 0.001, 0.001), Material.water, this)) {
    ...
    protected void resetHeight() {
        ...
        this.playSound(this.getSplashSound(), f, 1.0f + (rand.nextFloat() - rand.nextFloat()) * 0.4f);
        float f1 = MathHelper.floor_double(this.getEntityBoundingBox().minY);
        int i = 0;
        while ((float)i < 1.0f + this.width * 20.0f) {
            float f2 = (rand.nextFloat() * 2.0f - 1.0f) * this.width;
            float f3 = (rand.nextFloat() * 2.0f - 1.0f) * this.width;
            worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX + (double)f2, (double)(f1 + 1.0f), this.posZ + (double)f3, this.motionX, this.motionY - (double)(rand.nextFloat() * 0.2f), this.motionZ, new int[0]);
            ++i;
        }
        int j = 0;
        while ((float)j < 1.0f + this.width * 20.0f) {
            float f4 = (rand.nextFloat() * 2.0f - 1.0f) * this.width;
            float f5 = (rand.nextFloat() * 2.0f - 1.0f) * this.width;
            worldObj.spawnParticle(EnumParticleTypes.WATER_SPLASH, this.posX + (double)f4, (double)(f1 + 1.0f), this.posZ + (double)f5, this.motionX, this.motionY, this.motionZ, new int[0]);
            ++j;
        }
    }
    ...
    protected void createRunningParticles() {
        ...
        if (block.getRenderType() != -1) {
            worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + ((double)rand.nextFloat() - 0.5) * (double)this.width, this.getEntityBoundingBox().minY + 0.1, this.posZ + ((double)rand.nextFloat() - 0.5) * (double)this.width, -this.motionX * 4.0, 1.5, -this.motionZ * 4.0, Block.getStateId(iblockstate));
        }
    }
    ...
    public boolean isInLava() {
        return worldObj.isMaterialInBB(this.getEntityBoundingBox().expand(-0.1f, -0.4f, -0.1f), Material.lava);
    }
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        if (f >= 1.0E-4f) {
            ...
            forward *= f;
            try {
                if (Client.INSTANCE.moduleManager.getModule("TestAura").isState() && TestAura.target != null && ((CheckBox)Client.INSTANCE.moduleManager.getModule((String)"TestAura").getSetting((String)"MoveFix").getSetting()).state || Client.INSTANCE.moduleManager.getModule("TestAura").isState() && TestAura.target != null && ((CheckBox)Client.INSTANCE.moduleManager.getModule((String)"TestAura").getSetting((String)"SilentMoveFix").getSetting()).state) {
    ...
    public Vec3 getLook(float partialTicks) {
        ...
        if (EventPreMotion.getInstance != null) {
            return this.getVectorForRotation(EventPreMotion.getInstance.getPitch(), EventPreMotion.getInstance.getYaw());
        }
    ...
    public EntityItem entityDropItem(ItemStack itemStackIn, float offsetY) {
        if (itemStackIn.stackSize != 0 && itemStackIn.getItem() != null) {
            EntityItem entityitem = new EntityItem(worldObj, this.posX, this.posY + (double)offsetY, this.posZ, itemStackIn);
    ...
    public void setPositionAndRotation2(double x, double y, double z, float yaw, float pitch, int posRotationIncrements, boolean p_180426_10_) {
        ...
        this.setRotation(yaw, pitch);
        List<AxisAlignedBB> list = worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().contract(0.03125, 0.0, 0.03125));
        if (!list.isEmpty()) {
            double d0 = 0.0;
    ...
    public void func_181015_d(BlockPos p_181015_1_) {
        if (this.timeUntilPortal > 0) {
            ...
            if (!Entity.worldObj.isRemote && !p_181015_1_.equals(this.field_181016_an)) {
                this.field_181016_an = p_181015_1_;
                BlockPattern.PatternHelper blockpattern$patternhelper = Blocks.portal.func_181089_f(worldObj, p_181015_1_);
    ...
    public boolean isBurning() {
        boolean flag;
        boolean bl = flag = worldObj != null && Entity.worldObj.isRemote;
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z) {
        ...
        if (!worldObj.isBlockFullCube(blockpos.east()) && 1.0 - d0 < d3) {
            d3 = 1.0 - d0;
            i = 1;
        }
        if (!worldObj.isBlockFullCube(blockpos.up()) && 1.0 - d1 < d3) {
            d3 = 1.0 - d1;
            i = 3;
        }
        ...
        if (!worldObj.isBlockFullCube(blockpos.south()) && 1.0 - d2 < d3) {
            d3 = 1.0 - d2;
            i = 5;
        }
    ...
    public String toString() {
        return String.format("%s['%s'/%d, l='%s', x=%.2f, y=%.2f, z=%.2f]", this.getClass().getSimpleName(), this.getName(), this.entityId, worldObj == null ? "~NULL~" : worldObj.getWorldInfo().getWorldName(), this.posX, this.posY, this.posZ);
    }
    ...
    public void travelToDimension(int dimensionId) {
        if (!Entity.worldObj.isRemote && !this.isDead) {
