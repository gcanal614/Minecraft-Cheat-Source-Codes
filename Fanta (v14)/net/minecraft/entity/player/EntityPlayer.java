...
extends EntityLivingBase {
    ...
    public void stopUsingItem() {
        if (this.itemInUse != null) {
            this.itemInUse.onPlayerStoppedUsing(worldObj, this, this.itemInUseCount);
        }
        this.clearItemInUse();
    }
    ...
    public void onUpdate() {
        ...
        if (this.itemInUse != null) {
            ...
            if (itemstack == this.itemInUse) {
                ...
                if (--this.itemInUseCount == 0 && !EntityPlayer.worldObj.isRemote) {
        ...
        super.onUpdate();
        if (!EntityPlayer.worldObj.isRemote && this.openContainer != null && !this.openContainer.canInteractWith(this)) {
            this.closeScreen();
    ...
    protected void updateItemUse(ItemStack itemStackIn, int p_71010_2_) {
        if (itemStackIn.getItemUseAction() == EnumAction.DRINK) {
            this.playSound("random.drink", 0.5f, EntityPlayer.worldObj.rand.nextFloat() * 0.1f + 0.9f);
        }
        if (itemStackIn.getItemUseAction() == EnumAction.EAT) {
            ...
            while (i < p_71010_2_) {
                ...
                if (itemStackIn.getHasSubtypes()) {
                    worldObj.spawnParticle(EnumParticleTypes.ITEM_CRACK, vec31.xCoord, vec31.yCoord, vec31.zCoord, vec3.xCoord, vec3.yCoord + 0.05, vec3.zCoord, Item.getIdFromItem(itemStackIn.getItem()), itemStackIn.getMetadata());
                } else {
                    worldObj.spawnParticle(EnumParticleTypes.ITEM_CRACK, vec31.xCoord, vec31.yCoord, vec31.zCoord, vec3.xCoord, vec3.yCoord + 0.05, vec3.zCoord, Item.getIdFromItem(itemStackIn.getItem()));
                }
                ++i;
            }
            this.playSound("random.eat", 0.5f + 0.5f * (float)rand.nextInt(2), (rand.nextFloat() - rand.nextFloat()) * 0.2f + 1.0f);
        }
    }
    ...
    protected void onItemUseFinish() {
        if (this.itemInUse != null) {
            ...
            ItemStack itemstack = this.itemInUse.onItemUseFinish(worldObj, this);
    ...
    public void updateRidden() {
        if (!EntityPlayer.worldObj.isRemote && this.isSneaking()) {
    ...
    public void onLivingUpdate() {
        ...
        if (Client.INSTANCE.moduleManager.getModule("Regen").isState()) {
            if (worldObj.getDifficulty() == EnumDifficulty.PEACEFUL && this.getHealth() < this.getMaxHealth()) {
                this.heal(6.0f);
            }
        } else if (worldObj.getDifficulty() == EnumDifficulty.PEACEFUL && worldObj.getGameRules().getBoolean("naturalRegeneration")) {
    ...
    public EntityItem dropItem(ItemStack droppedItem, boolean dropAround, boolean traceItem) {
        ...
        EntityItem entityitem = new EntityItem(worldObj, this.posX, d0, this.posZ, droppedItem);
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        ...
        if (this.isPlayerSleeping() && !EntityPlayer.worldObj.isRemote) {
            this.wakeUpPlayer(true, true, false);
        }
    ...
    public float getArmorVisibility() {
        ...
        int n = this.inventory.armorInventory.length;
        int n2 = 0;
        while (n2 < n) {
            ...
            if (f > 0.0f || f1 > 0.0f) {
                ...
                if (flag2) {
                    ...
                    if (i > 0) {
                        ...
                        if (!((CheckBox)Client.INSTANCE.moduleManager.getModule((String)"TestAura").getSetting((String)"KeepSprint").getSetting()).state) {
                            this.motionX *= 0.6;
                            this.motionZ *= 0.6;
    ...
    public void wakeUpPlayer(boolean p_70999_1_, boolean updateWorldFlag, boolean setSpawn) {
        this.setSize(0.6f, 1.8f);
        IBlockState iblockstate = worldObj.getBlockState(this.playerLocation);
        if (this.playerLocation != null && iblockstate.getBlock() == Blocks.bed) {
            worldObj.setBlockState(this.playerLocation, iblockstate.withProperty(BlockBed.OCCUPIED, false), 4);
            BlockPos blockpos = BlockBed.getSafeExitLocation(worldObj, this.playerLocation, 0);
            if (blockpos == null) {
        ...
        this.sleeping = false;
        if (!EntityPlayer.worldObj.isRemote && updateWorldFlag) {
            worldObj.updateAllPlayersSleepingFlag();
        }
    ...
    private boolean isInBed() {
        return worldObj.getBlockState(this.playerLocation).getBlock() == Blocks.bed;
    }
    ...
    public float getBedOrientationInDegrees() {
        if (this.playerLocation != null) {
            EnumFacing enumfacing = worldObj.getBlockState(this.playerLocation).getValue(BlockDirectional.FACING);
            switch (enumfacing) {
                case SOUTH: {
                    return 90.0f;
                }
    ...
    public void addExhaustion(float p_71020_1_) {
        if (!this.capabilities.disableDamage && !EntityPlayer.worldObj.isRemote) {
