...
extends EntityHanging {
    ...
    public boolean interactFirst(EntityPlayer playerIn) {
        ...
        boolean flag = false;
        if (itemstack != null && itemstack.getItem() == Items.lead && !EntityLeashKnot.worldObj.isRemote) {
            double d0 = 7.0;
            for (EntityLiving entityliving : worldObj.getEntitiesWithinAABB(EntityLiving.class, new AxisAlignedBB(this.posX - d0, this.posY - d0, this.posZ - d0, this.posX + d0, this.posY + d0, this.posZ + d0))) {
                if (!entityliving.getLeashed() || entityliving.getLeashedToEntity() != playerIn) continue;
                entityliving.setLeashedToEntity(this, true);
                flag = true;
            }
        }
        if (!EntityLeashKnot.worldObj.isRemote && !flag) {
            this.setDead();
            if (playerIn.capabilities.isCreativeMode) {
                double d1 = 7.0;
                for (EntityLiving entityliving1 : worldObj.getEntitiesWithinAABB(EntityLiving.class, new AxisAlignedBB(this.posX - d1, this.posY - d1, this.posZ - d1, this.posX + d1, this.posY + d1, this.posZ + d1))) {
    ...
    public boolean onValidSurface() {
        return worldObj.getBlockState(this.hangingPosition).getBlock() instanceof BlockFence;
    }
