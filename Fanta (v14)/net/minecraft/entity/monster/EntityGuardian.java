...
extends EntityMob {
    ...
    public EntityGuardian(World worldIn) {
        ...
        this.field_175484_c = this.field_175482_b = rand.nextFloat();
    }
    ...
    public EntityLivingBase getTargetedEntity() {
        ...
        if (EntityGuardian.worldObj.isRemote) {
            if (this.targetedEntity != null) {
                return this.targetedEntity;
            }
            Entity entity = worldObj.getEntityByID(this.dataWatcher.getWatchableObjectInt(17));
    ...
    public void onLivingUpdate() {
        if (EntityGuardian.worldObj.isRemote) {
            ...
            if (!this.isInWater()) {
                ...
                if (this.motionY > 0.0 && this.field_175480_bp && !this.isSilent()) {
                    worldObj.playSound(this.posX, this.posY, this.posZ, "mob.guardian.flop", 1.0f, 1.0f, false);
                }
                this.field_175480_bp = this.motionY < 0.0 && worldObj.isBlockNormalCube(new BlockPos(this).down(), false);
            } else {
            ...
            this.field_175485_bl = !this.isInWater() ? rand.nextFloat() : (this.func_175472_n() ? (this.field_175485_bl += (0.0f - this.field_175485_bl) * 0.25f) : (this.field_175485_bl += (1.0f - this.field_175485_bl) * 0.06f));
            if (this.func_175472_n() && this.isInWater()) {
                Vec3 vec3 = this.getLook(0.0f);
                int i = 0;
                while (i < 2) {
                    worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX + (rand.nextDouble() - 0.5) * (double)this.width - vec3.xCoord * 1.5, this.posY + rand.nextDouble() * (double)this.height - vec3.yCoord * 1.5, this.posZ + (rand.nextDouble() - 0.5) * (double)this.width - vec3.zCoord * 1.5, 0.0, 0.0, 0.0, new int[0]);
                    ++i;
                }
            }
            if (this.hasTargetedEntity()) {
                ...
                if ((entitylivingbase = this.getTargetedEntity()) != null) {
                    ...
                    while (d4 < d3) {
                        worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX + d0 * (d4 += 1.8 - d5 + rand.nextDouble() * (1.7 - d5)), this.posY + d1 * d4 + (double)this.getEyeHeight(), this.posZ + d2 * d4, 0.0, 0.0, 0.0, new int[0]);
                    }
                }
            }
        }
        if (this.inWater) {
            ...
            this.motionY += 0.5;
            this.motionX += (double)((rand.nextFloat() * 2.0f - 1.0f) * 0.4f);
            this.motionZ += (double)((rand.nextFloat() * 2.0f - 1.0f) * 0.4f);
            this.rotationYaw = rand.nextFloat() * 360.0f;
            this.onGround = false;
    ...
    protected void dropFewItems(boolean p_70628_1_, int p_70628_2_) {
        int i = rand.nextInt(3) + rand.nextInt(p_70628_2_ + 1);
        if (i > 0) {
    ...
    public boolean isNotColliding() {
        return worldObj.checkNoEntityCollision(this.getEntityBoundingBox(), this) && worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox()).isEmpty();
    }
        ...
        public void updateTask() {
            ...
            if (!this.theEntity.canEntityBeSeen(entitylivingbase)) {
                ...
                if (this.tickCounter == 0) {
                    ...
                    worldObj.setEntityState(this.theEntity, (byte)21);
