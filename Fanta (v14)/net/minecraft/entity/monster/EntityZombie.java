...
extends EntityMob {
    ...
    protected void applyEntityAttributes() {
        ...
        this.getAttributeMap().registerAttribute(reinforcementChance).setBaseValue(rand.nextDouble() * (double)0.1f);
    }
    ...
    public void setChild(boolean childZombie) {
        ...
        if (worldObj != null && !EntityZombie.worldObj.isRemote) {
    ...
    public void onLivingUpdate() {
        if (worldObj.isDaytime() && !EntityZombie.worldObj.isRemote && !this.isChild()) {
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        if (super.attackEntityFrom(source, amount)) {
            ...
            if (entitylivingbase != null && worldObj.getDifficulty() == EnumDifficulty.HARD && (double)rand.nextFloat() < this.getEntityAttribute(reinforcementChance).getAttributeValue()) {
                ...
                while (l < 50) {
                    ...
                    if (World.doesBlockHaveSolidTopSurface(worldObj, new BlockPos(i1, (j1 = j + MathHelper.getRandomIntegerInRange(rand, 7, 40) * MathHelper.getRandomIntegerInRange(rand, -1, 1)) - 1, k1 = k + MathHelper.getRandomIntegerInRange(rand, 7, 40) * MathHelper.getRandomIntegerInRange(rand, -1, 1))) && worldObj.getLightFromNeighbors(new BlockPos(i1, j1, k1)) < 10) {
                        entityzombie.setPosition(i1, j1, k1);
                        if (!worldObj.isAnyPlayerWithinRangeAt(i1, j1, k1, 7.0) && worldObj.checkNoEntityCollision(entityzombie.getEntityBoundingBox(), entityzombie) && worldObj.getCollidingBoundingBoxes(entityzombie, entityzombie.getEntityBoundingBox()).isEmpty() && !worldObj.isAnyLiquid(entityzombie.getEntityBoundingBox())) {
    ...
    public void onUpdate() {
        if (!EntityZombie.worldObj.isRemote && this.isConverting()) {
    ...
    public boolean attackEntityAsMob(Entity entityIn) {
        ...
        if (flag) {
            ...
            if (this.getHeldItem() == null && this.isBurning() && rand.nextFloat() < (float)i * 0.3f) {
    ...
    protected void setEquipmentBasedOnDifficulty(DifficultyInstance difficulty) {
        ...
        float f2 = worldObj.getDifficulty() == EnumDifficulty.HARD ? 0.05f : 0.01f;
        if (f < f2) {
    ...
    public void onKillEntity(EntityLivingBase entityLivingIn) {
        ...
        if ((worldObj.getDifficulty() == EnumDifficulty.NORMAL || worldObj.getDifficulty() == EnumDifficulty.HARD) && entityLivingIn instanceof EntityVillager) {
            ...
            worldObj.spawnEntityInWorld(entityzombie);
            worldObj.playAuxSFXAtEntity(null, 1016, new BlockPos((int)this.posX, (int)this.posY, (int)this.posZ), 0);
        }
    }
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, IEntityLivingData livingdata) {
        ...
        if (livingdata == null) {
            livingdata = new GroupData(EntityZombie.worldObj.rand.nextFloat() < 0.05f, EntityZombie.worldObj.rand.nextFloat() < 0.05f);
        }
        if (livingdata instanceof GroupData) {
            ...
            if (entityzombie$groupdata.isChild) {
                this.setChild(true);
                if ((double)EntityZombie.worldObj.rand.nextFloat() < 0.05) {
                    List<Entity> list = worldObj.getEntitiesWithinAABB(EntityChicken.class, this.getEntityBoundingBox().expand(5.0, 3.0, 5.0), EntitySelectors.IS_STANDALONE);
                    if (!list.isEmpty()) {
                        EntityChicken entitychicken = (EntityChicken)list.get(0);
                        entitychicken.setChickenJockey(true);
                        this.mountEntity(entitychicken);
                    }
                } else if ((double)EntityZombie.worldObj.rand.nextFloat() < 0.05) {
        ...
        this.setBreakDoorsAItask(rand.nextFloat() < f * 0.1f);
        this.setEquipmentBasedOnDifficulty(difficulty);
        this.setEnchantmentBasedOnDifficulty(difficulty);
        if (this.getEquipmentInSlot(4) == null && (calendar = worldObj.getCurrentDate()).get(2) + 1 == 10 && calendar.get(5) == 31 && rand.nextFloat() < 0.25f) {
            this.setCurrentItemOrArmor(4, new ItemStack(rand.nextFloat() < 0.1f ? Blocks.lit_pumpkin : Blocks.pumpkin));
            this.equipmentDropChances[4] = 0.0f;
        }
        this.getEntityAttribute(SharedMonsterAttributes.knockbackResistance).applyModifier(new AttributeModifier("Random spawn bonus", rand.nextDouble() * (double)0.05f, 0));
        double d0 = rand.nextDouble() * 1.5 * (double)f;
        if (d0 > 1.0) {
        ...
        if (rand.nextFloat() < f * 0.05f) {
            this.getEntityAttribute(reinforcementChance).applyModifier(new AttributeModifier("Leader zombie bonus", rand.nextDouble() * 0.25 + 0.5, 0));
            this.getEntityAttribute(SharedMonsterAttributes.maxHealth).applyModifier(new AttributeModifier("Leader zombie bonus", rand.nextDouble() * 3.0 + 1.0, 2));
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (itemstack != null && itemstack.getItem() == Items.golden_apple && itemstack.getMetadata() == 0 && this.isVillager() && this.isPotionActive(Potion.weakness)) {
            ...
            if (!EntityZombie.worldObj.isRemote) {
                this.startConversion(rand.nextInt(2401) + 3600);
            }
            return true;
        }
        return false;
    ...
    protected void startConversion(int ticks) {
        ...
        this.addPotionEffect(new PotionEffect(Potion.damageBoost.id, ticks, Math.min(worldObj.getDifficulty().getDifficultyId() - 1, 0)));
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 16) {
            if (!this.isSilent()) {
                worldObj.playSound(this.posX + 0.5, this.posY + 0.5, this.posZ + 0.5, "mob.zombie.remedy", 1.0f + rand.nextFloat(), rand.nextFloat() * 0.7f + 0.3f, false);
            }
        } else {
    ...
    protected void convertToVillager() {
        ...
        worldObj.playAuxSFXAtEntity(null, 1017, new BlockPos((int)this.posX, (int)this.posY, (int)this.posZ), 0);
    }
            ...
            while (k < (int)this.posX + 4 && j < 14) {
                ...
                while (l < (int)this.posY + 4 && j < 14) {
                    ...
                    while (i1 < (int)this.posZ + 4 && j < 14) {
                        Block block = worldObj.getBlockState(blockpos$mutableblockpos.func_181079_c(k, l, i1)).getBlock();
