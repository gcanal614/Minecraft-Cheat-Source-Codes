...
extends EntityMob {
    ...
    public void onLivingUpdate() {
        ...
        if (EntityBlaze.worldObj.isRemote) {
            if (rand.nextInt(24) == 0 && !this.isSilent()) {
                worldObj.playSound(this.posX + 0.5, this.posY + 0.5, this.posZ + 0.5, "fire.fire", 1.0f + rand.nextFloat(), rand.nextFloat() * 0.7f + 0.3f, false);
            }
            int i = 0;
            while (i < 2) {
                worldObj.spawnParticle(EnumParticleTypes.SMOKE_LARGE, this.posX + (rand.nextDouble() - 0.5) * (double)this.width, this.posY + rand.nextDouble() * (double)this.height, this.posZ + (rand.nextDouble() - 0.5) * (double)this.width, 0.0, 0.0, 0.0, new int[0]);
                ++i;
            }
        }
    ...
    protected void updateAITasks() {
        ...
        if (this.heightOffsetUpdateTime <= 0) {
            ...
            this.heightOffset = 0.5f + (float)rand.nextGaussian() * 3.0f;
        }
            ...
            if (d0 < 4.0) {
                ...
                if (this.field_179468_c <= 0) {
                    ...
                    if (this.field_179467_b > 1) {
                        ...
                        worldObj.playAuxSFXAtEntity(null, 1009, new BlockPos((int)this.blaze.posX, (int)this.blaze.posY, (int)this.blaze.posZ), 0);
                        int i = 0;
                        while (i < 1) {
                            EntitySmallFireball entitysmallfireball = new EntitySmallFireball(worldObj, this.blaze, d1 + this.blaze.getRNG().nextGaussian() * (double)f, d2, d3 + this.blaze.getRNG().nextGaussian() * (double)f);
