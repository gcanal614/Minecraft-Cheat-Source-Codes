...
extends EntityMob {
    ...
    public void onLivingUpdate() {
        if (EntityEnderman.worldObj.isRemote) {
            int i = 0;
            while (i < 2) {
                worldObj.spawnParticle(EnumParticleTypes.PORTAL, this.posX + (rand.nextDouble() - 0.5) * (double)this.width, this.posY + rand.nextDouble() * (double)this.height - 0.25, this.posZ + (rand.nextDouble() - 0.5) * (double)this.width, (rand.nextDouble() - 0.5) * 2.0, -rand.nextDouble(), (rand.nextDouble() - 0.5) * 2.0, new int[0]);
                ++i;
            }
        }
    ...
    protected void updateAITasks() {
        ...
        if (this.isScreaming() && !this.isAggressive && rand.nextInt(100) == 0) {
            this.setScreaming(false);
        }
        if (worldObj.isDaytime() && (f = this.getBrightness(1.0f)) > 0.5f && worldObj.canSeeSky(new BlockPos(this)) && rand.nextFloat() * 30.0f < (f - 0.4f) * 2.0f) {
    ...
    protected boolean teleportRandomly() {
        double d0 = this.posX + (rand.nextDouble() - 0.5) * 64.0;
        double d1 = this.posY + (double)(rand.nextInt(64) - 32);
        double d2 = this.posZ + (rand.nextDouble() - 0.5) * 64.0;
    ...
    protected boolean teleportToEntity(Entity p_70816_1_) {
        ...
        vec3 = vec3.normalize();
        double d0 = 16.0;
        double d1 = this.posX + (rand.nextDouble() - 0.5) * 8.0 - vec3.xCoord * d0;
        double d2 = this.posY + (double)(rand.nextInt(16) - 8) - vec3.yCoord * d0;
        double d3 = this.posZ + (rand.nextDouble() - 0.5) * 8.0 - vec3.zCoord * d0;
    ...
    protected boolean teleportTo(double x, double y, double z) {
        ...
        if (worldObj.isBlockLoaded(blockpos)) {
            ...
            if (flag1) {
                ...
                if (worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox()).isEmpty() && !worldObj.isAnyLiquid(this.getEntityBoundingBox())) {
                    flag = true;
                }
            }
        }
        ...
        while (j < i) {
            ...
            double d3 = d0 + (this.posX - d0) * d6 + (rand.nextDouble() - 0.5) * (double)this.width * 2.0;
            double d4 = d1 + (this.posY - d1) * d6 + rand.nextDouble() * (double)this.height;
            double d5 = d2 + (this.posZ - d2) * d6 + (rand.nextDouble() - 0.5) * (double)this.width * 2.0;
    ...
    extends EntityAINearestAttackableTarget {
        ...
        public boolean shouldExecute() {
            ...
            List<EntityPlayer> list = EntityCreature.worldObj.getEntitiesWithinAABB(EntityPlayer.class, this.taskOwner.getEntityBoundingBox().expand(d0, 4.0, d0), this.targetEntitySelector);
    ...
    extends EntityAIBase {
        ...
        @Override
        public boolean shouldExecute() {
            return !worldObj.getGameRules().getBoolean("mobGriefing") ? false : (this.enderman.getHeldBlockState().getBlock().getMaterial() == Material.air ? false : this.enderman.getRNG().nextInt(2000) == 0);
        }
    ...
    extends EntityAIBase {
        ...
        @Override
        public boolean shouldExecute() {
            return !worldObj.getGameRules().getBoolean("mobGriefing") ? false : (this.enderman.getHeldBlockState().getBlock().getMaterial() != Material.air ? false : this.enderman.getRNG().nextInt(20) == 0);
        }
