...
implements IRangedAttackMob {
    ...
    public void onLivingUpdate() {
        if (!EntityWitch.worldObj.isRemote) {
            if (this.getAggressive()) {
                ...
                int i = -1;
                if (rand.nextFloat() < 0.15f && this.isInsideOfMaterial(Material.water) && !this.isPotionActive(Potion.waterBreathing)) {
                    i = 8237;
                } else if (rand.nextFloat() < 0.15f && this.isBurning() && !this.isPotionActive(Potion.fireResistance)) {
                    i = 16307;
                } else if (rand.nextFloat() < 0.05f && this.getHealth() < this.getMaxHealth()) {
                    i = 16341;
                } else if (rand.nextFloat() < 0.25f && this.getAttackTarget() != null && !this.isPotionActive(Potion.moveSpeed) && this.getAttackTarget().getDistanceSqToEntity(this) > 121.0) {
                    i = 16274;
                } else if (rand.nextFloat() < 0.25f && this.getAttackTarget() != null && !this.isPotionActive(Potion.moveSpeed) && this.getAttackTarget().getDistanceSqToEntity(this) > 121.0) {
                    i = 16274;
                }
    ...
    public void handleStatusUpdate(byte id) {
        if (id == 15) {
            int i = 0;
            while (i < rand.nextInt(35) + 10) {
                worldObj.spawnParticle(EnumParticleTypes.SPELL_WITCH, this.posX + rand.nextGaussian() * (double)0.13f, this.getEntityBoundingBox().maxY + 0.5 + rand.nextGaussian() * (double)0.13f, this.posZ + rand.nextGaussian() * (double)0.13f, 0.0, 0.0, 0.0, new int[0]);
                ++i;
            }
        } else {
    ...
    public void attackEntityWithRangedAttack(EntityLivingBase p_82196_1_, float p_82196_2_) {
        if (!this.getAggressive()) {
            EntityPotion entitypotion = new EntityPotion(worldObj, (EntityLivingBase)this, 32732);
