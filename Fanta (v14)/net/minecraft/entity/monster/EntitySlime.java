...
implements IMob {
    ...
    public void onUpdate() {
        if (!EntitySlime.worldObj.isRemote && worldObj.getDifficulty() == EnumDifficulty.PEACEFUL && this.getSlimeSize() > 0) {
            this.isDead = true;
        }
    ...
    public void setDead() {
        int i = this.getSlimeSize();
        if (!EntitySlime.worldObj.isRemote && i > 1 && this.getHealth() <= 0.0f) {
            ...
            while (k < j) {
                ...
                entityslime.setLocationAndAngles(this.posX + (double)f, this.posY + 0.5, this.posZ + (double)f1, rand.nextFloat() * 360.0f, 0.0f);
    ...
    public boolean getCanSpawnHere() {
        ...
        if (worldObj.getDifficulty() != EnumDifficulty.PEACEFUL) {
            ...
            if (biomegenbase == BiomeGenBase.swampland && this.posY > 50.0 && this.posY < 70.0 && rand.nextFloat() < 0.5f && rand.nextFloat() < worldObj.getCurrentMoonPhaseFactor() && worldObj.getLightFromNeighbors(new BlockPos(this)) <= rand.nextInt(8)) {
                return super.getCanSpawnHere();
            }
            if (rand.nextInt(10) == 0 && chunk.getRandomWithSeed(987234911L).nextInt(10) == 0 && this.posY < 40.0) {
                return super.getCanSpawnHere();
            }
        }
        return false;
