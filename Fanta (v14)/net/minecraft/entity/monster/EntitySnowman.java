...
implements IRangedAttackMob {
    ...
    public void onLivingUpdate() {
        ...
        if (!EntitySnowman.worldObj.isRemote) {
            ...
            if (worldObj.getBiomeGenForCoords(blockPos).getFloatTemperature(blockPos2) > 1.0f) {
            ...
            while (l < 4) {
                ...
                if (worldObj.getBlockState(blockpos).getBlock().getMaterial() == Material.air) {
                    BlockPos blockPos3 = new BlockPos(i, 0, k);
                    if (worldObj.getBiomeGenForCoords(blockPos3).getFloatTemperature(blockpos) < 0.8f && Blocks.snow_layer.canPlaceBlockAt(worldObj, blockpos)) {
