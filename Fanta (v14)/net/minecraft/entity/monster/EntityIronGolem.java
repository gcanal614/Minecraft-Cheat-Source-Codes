...
extends EntityGolem {
    ...
    protected void updateAITasks() {
        if (--this.homeCheckTimer <= 0) {
            this.homeCheckTimer = 70 + rand.nextInt(50);
            this.villageObj = worldObj.getVillageCollection().getNearestVillage(new BlockPos(this), 32);
    ...
    public void onLivingUpdate() {
        ...
        if (this.motionX * this.motionX + this.motionZ * this.motionZ > 2.500000277905201E-7 && rand.nextInt(5) == 0 && (block = (iblockstate = worldObj.getBlockState(new BlockPos(i = MathHelper.floor_double(this.posX), j = MathHelper.floor_double(this.posY - (double)0.2f), k = MathHelper.floor_double(this.posZ)))).getBlock()).getMaterial() != Material.air) {
            worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + ((double)rand.nextFloat() - 0.5) * (double)this.width, this.getEntityBoundingBox().minY + 0.1, this.posZ + ((double)rand.nextFloat() - 0.5) * (double)this.width, 4.0 * ((double)rand.nextFloat() - 0.5), 0.5, ((double)rand.nextFloat() - 0.5) * 4.0, Block.getStateId(iblockstate));
        }
    }
