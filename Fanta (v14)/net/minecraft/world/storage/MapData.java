...
extends WorldSavedData {
    ...
    public void updateVisiblePlayers(EntityPlayer player, ItemStack mapStack) {
        ...
        while (i < this.playersArrayList.size()) {
            ...
            if (!mapdata$mapinfo1.entityplayerObj.isDead && (mapdata$mapinfo1.entityplayerObj.inventory.hasItemStack(mapStack) || mapStack.isOnItemFrame())) {
                if (!mapStack.isOnItemFrame() && mapdata$mapinfo1.entityplayerObj.dimension == this.dimension) {
                    EntityPlayer cfr_ignored_0 = mapdata$mapinfo1.entityplayerObj;
                    this.updateDecorations(0, EntityPlayer.worldObj, mapdata$mapinfo1.entityplayerObj.getName(), mapdata$mapinfo1.entityplayerObj.posX, mapdata$mapinfo1.entityplayerObj.posZ, mapdata$mapinfo1.entityplayerObj.rotationYaw);
                }
            } else {
        ...
        if (mapStack.isOnItemFrame()) {
            ...
            this.updateDecorations(1, EntityPlayer.worldObj, "frame-" + entityitemframe.getEntityId(), blockpos.getX(), blockpos.getZ(), entityitemframe.facingDirection.getHorizontalIndex() * 90);
        }
